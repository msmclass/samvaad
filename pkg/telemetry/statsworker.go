...
type StatsWorker struct {
	...
	lock             sync.RWMutex
	outgoingPerTrack map[samvaad.TrackID][]*samvaad.AnalyticsStat
	incomingPerTrack map[samvaad.TrackID][]*samvaad.AnalyticsStat
...
func newStatsWorker(
	...
	s := &StatsWorker{
		...
		outgoingPerTrack:    make(map[samvaad.TrackID][]*samvaad.AnalyticsStat),
		incomingPerTrack:    make(map[samvaad.TrackID][]*samvaad.AnalyticsStat),
	}
	s.refCount.Activate(guard)
	return s
}

func (s *StatsWorker) OnTrackStat(trackID samvaad.TrackID, direction samvaad.StreamType, stat *samvaad.AnalyticsStat) {
	s.lock.Lock()
	if direction == samvaad.StreamType_DOWNSTREAM {
...
func (s *StatsWorker) ParticipantID() samvaad.ParticipantID {
...
func (s *StatsWorker) Flush(now time.Time, closeWait time.Duration) bool {
	...
	s.lock.Lock()
	stats := make([]*samvaad.AnalyticsStat, 0, len(s.incomingPerTrack)+len(s.outgoingPerTrack))

	incomingPerTrack := s.incomingPerTrack
	s.incomingPerTrack = make(map[samvaad.TrackID][]*samvaad.AnalyticsStat)

	outgoingPerTrack := s.outgoingPerTrack
	s.outgoingPerTrack = make(map[samvaad.TrackID][]*samvaad.AnalyticsStat)

	closed := !s.closedAt.IsZero() && now.Sub(s.closedAt) > closeWait
	s.lock.Unlock()

	stats = s.collectStats(ts, samvaad.StreamType_UPSTREAM, incomingPerTrack, stats)
	stats = s.collectStats(ts, samvaad.StreamType_DOWNSTREAM, outgoingPerTrack, stats)
	if len(stats) > 0 {
...
func (s *StatsWorker) collectStats(
	...
	streamType samvaad.StreamType,
	perTrack map[samvaad.TrackID][]*samvaad.AnalyticsStat,
	stats []*samvaad.AnalyticsStat,
...
func coalesce(stats []*samvaad.AnalyticsStat) *samvaad.AnalyticsStat {
	if len(stats) == 0 {
		return nil
	}
	...
	maxRtt := uint32(0)
	maxJitter := uint32(0)
	coalescedVideoLayers := make(map[int32]*samvaad.AnalyticsVideoLayer)
	coalescedStream := &samvaad.AnalyticsStream{}
	...
	for _, coalescedVideoLayer := range coalescedVideoLayers {
		if maxVideoLayer == -1 || maxVideoLayer < coalescedVideoLayer.Layer {
			...
			coalescedStream.VideoLayers = []*samvaad.AnalyticsVideoLayer{coalescedVideoLayer}
		}
	}
	...
	stat := &samvaad.AnalyticsStat{
		...
		Streams:     []*samvaad.AnalyticsStream{coalescedStream},
...
func CondenseStat(stat *samvaad.AnalyticsStat) (ps CondensedStat, ok bool) {
...
func isValid(stat *samvaad.AnalyticsStat) bool {
	for _, analyticsStream := range stat.Streams {
