...
func initPacketStats(nodeID string, nodeType samvaad.NodeType) {
	promPacketTotal = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace:   SamvaadNamespace,
		Subsystem:   "packet",
		Name:        "total",
...
func RecordPacketOutOfOrder(country string, direction Direction, trackSource samvaad.TrackSource, trackType samvaad.TrackType, ooo, total uint32) {
	if total > 0 {
...
func RecordJitter(country string, direction Direction, trackSource samvaad.TrackSource, trackType samvaad.TrackType, jitter uint32) {
	if jitter > 0 {
		promJitter.WithLabelValues(string(direction), trackSource.String(), trackType.String(), country).Observe(float64(jitter))
	}
}

func RecordRTT(country string, direction Direction, trackSource samvaad.TrackSource, trackType samvaad.TrackType, rtt uint32) {
	if rtt > 0 {
