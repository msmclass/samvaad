...
func initDataPacketStats(nodeID string, nodeType samvaad.NodeType) {
...
func RecordDataPacketStream(h *samvaad.DataStream_Header, destCount int) {
	streamType := "unknown"
	switch h.ContentHeader.(type) {
	case *samvaad.DataStream_Header_TextHeader:
		streamType = "text"
	case *samvaad.DataStream_Header_ByteHeader:
		streamType = "bytes"
	}
