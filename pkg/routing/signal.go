...
type SignalClient interface {
	ActiveCount() int
	StartParticipantSignal(ctx context.Context, roomName samvaad.RoomName, pi ParticipantInit, nodeID samvaad.NodeID) (connectionID samvaad.ConnectionID, reqSink MessageSink, resSource MessageSource, err error)
}
...
func NewSignalClient(nodeID samvaad.NodeID, bus psrpc.MessageBus, config config.SignalRelayConfig) (SignalClient, error) {
...
func (r *signalClient) StartParticipantSignal(
	...
	connectionID = samvaad.ConnectionID(guid.New("CO_"))
...
func (e signalRequestMessageWriter) Write(seq uint64, close bool, msgs []proto.Message) *rpc.RelaySignalRequest {
	r := &rpc.RelaySignalRequest{
		Seq:      seq,
		Requests: make([]*samvaad.SignalRequest, 0, len(msgs)),
		Close:    close,
	}
	for _, m := range msgs {
		r.Requests = append(r.Requests, m.(*samvaad.SignalRequest))
	}
	return r
}
...
func (s *signalMessageSink[SendType, RecvType]) ConnectionID() samvaad.ConnectionID {
