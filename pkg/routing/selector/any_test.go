...
func createTestNode(id string, cpuLoad float32, numRooms int32, numClients int32, state samvaad.NodeState) *samvaad.Node {
	return &samvaad.Node{
		Id:    id,
		State: state,
...
func TestAnySelector_SelectNode_TwoChoice(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			algorithm: "twochoice",
			nodes: []*samvaad.Node{
				createTestNode("node1", 0.8, 5, 10, samvaad.NodeState_SERVING),
				createTestNode("node2", 0.3, 2, 5, samvaad.NodeState_SERVING),
				createTestNode("node3", 0.6, 3, 8, samvaad.NodeState_SERVING),
				createTestNode("node4", 0.9, 6, 12, samvaad.NodeState_SERVING),
			},
			wantErr:     "",
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*samvaad.Node{
				createTestNode("node1", 0.5, 8, 15, samvaad.NodeState_SERVING),
				createTestNode("node2", 0.4, 2, 5, samvaad.NodeState_SERVING),
				createTestNode("node3", 0.6, 12, 20, samvaad.NodeState_SERVING),
			},
			wantErr:     "",
			expected:    "",      // Not determinstic selection, so no specific expected node
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*samvaad.Node{
				createTestNode("node1", 0.5, 3, 25, samvaad.NodeState_SERVING),
				createTestNode("node2", 0.4, 2, 5, samvaad.NodeState_SERVING),
				createTestNode("node3", 0.6, 4, 30, samvaad.NodeState_SERVING),
			},
			wantErr:     "",
			expected:    "",      // Not determinstic selection, so no specific expected node
		...
		},
		{
			...
			nodes: []*samvaad.Node{
				{
					Id:    "node1",
					State: samvaad.NodeState_SHUTTING_DOWN,
					Stats: &samvaad.NodeStats{
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*samvaad.Node{
				createTestNode("node1", 0.5, 3, 10, samvaad.NodeState_SERVING),
			},
			wantErr:     "",
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*samvaad.Node{
				createTestNode("node1", 0.8, 5, 15, samvaad.NodeState_SERVING),
				createTestNode("node2", 0.3, 2, 5, samvaad.NodeState_SERVING),
			},
			wantErr:     "",
...
func TestAnySelector_SelectNode_TwoChoice_Probabilistic_Behavior(t *testing.T) {
	...
	nodes := []*samvaad.Node{
		createTestNode("node1", 0.95, 10, 20, samvaad.NodeState_SERVING), // Very high load
		createTestNode("node2", 0.1, 1, 2, samvaad.NodeState_SERVING),    // Low load
		createTestNode("node3", 0.5, 9, 18, samvaad.NodeState_SERVING),   // Medium load
		createTestNode("node4", 0.85, 8, 16, samvaad.NodeState_SERVING),  // High load
	}
