...
func IsAvailable(node *samvaad.Node) bool {
	if node.Stats == nil {
...
func GetAvailableNodes(nodes []*samvaad.Node) []*samvaad.Node {
	return funk.Filter(nodes, func(node *samvaad.Node) bool {
		return IsAvailable(node) && node.State == samvaad.NodeState_SERVING
	}).([]*samvaad.Node)
}

func GetNodeSysload(node *samvaad.Node) float32 {
	stats := node.Stats
...
func LimitsReached(limitConfig config.LimitConfig, nodeStats *samvaad.NodeStats) bool {
	if nodeStats == nil {
		return false
	}
...
func SelectSortedNode(nodes []*samvaad.Node, sortBy string, algorithm string) (*samvaad.Node, error) {
	if sortBy == "" {
...
func selectTwoChoiceSortedNode(nodes []*samvaad.Node, sortBy string) (*samvaad.Node, error) {
	if len(nodes) <= 2 {
		return selectLowestSortedNode(nodes, sortBy)
	}
	...
	selectedNode, err := selectLowestSortedNode([]*samvaad.Node{node1, node2}, sortBy)
	if err != nil {
		return nil, err
	}
...
func selectLowestSortedNode(nodes []*samvaad.Node, sortBy string) (*samvaad.Node, error) {
	...
	case "sysload":
		slices.SortFunc(nodes, func(a, b *samvaad.Node) int {
	...
	case "bytespersec":
		slices.SortFunc(nodes, func(a, b *samvaad.Node) int {
			ratea := &samvaad.NodeStatsRate{}
			if len(a.Stats.Rates) > 0 {
				ratea = a.Stats.Rates[0]
			}

			rateb := &samvaad.NodeStatsRate{}
...
func selectTwoRandomNodes(nodes []*samvaad.Node) (*samvaad.Node, *samvaad.Node, error) {
	if len(nodes) < 2 {
