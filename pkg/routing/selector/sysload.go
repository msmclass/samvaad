...
func (s *SystemLoadSelector) filterNodes(nodes []*samvaad.Node) ([]*samvaad.Node, error) {
	nodes = GetAvailableNodes(nodes)
	if len(nodes) == 0 {
		return nil, ErrNoAvailableNodes
	}

	nodesLowLoad := make([]*samvaad.Node, 0)
...
func (s *SystemLoadSelector) SelectNode(nodes []*samvaad.Node) (*samvaad.Node, error) {
