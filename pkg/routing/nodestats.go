...
type NodeStats struct {
	...
	lock                 sync.Mutex
	statsHistory         []*samvaad.NodeStats
...
func (n *NodeStats) UpdateConfig(conf *config.NodeStatsConfig) {
	...
	n.statsHistory = make([]*samvaad.NodeStats, (maxInterval+conf.StatsUpdateInterval-1)/conf.StatsUpdateInterval)
	n.statsHistoryWritePtr = 0
}

func (n *NodeStats) UpdateAndGetNodeStats() (*samvaad.NodeStats, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
