...
func (r *RedisRouter) GetNodeForRoom(_ context.Context, roomName samvaad.RoomName) (*samvaad.Node, error) {
	...
	return r.GetNode(samvaad.NodeID(nodeID))
}

func (r *RedisRouter) SetNodeForRoom(_ context.Context, roomName samvaad.RoomName, nodeID samvaad.NodeID) error {
	return r.rc.HSet(r.ctx, NodeRoomKey, string(roomName), string(nodeID)).Err()
}

func (r *RedisRouter) ClearRoomState(_ context.Context, roomName samvaad.RoomName) error {
...
func (r *RedisRouter) GetNode(nodeID samvaad.NodeID) (*samvaad.Node, error) {
...
func (r *RedisRouter) CreateRoom(ctx context.Context, req *samvaad.CreateRoomRequest) (res *samvaad.Room, err error) {
	rtcNode, err := r.GetNodeForRoom(ctx, samvaad.RoomName(req.Name))
	if err != nil {
		return
	}

	return r.CreateRoomWithNodeID(ctx, req, samvaad.NodeID(rtcNode.Id))
}
...
func (r *RedisRouter) StartParticipantSignal(ctx context.Context, roomName samvaad.RoomName, pi ParticipantInit) (res StartParticipantSignalResults, err error) {
	rtcNode, err := r.GetNodeForRoom(ctx, roomName)
	if err != nil {
		return
	}

	return r.StartParticipantSignalWithNodeID(ctx, roomName, pi, samvaad.NodeID(rtcNode.Id))
}
