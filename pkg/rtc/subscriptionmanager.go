...
const (
	trackIDForReconcileSubscriptions = samvaad.TrackID("subscriptions_reconcile")
)
...
type SubscriptionManagerParams struct {
	...
	OnSubscriptionError func(trackID samvaad.TrackID, fatal bool, err error)
...
type SubscriptionManager struct {
	...
	lock                sync.RWMutex
	subscriptions       map[samvaad.TrackID]*mediaTrackSubscription
	pendingUnsubscribes atomic.Int32

	subscribedVideoCount, subscribedAudioCount atomic.Int32

	subscribedTo         map[samvaad.ParticipantID]map[samvaad.TrackID]struct{}
	reconcileCh          chan samvaad.TrackID
	reconcileDataTrackCh chan samvaad.TrackID
	closeCh              chan struct{}
	doneCh               chan struct{}

	onSubscribeStatusChanged func(publisherID samvaad.ParticipantID, subscribed bool)

	dataTrackSubscriptions map[samvaad.TrackID]*dataTrackSubscription
}

func NewSubscriptionManager(params SubscriptionManagerParams) *SubscriptionManager {
	m := &SubscriptionManager{
		params:                 params,
		subscriptions:          make(map[samvaad.TrackID]*mediaTrackSubscription),
		subscribedTo:           make(map[samvaad.ParticipantID]map[samvaad.TrackID]struct{}),
		reconcileCh:            make(chan samvaad.TrackID, 50),
		reconcileDataTrackCh:   make(chan samvaad.TrackID, 5),
		closeCh:                make(chan struct{}),
		doneCh:                 make(chan struct{}),
		dataTrackSubscriptions: make(map[samvaad.TrackID]*dataTrackSubscription),
	}
...
func (m *SubscriptionManager) SubscribeToTrack(trackID samvaad.TrackID, isSync bool) {
	if m.isClosed() {
		return
	}
...
func (m *SubscriptionManager) UnsubscribeFromTrack(trackID samvaad.TrackID) {
...
func (m *SubscriptionManager) SubscribeToDataTrack(trackID samvaad.TrackID) {
	if m.isClosed() {
		return
	}
...
func (m *SubscriptionManager) UnsubscribeFromDataTrack(trackID samvaad.TrackID) {
...
func (m *SubscriptionManager) IsTrackNameSubscribed(publisherIdentity samvaad.ParticipantIdentity, trackName string) bool {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) StopAndGetSubscribedTracksForwarderState() map[samvaad.TrackID]*samvaad.RTPForwarderState {
	m.lock.RLock()
	defer m.lock.RUnlock()

	states := make(map[samvaad.TrackID]*samvaad.RTPForwarderState, len(m.subscriptions))
...
func (m *SubscriptionManager) GetSubscribedParticipants() []samvaad.ParticipantID {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) IsSubscribedTo(participantID samvaad.ParticipantID) bool {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) UpdateSubscribedTrackSettings(trackID samvaad.TrackID, settings *samvaad.UpdateTrackSettings) {
	m.lock.Lock()
...
func (m *SubscriptionManager) UpdateDataTrackSubscriptionOptions(trackID samvaad.TrackID, subscriptionOptions *samvaad.DataTrackSubscriptionOptions) {
	m.lock.Lock()
...
func (m *SubscriptionManager) OnSubscribeStatusChanged(fn func(publisherID samvaad.ParticipantID, subscribed bool)) {
	m.lock.Lock()
...
func (m *SubscriptionManager) setDesired(trackID samvaad.TrackID, desired bool) (*mediaTrackSubscription, bool) {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) setDataTrackDesired(trackID samvaad.TrackID, desired bool) (*dataTrackSubscription, bool) {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) queueReconcile(trackID samvaad.TrackID) {
	select {
...
func (m *SubscriptionManager) queueReconcileDataTrack(trackID samvaad.TrackID) {
	select {
...
func (m *SubscriptionManager) hasCapacityForSubscription(kind samvaad.TrackType) bool {
	switch kind {
...
func (m *SubscriptionManager) subscribeSynchronous(trackID samvaad.TrackID) error {
...
func (m *SubscriptionManager) unsubscribeSynchronous(trackID samvaad.TrackID) error {
	m.lock.Lock()
...
func (m *SubscriptionManager) handleSourceTrackRemoved(trackID samvaad.TrackID) {
	m.lock.Lock()
	sub := m.subscriptions[trackID]
	m.lock.Unlock()
...
func (m *SubscriptionManager) handleSubscribedTrackClose(s *mediaTrackSubscription, isExpectedToResume bool) {
	...
	if wasBound {
		m.params.TelemetryListener.OnTrackUnsubscribed(
			s.subscriberID,
			&samvaad.TrackInfo{Sid: string(s.trackID), Type: subTrack.MediaTrack().Kind()},
			!isExpectedToResume,
		)
...
func (m *SubscriptionManager) notifyDataTrackSubscriberHandles() {
	m.lock.Lock()
	handles := make(map[uint32]*samvaad.DataTrackSubscriberHandles_PublishedDataTrack, len(m.dataTrackSubscriptions))
	for _, sub := range m.dataTrackSubscriptions {
		if !sub.isDesired() {
			continue
		}
		...
		handles[uint32(dataDownTrack.Handle())] = &samvaad.DataTrackSubscriberHandles_PublishedDataTrack{
...
func (m *SubscriptionManager) handleSourceDataTrackRemoved(trackID samvaad.TrackID) {
	m.lock.Lock()
...
func (m *SubscriptionManager) markSubscribedTo(publisherID samvaad.ParticipantID, trackID samvaad.TrackID) {
	...
	if pTracks == nil {
		pTracks = make(map[samvaad.TrackID]struct{})
...
func (m *SubscriptionManager) unmarkSubscribedTo(publisherID samvaad.ParticipantID, trackID samvaad.TrackID) {
...
func (s *trackSubscription) setPublisher(publisherIdentity samvaad.ParticipantIdentity, publisherID samvaad.ParticipantID) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *trackSubscription) getPublisherID() samvaad.ParticipantID {
	s.lock.RLock()
...
type mediaTrackSubscription struct {
	...
	settings                 *samvaad.UpdateTrackSettings
	hasPermissionInitialized bool
	hasPermission            bool
	subscribedTrack          types.SubscribedTrack
	eventSent                atomic.Bool
	bound                    bool
	kind                     atomic.Pointer[samvaad.TrackType]

	succRecordCounter atomic.Int32
}

func newMediaTrackSubscription(subscriberID samvaad.ParticipantID, trackID samvaad.TrackID, l logger.Logger) *mediaTrackSubscription {
...
func (s *mediaTrackSubscription) trySetKind(kind samvaad.TrackType) {
	s.kind.CompareAndSwap(nil, &kind)
}

func (s *mediaTrackSubscription) getKind() (samvaad.TrackType, bool) {
	kind := s.kind.Load()
	if kind == nil {
		return samvaad.TrackType_AUDIO, false
	}
	return *kind, true
}

func (s *mediaTrackSubscription) setSettings(settings *samvaad.UpdateTrackSettings) {
	s.lock.Lock()
	s.settings = settings
...
type dataTrackSubscription struct {
	...
	subscriptionOptions *samvaad.DataTrackSubscriptionOptions

	dataDownTrack types.DataDownTrack
}

func newDataTrackSubscription(subscriberID samvaad.ParticipantID, trackID samvaad.TrackID, l logger.Logger) *dataTrackSubscription {
...
func (s *dataTrackSubscription) setSubscriptionOptions(subscriptionOptions *samvaad.DataTrackSubscriptionOptions) {
	s.lock.Lock()
