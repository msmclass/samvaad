...
type ParticipantSupervisor struct {
	...
	publications         map[samvaad.TrackID]*trackMonitor

	isStopped atomic.Bool

	onPublicationError func(trackID samvaad.TrackID)
}

func NewParticipantSupervisor(params ParticipantSupervisorParams) *ParticipantSupervisor {
	p := &ParticipantSupervisor{
		params:       params,
		publications: make(map[samvaad.TrackID]*trackMonitor),
	}
...
func (p *ParticipantSupervisor) OnPublicationError(f func(trackID samvaad.TrackID)) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *ParticipantSupervisor) getOnPublicationError() func(trackID samvaad.TrackID) {
	p.lock.RLock()
...
func (p *ParticipantSupervisor) AddPublication(trackID samvaad.TrackID) {
	p.lock.Lock()
...
func (p *ParticipantSupervisor) SetPublicationMute(trackID samvaad.TrackID, isMuted bool) {
	p.lock.Lock()
	pm, ok := p.publications[trackID]
	if ok {
...
func (p *ParticipantSupervisor) SetPublishedTrack(trackID samvaad.TrackID, pubTrack types.LocalMediaTrack) {
	p.lock.RLock()
	pm, ok := p.publications[trackID]
	if ok {
...
func (p *ParticipantSupervisor) ClearPublishedTrack(trackID samvaad.TrackID, pubTrack types.LocalMediaTrack) {
	p.lock.RLock()
	pm, ok := p.publications[trackID]
	if ok {
...
func (p *ParticipantSupervisor) checkPublications() {
	var erroredPublications []samvaad.TrackID
	var removablePublications []samvaad.TrackID
	p.lock.RLock()
