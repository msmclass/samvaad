...
func (p *ParticipantImpl) populateSdpCid(parsedOffer *sdp.SessionDescription) ([]*sdp.MediaDescription, []*sdp.MediaDescription) {
	processUnmatch := func(unmatches []*sdp.MediaDescription, trackType samvaad.TrackType) {
	...
	processUnmatch(unmatchAudios, samvaad.TrackType_AUDIO)
	processUnmatch(unmatchVideos, samvaad.TrackType_VIDEO)
...
func (p *ParticipantImpl) setCodecPreferencesOpusRedForPublisher(
	...
	for _, unmatchAudio := range unmatchAudios {
		...
		p.pendingTracksLock.RLock()
		_, ti, _, _, _ := p.getPendingTrack(streamID, samvaad.TrackType_AUDIO, false)
		p.pendingTracksLock.RUnlock()
		if ti == nil {
			continue
		}
		...
		case "audio":
			...
			for i, attr := range m.Attributes {
				if strings.HasPrefix(attr.String(), fmt.Sprintf("fmtp:%d", opusPT)) {
					if !slices.Contains(ti.AudioFeatures, samvaad.AudioTrackFeature_TF_NO_DTX) {
						attr.Value += ";usedtx=1"
					} else {
						attr.Value = strings.ReplaceAll(attr.Value, ";usedtx=1", "")
					}
					if slices.Contains(ti.AudioFeatures, samvaad.AudioTrackFeature_TF_STEREO) {
