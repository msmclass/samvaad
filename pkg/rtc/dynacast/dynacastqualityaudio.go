...
type dynacastQualityAudio struct {
	...
	lock                  sync.RWMutex
	initialized           bool
	subscriberEnables     map[samvaad.ParticipantID]bool
	subscriberNodeEnables map[samvaad.NodeID]bool
	enabled               bool
...
func newDynacastQualityAudio(params dynacastQualityAudioParams) dynacastQuality {
	return &dynacastQualityAudio{
		params:                params,
		subscriberEnables:     make(map[samvaad.ParticipantID]bool),
		subscriberNodeEnables: make(map[samvaad.NodeID]bool),
	}
}
...
func (d *dynacastQualityAudio) NotifySubscription(subscriberID samvaad.ParticipantID, enabled bool) {
...
func (d *dynacastQualityAudio) NotifySubscriptionNode(nodeID samvaad.NodeID, enabled bool) {
	d.params.Logger.Debugw(
...
func (d *dynacastQualityAudio) ClearSubscriberNodes() {
	d.lock.Lock()
	d.subscriberNodeEnables = make(map[samvaad.NodeID]bool)
	d.lock.Unlock()
