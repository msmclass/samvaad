...
func (d *dynacastManagerAudio) enqueueSubscribedChange() {
	...
	subscribedCodecs := make([]*samvaad.SubscribedAudioCodec, 0, len(d.committedSubscribedCodecs))
	for mime, enabled := range d.committedSubscribedCodecs {
		subscribedCodecs = append(subscribedCodecs, &samvaad.SubscribedAudioCodec{
			Codec:   mime.String(),
			Enabled: enabled,
		})
	}
