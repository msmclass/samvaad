...
type UpTrackManager struct {
	...
	publishedTracks        map[samvaad.TrackID]types.MediaTrack
	subscriptionPermission *samvaad.SubscriptionPermission
	// subscriber permission for published tracks
	subscriberPermissions map[samvaad.ParticipantIdentity]*samvaad.TrackPermission // subscriberIdentity => *samvaad.TrackPermission
...
func NewUpTrackManager(params UpTrackManagerParams) *UpTrackManager {
	return &UpTrackManager{
		params:          params,
		publishedTracks: make(map[samvaad.TrackID]types.MediaTrack),
	}
}
...
func (u *UpTrackManager) Close(isExpectedToResume bool) {
	...
	u.publishedTracks = make(map[samvaad.TrackID]types.MediaTrack)
	u.lock.Unlock()
...
func (u *UpTrackManager) ToProto() []*samvaad.TrackInfo {
	u.lock.RLock()
	defer u.lock.RUnlock()

	var trackInfos []*samvaad.TrackInfo
...
func (u *UpTrackManager) SetPublishedTrackMuted(trackID samvaad.TrackID, muted bool) (types.MediaTrack, bool) {
	changed := false
...
func (u *UpTrackManager) GetPublishedTrack(trackID samvaad.TrackID) types.MediaTrack {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) UpdateSubscriptionPermission(
	...
	resolverBySid func(participantID samvaad.ParticipantID) types.LocalParticipant,
) error {
	u.lock.Lock()
	...
	if err := u.parseSubscriptionPermissionsLocked(subscriptionPermission, func(pID samvaad.ParticipantID) types.LocalParticipant {
		u.lock.Unlock()
...
func (u *UpTrackManager) SubscriptionPermission() (*samvaad.SubscriptionPermission, utils.TimedVersion) {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) HasPermission(trackID samvaad.TrackID, subIdentity samvaad.ParticipantIdentity) bool {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) UpdatePublishedAudioTrack(update *samvaad.UpdateLocalAudioTrack) types.MediaTrack {
	track := u.GetPublishedTrack(samvaad.TrackID(update.TrackSid))
	if track != nil {
...
func (u *UpTrackManager) UpdatePublishedVideoTrack(update *samvaad.UpdateLocalVideoTrack) types.MediaTrack {
	track := u.GetPublishedTrack(samvaad.TrackID(update.TrackSid))
	if track != nil {
...
func (u *UpTrackManager) getPublishedTrackLocked(trackID samvaad.TrackID) types.MediaTrack {
	return u.publishedTracks[trackID]
}

func (u *UpTrackManager) parseSubscriptionPermissionsLocked(
	subscriptionPermission *samvaad.SubscriptionPermission,
	resolver func(participantID samvaad.ParticipantID) types.LocalParticipant,
) error {
	...
	subscriberPermissions := make(map[samvaad.ParticipantIdentity]*samvaad.TrackPermission)
	for _, trackPerms := range subscriptionPermission.TrackPermissions {
		subscriberIdentity := samvaad.ParticipantIdentity(trackPerms.ParticipantIdentity)
		if subscriberIdentity == "" {
			if trackPerms.ParticipantSid == "" {
				return ErrSubscriptionPermissionNeedsId
			}

			sub := resolver(samvaad.ParticipantID(trackPerms.ParticipantSid))
			if sub == nil {
...
func (u *UpTrackManager) hasPermissionLocked(trackID samvaad.TrackID, subscriberIdentity samvaad.ParticipantIdentity) bool {
	...
	for _, sid := range perms.TrackSids {
		if samvaad.TrackID(sid) == trackID {
			return true
		}
	}
...
func (u *UpTrackManager) getAllowedSubscribersLocked(trackID samvaad.TrackID) []samvaad.ParticipantIdentity {
	if u.subscriberPermissions == nil {
		return nil
	}

	allowed := make([]samvaad.ParticipantIdentity, 0)
...
func (u *UpTrackManager) DebugInfo() map[string]any {
	info := map[string]any{}
	publishedTrackInfo := make(map[samvaad.TrackID]any)
...
func (u *UpTrackManager) GetAudioLevel() (level float64, active bool) {
	...
	for _, pt := range u.GetPublishedTracks() {
		if pt.Source() == samvaad.TrackSource_MICROPHONE {
