...
func TestSubscribe(t *testing.T) {
	t.Run("happy path subscribe", func(t *testing.T) {
		...
		sm.params.OnSubscriptionError = func(trackID samvaad.TrackID, fatal bool, err error) {
			failed.Store(true)
		}
		numParticipantSubscribed := atomic.Int32{}
		numParticipantUnsubscribed := atomic.Int32{}
		sm.OnSubscribeStatusChanged(func(pubID samvaad.ParticipantID, subscribed bool) {
			if subscribed {
...
func TestUnsubscribe(t *testing.T) {
	...
	res.Track.(*typesfakes.FakeMediaTrack).RemoveSubscriberCalls(func(pID samvaad.ParticipantID, isExpectedToResume bool) {
...
func TestSubscribeStatusChanged(t *testing.T) {
	...
	st1.MediaTrack().(*typesfakes.FakeMediaTrack).RemoveSubscriberCalls(func(pID samvaad.ParticipantID, isExpectedToResume bool) {
		setTestSubscribedTrackClosed(t, st1, isExpectedToResume)
	})
	st2.MediaTrack().(*typesfakes.FakeMediaTrack).RemoveSubscriberCalls(func(pID samvaad.ParticipantID, isExpectedToResume bool) {
...
func newTestSubscriptionManagerWithParams(params testSubscriptionParams) *SubscriptionManager {
	...
	p.KindReturns(samvaad.ParticipantInfo_STANDARD)
	return NewSubscriptionManager(SubscriptionManagerParams{
		...
		TrackResolver: func(sub types.LocalParticipant, trackID samvaad.TrackID) types.MediaResolverResult {
...
type testResolver struct {
	...
	hasPermission bool
	hasTrack      bool
	pubIdentity   samvaad.ParticipantIdentity
	pubID         samvaad.ParticipantID

	paused bool
}

func newTestResolver(hasPermission bool, hasTrack bool, pubIdentity samvaad.ParticipantIdentity, pubID samvaad.ParticipantID) *testResolver {
	return &testResolver{
...
func (t *testResolver) Resolve(_subscriber types.LocalParticipant, trackID samvaad.TrackID) types.MediaResolverResult {
	t.lock.Lock()
	defer t.lock.Unlock()
