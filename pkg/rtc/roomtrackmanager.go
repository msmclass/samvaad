...
type RoomTrackManager struct {
	...
	tracks          map[samvaad.TrackID][]*TrackInfo
	dataTracks      map[samvaad.TrackID][]*DataTrackInfo
}
...
func NewRoomTrackManager(logger logger.Logger) *RoomTrackManager {
	return &RoomTrackManager{
		logger:          logger,
		tracks:          make(map[samvaad.TrackID][]*TrackInfo),
		dataTracks:      make(map[samvaad.TrackID][]*DataTrackInfo),
		changedNotifier: utils.NewChangeNotifierManager(),
		removedNotifier: utils.NewChangeNotifierManager(),
	}
}

func (r *RoomTrackManager) AddTrack(track types.MediaTrack, publisherIdentity samvaad.ParticipantIdentity, publisherID samvaad.ParticipantID) {
	trackID := track.ID()
	r.lock.Lock()
...
func (r *RoomTrackManager) GetTrackInfo(trackID samvaad.TrackID) *TrackInfo {
	r.lock.RLock()
...
func (r *RoomTrackManager) NotifyTrackChanged(trackID samvaad.TrackID) {
...
func (r *RoomTrackManager) GetOrCreateTrackChangeNotifier(trackID samvaad.TrackID) *utils.ChangeNotifier {
	return r.changedNotifier.GetOrCreateNotifier(string(trackID))
}

func (r *RoomTrackManager) GetOrCreateTrackRemoveNotifier(trackID samvaad.TrackID) *utils.ChangeNotifier {
	return r.removedNotifier.GetOrCreateNotifier(string(trackID))
}

func (r *RoomTrackManager) AddDataTrack(dataTrack types.DataTrack, publisherIdentity samvaad.ParticipantIdentity, publisherID samvaad.ParticipantID) {
...
func (r *RoomTrackManager) GetDataTrackInfo(trackID samvaad.TrackID) *DataTrackInfo {
	r.lock.RLock()
	defer r.lock.RUnlock()
