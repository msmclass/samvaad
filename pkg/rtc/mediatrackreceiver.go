...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/transport/pkg/codec"
	...
	"github.com/msmclass/samvaad/pkg/sfu/rtpstats"
...
type MediaTrackReceiver struct {
	...
	trackInfo          atomic.Pointer[samvaad.TrackInfo]
...
func NewMediaTrackReceiver(params MediaTrackReceiverParams, ti *samvaad.TrackInfo) *MediaTrackReceiver {
...
func (t *MediaTrackReceiver) Restart() {
	for _, receiver := range t.loadReceivers() {
		hq := buffer.GetSpatialLayerForVideoQuality(receiver.Mime(), samvaad.VideoQuality_HIGH, t.TrackInfo())
...
func (t *MediaTrackReceiver) ID() samvaad.TrackID {
	return samvaad.TrackID(t.TrackInfo().Sid)
}

func (t *MediaTrackReceiver) Kind() samvaad.TrackType {
	return t.TrackInfo().Type
}

func (t *MediaTrackReceiver) Source() samvaad.TrackSource {
...
func (t *MediaTrackReceiver) PublisherID() samvaad.ParticipantID {
	return t.params.ParticipantID()
}

func (t *MediaTrackReceiver) PublisherIdentity() samvaad.ParticipantIdentity {
...
func (t *MediaTrackReceiver) IsEncrypted() bool {
	return t.TrackInfo().Encryption != samvaad.Encryption_NONE
}
...
func (t *MediaTrackReceiver) RemoveSubscriber(subscriberID samvaad.ParticipantID, isExpectedToResume bool) {
...
func (t *MediaTrackReceiver) RevokeDisallowedSubscribers(allowedSubscriberIdentities []samvaad.ParticipantIdentity) []samvaad.ParticipantIdentity {
	var revokedSubscriberIdentities []samvaad.ParticipantIdentity
...
func (t *MediaTrackReceiver) SetLayerSsrcsForRid(mimeType mime.MimeType, rid string, ssrc uint32, repairSSRC uint32) {
	...
	for i, ci := range trackInfo.Codecs {
		...
		var matchingLayer *samvaad.VideoLayer
		ssrcFound := false
...
func (t *MediaTrackReceiver) UpdateCodecInfo(codecs []*samvaad.SimulcastCodec) {
	t.lock.Lock()
	trackInfo := t.TrackInfoClone()
	for _, c := range codecs {
		for _, origin := range trackInfo.Codecs {
			if mime.GetMimeTypeCodec(origin.MimeType) == mime.NormalizeMimeTypeCodec(c.Codec) {
				...
				if len(c.Layers) != 0 {
					clonedLayers := make([]*samvaad.VideoLayer, 0, len(c.Layers))
...
func (t *MediaTrackReceiver) UpdateTrackInfo(ti *samvaad.TrackInfo) {
...
func (t *MediaTrackReceiver) UpdateAudioTrack(update *samvaad.UpdateLocalAudioTrack) {
	if t.Kind() != samvaad.TrackType_AUDIO {
		return
	}
...
func (t *MediaTrackReceiver) UpdateVideoTrack(update *samvaad.UpdateLocalVideoTrack) {
	if t.Kind() != samvaad.TrackType_VIDEO {
		return
	}
...
func (t *MediaTrackReceiver) TrackInfo() *samvaad.TrackInfo {
	return t.trackInfo.Load()
}

func (t *MediaTrackReceiver) TrackInfoClone() *samvaad.TrackInfo {
...
func (t *MediaTrackReceiver) NotifyMaxLayerChange(mimeType mime.MimeType, maxLayer int32) {
	...
	ti := &samvaad.TrackInfo{
		Sid:    trackInfo.Sid,
		Type:   trackInfo.Type,
		Layers: []*samvaad.VideoLayer{{Quality: quality}},
	}
...
func (t *MediaTrackReceiver) GetQualityForDimension(mimeType mime.MimeType, width, height uint32) samvaad.VideoQuality {
	...
	for i, s := range layerSizes {
		quality = samvaad.VideoQuality(i)
...
func (t *MediaTrackReceiver) GetTrackStats() *samvaad.RTPStats {
	receivers := t.loadReceivers()
	stats := make([]*samvaad.RTPStats, 0, len(receivers))
	for _, receiver := range receivers {
