...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/transport/pkg/twcc"
	...
	"github.com/msmclass/samvaad/pkg/samvaad/observability"
	"github.com/msmclass/samvaad/pkg/samvaad/observability/roomobs"
	protosdp "github.com/msmclass/samvaad/pkg/samvaad/sdp"
	...
	"github.com/msmclass/samvaad/pkg/samvaad/utils/pointer"
	"github.com/msmclass/samvaad/pkg/samvaad/psrpc"

	"github.com/msmclass/samvaad/pkg/config"
	"github.com/msmclass/samvaad/pkg/metric"
	"github.com/msmclass/samvaad/pkg/routing"
	"github.com/msmclass/samvaad/pkg/rtc/signalling"
	"github.com/msmclass/samvaad/pkg/rtc/supervisor"
	"github.com/msmclass/samvaad/pkg/rtc/transport"
	"github.com/msmclass/samvaad/pkg/rtc/types"
	"github.com/msmclass/samvaad/pkg/sfu"
	"github.com/msmclass/samvaad/pkg/sfu/buffer"
	"github.com/msmclass/samvaad/pkg/sfu/connectionquality"
	"github.com/msmclass/samvaad/pkg/sfu/interceptor"
	"github.com/msmclass/samvaad/pkg/sfu/pacer"
	"github.com/msmclass/samvaad/pkg/sfu/streamallocator"
	"github.com/msmclass/samvaad/pkg/telemetry"
	"github.com/msmclass/samvaad/pkg/telemetry/prometheus"
	sutils "github.com/msmclass/samvaad/pkg/utils"
)
...
type reliableDataInfo struct {
	...
	joiningMessageFirstSeqs       map[samvaad.ParticipantID]uint32
	joiningMessageLastWrittenSeqs map[samvaad.ParticipantID]uint32
...
type ParticipantParams struct {
	...
	PublishEnabledCodecs            []*samvaad.Codec
	SubscribeEnabledCodecs          []*samvaad.Codec
	...
	PlayoutDelay                    *samvaad.PlayoutDelay
	SyncStreams                     bool
...
type ParticipantImpl struct {
	...
	state        atomic.Value // samvaad.ParticipantInfo_State
	...
	pendingPublishingTracks map[samvaad.TrackID]*pendingTrackInfo
	pendingRemoteTracks     []*pendingRemoteTrack

	// supported codecs
	enabledPublishCodecs   []*samvaad.Codec
	enabledSubscribeCodecs []*samvaad.Codec
	...
	*SubscriptionManager
	...
	queuedUpdates []*samvaad.ParticipantInfo
	// cache of recently sent updates, to ensure ordering by version
	// guarded by updateLock
	updateCache *lru.Cache[samvaad.ParticipantID, participantUpdateInfo]
	...
	onICEConfigChanged func(participant types.LocalParticipant, iceConfig *samvaad.ICEConfig)

	cachedDownTracks map[samvaad.TrackID]*downTrackState
	forwarderState   map[samvaad.TrackID]*samvaad.RTPForwarderState

	supervisor *supervisor.ParticipantSupervisor

	connectionQuality samvaad.ConnectionQuality
...
func NewParticipant(params ParticipantParams) (*ParticipantImpl, error) {
	...
	p := &ParticipantImpl{
		...
		pendingPublishingTracks: make(map[samvaad.TrackID]*pendingTrackInfo),
		connectedAt:             time.Now().Truncate(time.Millisecond),
		rttUpdatedAt:            time.Now(),
		cachedDownTracks:        make(map[samvaad.TrackID]*downTrackState),
		connectionQuality:       samvaad.ConnectionQuality_EXCELLENT,
		pubLogger:               params.Logger.WithComponent(sutils.ComponentPub),
		subLogger:               params.Logger.WithComponent(sutils.ComponentSub),
		reliableDataInfo: reliableDataInfo{
			joiningMessageFirstSeqs:       make(map[samvaad.ParticipantID]uint32),
			joiningMessageLastWrittenSeqs: make(map[samvaad.ParticipantID]uint32),
		},
	...
	p.state.Store(samvaad.ParticipantInfo_JOINING)
	...
	if p.updateCache, err = lru.New[samvaad.ParticipantID, participantUpdateInfo](128); err != nil {
		return nil, err
	}
...
func (p *ParticipantImpl) ID() samvaad.ParticipantID {
	return p.id.Load().(samvaad.ParticipantID)
}

func (p *ParticipantImpl) Identity() samvaad.ParticipantIdentity {
	return p.params.Identity
}

func (p *ParticipantImpl) State() samvaad.ParticipantInfo_State {
	return p.state.Load().(samvaad.ParticipantInfo_State)
}

func (p *ParticipantImpl) Kind() samvaad.ParticipantInfo_Kind {
	return p.grants.Load().GetParticipantKind()
}

func (p *ParticipantImpl) KindDetails() []samvaad.ParticipantInfo_KindDetail {
	return p.grants.Load().GetKindDetails()
}

func (p *ParticipantImpl) IsRecorder() bool {
	grants := p.grants.Load()
	return grants.GetParticipantKind() == samvaad.ParticipantInfo_EGRESS || grants.Video.Recorder
}

func (p *ParticipantImpl) IsAgent() bool {
	grants := p.grants.Load()
	return grants.GetParticipantKind() == samvaad.ParticipantInfo_AGENT || grants.Video.Agent
}

func (p *ParticipantImpl) IsDependent() bool {
	grants := p.grants.Load()
	switch grants.GetParticipantKind() {
	case samvaad.ParticipantInfo_AGENT, samvaad.ParticipantInfo_EGRESS:
		return true
	default:
...
func (p *ParticipantImpl) IsReady() bool {
	...
	if p.params.Migration {
		return state != samvaad.ParticipantInfo_DISCONNECTED
	}

	// when not migrating, there is a JoinResponse, state transitions from JOINING -> JOINED -> ACTIVE -> DISCONNECTED
	return state == samvaad.ParticipantInfo_JOINED || state == samvaad.ParticipantInfo_ACTIVE
}

func (p *ParticipantImpl) IsDisconnected() bool {
	return p.State() == samvaad.ParticipantInfo_DISCONNECTED
}
...
func (p *ParticipantImpl) GetClientInfo() *samvaad.ClientInfo {
	p.lock.RLock()
	defer p.lock.RUnlock()
	return p.params.ClientInfo.ClientInfo
}

func (p *ParticipantImpl) GetClientConfiguration() *samvaad.ClientConfiguration {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *ParticipantImpl) UpdateMetadata(update *samvaad.UpdateParticipantMetadata, fromAdmin bool) error {
	...
	var err error
	requestResponse := &samvaad.RequestResponse{
		RequestId: update.RequestId,
	}
	sendRequestResponse := func() error {
		if !fromAdmin || (update.RequestId != 0 || err != nil) {
			requestResponse.Request = &samvaad.RequestResponse_UpdateMetadata{
	...
	if !fromAdmin && !p.ClaimGrants().Video.GetCanUpdateOwnMetadata() {
		requestResponse.Reason = samvaad.RequestResponse_NOT_ALLOWED
	...
	if err = p.checkMetadataLimits(update.Name, update.Metadata, update.Attributes); err != nil {
		...
		case signalling.ErrNameExceedsLimits:
			requestResponse.Reason = samvaad.RequestResponse_LIMIT_EXCEEDED
...
func (p *ParticipantImpl) SetPermission(permission *samvaad.ParticipantPermission) bool {
	if permission == nil {
		return false
	}
	p.lock.Lock()
...
func (p *ParticipantImpl) ToProtoWithVersion() (*samvaad.ParticipantInfo, utils.TimedVersion) {
	p.maybeIncVersion()
...
func (p *ParticipantImpl) ToProto() *samvaad.ParticipantInfo {
...
func (p *ParticipantImpl) synthesizeAddTrackRequests(parsedOffer *sdp.SessionDescription) error {
	for _, m := range parsedOffer.MediaDescriptions {
		...
		var (
			name        string
			trackSource samvaad.TrackSource
			trackType   samvaad.TrackType
		)
		if strings.EqualFold(m.MediaName.Media, "audio") {
			name = "synthesized-microphone"
			trackSource = samvaad.TrackSource_MICROPHONE
			trackType = samvaad.TrackType_AUDIO
		} else {
			name = "synthesized-camera"
			trackSource = samvaad.TrackSource_CAMERA
			trackType = samvaad.TrackType_VIDEO
		}
		...
		if strings.EqualFold(m.MediaName.Media, "video") {
			if ridsOk {
				...
				for i := range n {
					...
					req.Layers = append(req.Layers, &samvaad.VideoLayer{Quality: samvaad.VideoQuality(i)})
				}
			} else {
				// dummy layer to ensure at least one layer is available
				req.Layers = []*samvaad.VideoLayer{{}}
			}
		}
		p.AddTrack(req)
	}
...
func (p *ParticipantImpl) HandleICETrickle(trickleRequest *samvaad.TrickleRequest) {
	...
	if err != nil {
		...
		p.sendRequestResponse(&samvaad.RequestResponse{
			Reason:  samvaad.RequestResponse_UNCLASSIFIED_ERROR,
			Message: err.Error(),
			Request: &samvaad.RequestResponse_Trickle{
...
func (p *ParticipantImpl) HandleOffer(sd *samvaad.SessionDescription) error {
	offer, offerId, _ := protosignalling.FromProtoSessionDescription(sd)
	lgr := p.pubLogger.WithUnlikelyValues(
		"transport", samvaad.SignalTarget_PUBLISHER,
		"offer", offer,
		"offerId", offerId,
	)
...
func (p *ParticipantImpl) HandleAnswer(sd *samvaad.SessionDescription) {
	answer, answerId, _ := protosignalling.FromProtoSessionDescription(sd)
	p.subLogger.Debugw(
		"received answer",
		"transport", samvaad.SignalTarget_SUBSCRIBER,
		"answer", answer,
		"answerId", answerId,
	)
...
func (p *ParticipantImpl) AddTrack(req *samvaad.AddTrackRequest) {
	...
	if !p.CanPublishSource(req.Source) {
		...
		p.sendRequestResponse(&samvaad.RequestResponse{
			...
			Request: &samvaad.RequestResponse_AddTrack{
	...
	if req.Type != samvaad.TrackType_AUDIO && req.Type != samvaad.TrackType_VIDEO {
		p.pubLogger.Warnw("unsupported track type", nil, "trackID", req.Sid, "kind", req.Type)
		p.sendRequestResponse(&samvaad.RequestResponse{
			Reason: samvaad.RequestResponse_UNSUPPORTED_TYPE,
			Request: &samvaad.RequestResponse_AddTrack{
				AddTrack: utils.CloneProto(req),
			},
		})
...
func (p *ParticipantImpl) SetMigrateInfo(
	...
	for _, t := range mediaTracks {
		...
		if p.supervisor != nil {
			p.supervisor.AddPublication(samvaad.TrackID(ti.Sid))
			p.supervisor.SetPublicationMute(samvaad.TrackID(ti.Sid), ti.Muted)
		}

		p.pendingTracks[t.GetCid()] = &pendingTrackInfo{
			trackInfos: []*samvaad.TrackInfo{ti},
			migrated:   true,
			createdAt:  time.Now(),
		}
		p.pubLogger.Infow(
	...
	for _, state := range dataChannelReceiveState {
		p.reliableDataInfo.joiningMessageFirstSeqs[samvaad.ParticipantID(state.PublisherSid)] = state.LastSeq + 1
	}
...
func (p *ParticipantImpl) Close(sendLeave bool, reason types.ParticipantCloseReason, isExpectedToResume bool) error {
	...
	p.pendingPublishingTracks = make(map[samvaad.TrackID]*pendingTrackInfo)
	p.pendingTracksLock.Unlock()
	...
	p.updateState(samvaad.ParticipantInfo_DISCONNECTED)
...
func (p *ParticipantImpl) ICERestart(iceConfig *samvaad.ICEConfig) {
	if p.params.UseOneShotSignallingMode {
		return
	}
...
func (p *ParticipantImpl) OnICEConfigChanged(f func(participant types.LocalParticipant, iceConfig *samvaad.ICEConfig)) {
	p.lock.Lock()
	p.onICEConfigChanged = f
	p.lock.Unlock()
}

func (p *ParticipantImpl) GetConnectionQuality() *samvaad.ConnectionQualityInfo {
	minQuality := samvaad.ConnectionQuality_EXCELLENT
	...
	if minQuality == samvaad.ConnectionQuality_LOST && !p.ProtocolVersion().SupportsConnectionQualityLost() {
		minQuality = samvaad.ConnectionQuality_POOR
	}
	...
	return &samvaad.ConnectionQualityInfo{
...
func (p *ParticipantImpl) CanPublishSource(source samvaad.TrackSource) bool {
...
func (p *ParticipantImpl) Verify() bool {
	state := p.State()
	isActive := state != samvaad.ParticipantInfo_JOINING && state != samvaad.ParticipantInfo_JOINED
	if p.params.UseOneShotSignallingMode {
...
func (p *ParticipantImpl) VerifySubscribeParticipantInfo(pID samvaad.ParticipantID, version uint32) {
	if !p.IsReady() {
	...
	if info := p.helper().GetParticipantInfo(pID); info != nil {
		_ = p.SendParticipantUpdate([]*samvaad.ParticipantInfo{info})
	}
}
...
func (h AnyTransportHandler) OnICECandidate(c *webrtc.ICECandidate, target samvaad.SignalTarget) error {
...
func (h PublisherTransportHandler) OnDataMessage(kind samvaad.DataPacket_Kind, data []byte) {
...
func (p *ParticipantImpl) setupTransportManager() error {
	...
	tm.OnICEConfigChanged(func(iceConfig *samvaad.ICEConfig) {
		p.lock.Lock()
		onICEConfigChanged := p.onICEConfigChanged

		if p.params.ClientConf == nil {
			p.params.ClientConf = &samvaad.ClientConfiguration{}
		}
		if iceConfig.PreferenceSubscriber == samvaad.ICECandidateType_ICT_TLS {
			p.params.ClientConf.ForceRelay = samvaad.ClientConfigSetting_ENABLED
		} else {
			// UNSET indicates that clients could override RTCConfiguration to forceRelay
			p.params.ClientConf.ForceRelay = samvaad.ClientConfigSetting_UNSET
		}
		p.lock.Unlock()
...
func (p *ParticipantImpl) setupSubscriptionManager() {
	p.SubscriptionManager = NewSubscriptionManager(SubscriptionManagerParams{
		...
		TrackResolver: func(lp types.LocalParticipant, ti samvaad.TrackID) types.MediaResolverResult {
			return p.helper().ResolveMediaTrack(lp, ti)
		},
		DataTrackResolver: func(lp types.LocalParticipant, ti samvaad.TrackID) types.DataResolverResult {
...
func (p *ParticipantImpl) MetricsCollectorBatchReady(mb *samvaad.MetricsBatch) {
	p.listener().OnMetrics(p, &samvaad.DataPacket{
		ParticipantIdentity: string(p.Identity()),
		Value: &samvaad.DataPacket_Metrics{
			Metrics: mb,
		},
	})
}

func (p *ParticipantImpl) MetricsReporterBatchReady(mb *samvaad.MetricsBatch) {
	dpData, err := proto.Marshal(&samvaad.DataPacket{
		ParticipantIdentity: string(p.Identity()),
		Value: &samvaad.DataPacket_Metrics{
			Metrics: mb,
		},
	...
	p.TransportManager.SendDataMessage(samvaad.DataPacket_RELIABLE, dpData)
}
...
func (p *ParticipantImpl) updateState(state samvaad.ParticipantInfo_State) {
	var oldState samvaad.ParticipantInfo_State
	for {
		oldState = p.state.Load().(samvaad.ParticipantInfo_State)
		if state <= oldState {
			p.params.Logger.Debugw("ignoring out of order participant state", "state", state.String())
			return
		}
		if state == samvaad.ParticipantInfo_ACTIVE {
	...
	if state == samvaad.ParticipantInfo_DISCONNECTED && oldState == samvaad.ParticipantInfo_ACTIVE {
...
func (p *ParticipantImpl) setIsPublisher(isPublisher bool) {
	...
	if p.State() == samvaad.ParticipantInfo_ACTIVE {
		p.listener().OnParticipantUpdate(p)
	}
}
...
func (p *ParticipantImpl) onReceivedDataMessage(kind samvaad.DataPacket_Kind, data []byte) {
	...
	if kind == samvaad.DataPacket_RELIABLE && dp.Sequence > 0 {
...
func (p *ParticipantImpl) handleReceivedDataMessage(kind samvaad.DataPacket_Kind, dp *samvaad.DataPacket) {
	if kind == samvaad.DataPacket_RELIABLE && dp.Sequence > 0 {
	...
	case *samvaad.DataPacket_SipDtmf:
		if payload.SipDtmf == nil {
			return
		}
	case *samvaad.DataPacket_Transcription:
	...
	case *samvaad.DataPacket_ChatMessage:
	...
	case *samvaad.DataPacket_Metrics:
	...
	case *samvaad.DataPacket_RpcRequest:
	...
	case *samvaad.DataPacket_RpcResponse:
	...
	case *samvaad.DataPacket_RpcAck:
	...
	case *samvaad.DataPacket_StreamHeader:
		...
		if p.IsAgent() && dp.ParticipantIdentity != "" && string(p.params.Identity) != dp.ParticipantIdentity {
			...
			case *samvaad.DataStream_Header_TextHeader:
	...
	case *samvaad.DataPacket_StreamChunk:
		if payload.StreamChunk == nil {
			return
		}
	case *samvaad.DataPacket_StreamTrailer:
		if payload.StreamTrailer == nil {
			return
		}
	case *samvaad.DataPacket_EncryptedPacket:
...
func (p *ParticipantImpl) onICECandidate(c *webrtc.ICECandidate, target samvaad.SignalTarget) error {
	if p.IsDisconnected() || p.IsClosed() {
		return nil
	}

	if target == samvaad.SignalTarget_SUBSCRIBER && p.MigrateState() == types.MigrateStateInit {
		return nil
	}
...
func (p *ParticipantImpl) onStreamStateChange(update *streamallocator.StreamStateUpdate) error {
	...
	streamStateUpdate := &samvaad.StreamStateUpdate{}
	for _, streamStateInfo := range update.StreamStates {
		state := samvaad.StreamState_ACTIVE
		if streamStateInfo.State == streamallocator.StreamStatePaused {
			state = samvaad.StreamState_PAUSED
		}
		streamStateUpdate.StreamStates = append(streamStateUpdate.StreamStates, &samvaad.StreamStateInfo{
...
func (p *ParticipantImpl) onSubscribedMaxQualityChange(
	...
	subscribedQualityUpdate := &samvaad.SubscribedQualityUpdate{
...
func (p *ParticipantImpl) onSubscribedAudioCodecChange(
	...
	subscribedAudioCodecUpdate := &samvaad.SubscribedAudioCodecUpdate{
		TrackSid:              string(trackID),
...
func (p *ParticipantImpl) addPendingTrackLocked(req *samvaad.AddTrackRequest) *samvaad.TrackInfo {
	if req.Sid != "" {
		track := p.GetPublishedTrack(samvaad.TrackID(req.Sid))
		if track == nil {
	...
	if (backupCodecPolicy != samvaad.BackupCodecPolicy_REGRESSION && req.Type == samvaad.TrackType_AUDIO) ||
		(backupCodecPolicy != samvaad.BackupCodecPolicy_SIMULCAST && p.params.DisableCodecRegression) {
		backupCodecPolicy = samvaad.BackupCodecPolicy_SIMULCAST
	}

	cloneLayers := func(layers []*samvaad.VideoLayer) []*samvaad.VideoLayer {
		if len(layers) == 0 {
			return nil
		}

		clonedLayers := make([]*samvaad.VideoLayer, 0, len(layers))
		for _, l := range layers {
			clonedLayers = append(clonedLayers, utils.CloneProto(l))
		}
		slices.SortFunc(clonedLayers, func(i, j *samvaad.VideoLayer) int {
	...
	if req.Stereo && !slices.Contains(ti.AudioFeatures, samvaad.AudioTrackFeature_TF_STEREO) {
		ti.AudioFeatures = append(ti.AudioFeatures, samvaad.AudioTrackFeature_TF_STEREO)
	}
	if req.DisableDtx && !slices.Contains(ti.AudioFeatures, samvaad.AudioTrackFeature_TF_NO_DTX) {
		ti.AudioFeatures = append(ti.AudioFeatures, samvaad.AudioTrackFeature_TF_NO_DTX)
	}
	if ti.Stream == "" {
	...
	if len(req.SimulcastCodecs) == 0 {
		...
		videoLayerMode := samvaad.VideoLayer_ONE_SPATIAL_LAYER_PER_STREAM
		if p.params.ClientInfo.isOBS() {
			videoLayerMode = samvaad.VideoLayer_ONE_SPATIAL_LAYER_PER_STREAM_INCOMPLETE_RTCP_SR
		}
		ti.Codecs = append(ti.Codecs, &samvaad.SimulcastCodecInfo{
			Cid:            req.Cid,
			Layers:         cloneLayers(req.Layers),
		...
		for _, codec := range req.SimulcastCodecs {
			...
			case samvaad.TrackType_VIDEO:
				...
				if videoLayerMode == samvaad.VideoLayer_MODE_UNUSED {
					if mime.IsMimeTypeStringSVCCapable(mimeType) {
						videoLayerMode = samvaad.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM
					} else {
						if p.params.ClientInfo.isOBS() {
							videoLayerMode = samvaad.VideoLayer_ONE_SPATIAL_LAYER_PER_STREAM_INCOMPLETE_RTCP_SR
						} else {
		...
		for idx, codec := range ti.Codecs {
			...
			if idx == 0 && codec.VideoLayerMode != samvaad.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM && len(codec.Layers) > 1 {
				ti.Simulcast = true
			}
		}
	}
	...
	if p.getPublishedTrackBySignalCid(req.Cid) != nil || p.getPublishedTrackBySdpCid(req.Cid) != nil || p.pendingTracks[req.Cid] != nil {
		if p.pendingTracks[req.Cid] == nil {
			...
			if ti.Type == samvaad.TrackType_VIDEO {
		...
		p.sendRequestResponse(&samvaad.RequestResponse{
			Reason: samvaad.RequestResponse_QUEUED,
			Request: &samvaad.RequestResponse_AddTrack{
				AddTrack: utils.CloneProto(req),
			},
		})
...
func (p *ParticipantImpl) GetPendingTrack(trackID samvaad.TrackID) *samvaad.TrackInfo {
	p.pendingTracksLock.RLock()
	defer p.pendingTracksLock.RUnlock()

	for _, t := range p.pendingTracks {
		if samvaad.TrackID(t.trackInfos[0].Sid) == trackID {
...
func (p *ParticipantImpl) SetTrackMuted(mute *samvaad.MuteTrackRequest, fromAdmin bool) *samvaad.TrackInfo {
	// when request is coming from admin, send message to current participant
	if fromAdmin {
		p.sendTrackMuted(samvaad.TrackID(mute.Sid), mute.Muted)
	}
...
func (p *ParticipantImpl) setTrackMuted(mute *samvaad.MuteTrackRequest, fromAdmin bool) *samvaad.TrackInfo {
	trackID := samvaad.TrackID(mute.Sid)
	p.dirty.Store(true)
	if p.supervisor != nil {
	...
	var trackInfo *samvaad.TrackInfo
	if track != nil {
	...
	for _, pti := range p.pendingTracks {
		for i, ti := range pti.trackInfos {
			if samvaad.TrackID(ti.Sid) == trackID {
	...
	if trackInfo == nil && !fromAdmin {
		p.sendRequestResponse(&samvaad.RequestResponse{
			Reason: samvaad.RequestResponse_NOT_FOUND,
			Request: &samvaad.RequestResponse_Mute{
...
func (p *ParticipantImpl) addMigratedTrack(cid string, ti *samvaad.TrackInfo) *MediaTrack {
...
func (p *ParticipantImpl) addMediaTrack(signalCid string, ti *samvaad.TrackInfo) *MediaTrack {
	mt := NewMediaTrack(MediaTrackParams{
		...
		TelemetryListener:      p.params.TelemetryListener,
		Logger:                 LoggerWithTrack(p.pubLogger, samvaad.TrackID(ti.Sid), false),
	...
	if p.supervisor != nil {
		p.supervisor.SetPublishedTrack(samvaad.TrackID(ti.Sid), mt)
	}
	p.UpTrackManager.AddPublishedTrack(mt)

	pti := p.pendingTracks[signalCid]
	if pti != nil {
		if p.pendingPublishingTracks[samvaad.TrackID(ti.Sid)] != nil {
			p.pubLogger.Infow("unexpected pending publish track", "trackID", ti.Sid)
		}
		p.pendingPublishingTracks[samvaad.TrackID(ti.Sid)] = &pendingTrackInfo{
			trackInfos: []*samvaad.TrackInfo{pti.trackInfos[0]},
	...
	trackID := samvaad.TrackID(ti.Sid)
...
func (p *ParticipantImpl) getPendingTrack(clientId string, kind samvaad.TrackType, skipQueued bool) (string, *samvaad.TrackInfo, buffer.VideoLayersRid, bool, time.Time) {
	signalCid := clientId
...
func (p *ParticipantImpl) setTrackID(cid string, info *samvaad.TrackInfo) {
	var trackID string
...
func (p *ParticipantImpl) getAndDeleteForwarderState(trackID samvaad.TrackID) *samvaad.RTPForwarderState {
	p.lock.Lock()
...
func (p *ParticipantImpl) CacheDownTrack(trackID samvaad.TrackID, rtpTransceiver *webrtc.RTPTransceiver, downTrack sfu.DownTrackState) {
	p.lock.Lock()
...
func (p *ParticipantImpl) GetCachedDownTrack(trackID samvaad.TrackID) (*webrtc.RTPTransceiver, sfu.DownTrackState) {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *ParticipantImpl) onPublicationError(trackID samvaad.TrackID) {
...
func (p *ParticipantImpl) onSubscriptionError(trackID samvaad.TrackID, fatal bool, err error) {
	signalErr := samvaad.SubscriptionError_SE_UNKNOWN
	switch {
	case errors.Is(err, webrtc.ErrUnsupportedCodec):
		signalErr = samvaad.SubscriptionError_SE_CODEC_UNSUPPORTED
	case errors.Is(err, ErrTrackNotFound):
		signalErr = samvaad.SubscriptionError_SE_TRACK_NOTFOUND
	}
...
func (p *ParticipantImpl) UpdateSubscribedQuality(nodeID samvaad.NodeID, trackID samvaad.TrackID, maxQualities []types.SubscribedCodecQuality) error {
...
func (p *ParticipantImpl) UpdateSubscribedAudioCodecs(nodeID samvaad.NodeID, trackID samvaad.TrackID, codecs []*samvaad.SubscribedAudioCodec) error {
	track := p.GetPublishedTrack(trackID)
	if track == nil {
		p.pubLogger.Debugw("could not find track", "trackID", trackID)
		return errors.New("could not find published track")
...
func (p *ParticipantImpl) UpdateMediaLoss(nodeID samvaad.NodeID, trackID samvaad.TrackID, fractionalLoss uint32) error {
	track := p.GetPublishedTrack(trackID)
	if track == nil {
		p.pubLogger.Debugw("could not find track", "trackID", trackID)
		return errors.New("could not find published track")
...
func (p *ParticipantImpl) GetPlayoutDelayConfig() *samvaad.PlayoutDelay {
...
func (p *ParticipantImpl) SendDataMessage(kind samvaad.DataPacket_Kind, data []byte, sender samvaad.ParticipantID, seq uint32) error {
	if sender == "" || kind != samvaad.DataPacket_RELIABLE || seq == 0 {
		if p.State() != samvaad.ParticipantInfo_ACTIVE {
...
func (p *ParticipantImpl) SendDataMessageUnlabeled(data []byte, useRaw bool, sender samvaad.ParticipantIdentity) error {
	if p.State() != samvaad.ParticipantInfo_ACTIVE {
		return ErrDataChannelUnavailable
	}
...
func (p *ParticipantImpl) setupEnabledCodecs(publishEnabledCodecs []*samvaad.Codec, subscribeEnabledCodecs []*samvaad.Codec, disabledCodecs *samvaad.DisabledCodecs) {
	shouldDisable := func(c *samvaad.Codec, disabled []*samvaad.Codec) bool {
	...
	publishCodecsAudio := make([]*samvaad.Codec, 0, len(publishEnabledCodecs))
	publishCodecsVideo := make([]*samvaad.Codec, 0, len(publishEnabledCodecs))
	...
	subscribeCodecs := make([]*samvaad.Codec, 0, len(subscribeEnabledCodecs))
...
func (p *ParticipantImpl) replayJoiningReliableMessages() {
	...
	for _, msgCache := range p.helper().GetCachedReliableDataMessage(p.reliableDataInfo.joiningMessageFirstSeqs) {
		...
		p.TransportManager.SendDataMessage(samvaad.DataPacket_RELIABLE, msgCache.Data)
	}

	p.reliableDataInfo.joiningMessageFirstSeqs = make(map[samvaad.ParticipantID]uint32)
	p.reliableDataInfo.canWriteReliable = true
	p.reliableDataInfo.joiningMessageLock.Unlock()
}

func (p *ParticipantImpl) GetEnabledPublishCodecs() []*samvaad.Codec {
	codecs := make([]*samvaad.Codec, 0, len(p.enabledPublishCodecs))
...
func (p *ParticipantImpl) UpdateAudioTrack(update *samvaad.UpdateLocalAudioTrack) error {
		...
		for _, ti := range pti.trackInfos {
			if ti.Sid == update.TrackSid {
				...
				for _, feature := range update.Features {
					switch feature {
					case samvaad.AudioTrackFeature_TF_STEREO:
						ti.Stereo = true
					case samvaad.AudioTrackFeature_TF_NO_DTX:
						ti.DisableDtx = true
					}
				}
	...
	p.sendRequestResponse(&samvaad.RequestResponse{
		Reason: samvaad.RequestResponse_NOT_FOUND,
		Request: &samvaad.RequestResponse_UpdateAudioTrack{
...
func (p *ParticipantImpl) UpdateVideoTrack(update *samvaad.UpdateLocalVideoTrack) error {
	...
	p.sendRequestResponse(&samvaad.RequestResponse{
		Reason: samvaad.RequestResponse_NOT_FOUND,
		Request: &samvaad.RequestResponse_UpdateVideoTrack{
...
func (p *ParticipantImpl) HandleMetrics(senderParticipantID samvaad.ParticipantID, metrics *samvaad.MetricsBatch) error {
	if p.State() != samvaad.ParticipantInfo_ACTIVE {
		return ErrDataChannelUnavailable
	}
...
func (p *ParticipantImpl) SupportsMoving() error {
	...
	if kind := p.Kind(); kind == samvaad.ParticipantInfo_EGRESS || kind == samvaad.ParticipantInfo_AGENT || p.params.UseOneShotSignallingMode {
...
func (p *ParticipantImpl) HandleUpdateSubscriptionPermission(subscriptionPermission *samvaad.SubscriptionPermission) error {
	return p.listener().OnUpdateSubscriptionPermission(p, subscriptionPermission)
}

func (p *ParticipantImpl) HandleSyncState(syncState *samvaad.SyncState) error {
	return p.listener().OnSyncState(p, syncState)
}

func (p *ParticipantImpl) HandleSimulateScenario(simulateScenario *samvaad.SimulateScenario) error {
...
func (p *ParticipantImpl) PerformRpc(req *samvaad.PerformRpcRequest, resultCh chan string, errorCh chan error) {
	...
	go func() {
		...
		rpcRequest := &samvaad.DataPacket{
			Kind:                samvaad.DataPacket_RELIABLE,
			ParticipantIdentity: id,
			Value: &samvaad.DataPacket_RpcRequest{
				RpcRequest: &samvaad.RpcRequest{
					Id:                id,
		...
		err = p.SendDataMessage(samvaad.DataPacket_RELIABLE, data, samvaad.ParticipantID(id), 0)
		if err != nil {
			ackTimer.Stop()
			responseTimer.Stop()
			errorCh <- psrpc.NewError(psrpc.Internal, err)
