...
func (s *signalhandler) HandleMessage(msg proto.Message) error {
	req, ok := msg.(*samvaad.SignalRequest)
	if !ok {
	...
	case *samvaad.SignalRequest_Subscription:
		...
		s.params.Participant.HandleUpdateSubscriptions(
			samvaad.StringsAsIDs[samvaad.TrackID](msg.Subscription.TrackSids),
			msg.Subscription.ParticipantTracks,
			msg.Subscription.Subscribe,
		)

	case *samvaad.SignalRequest_TrackSetting:
		for _, sid := range samvaad.StringsAsIDs[samvaad.TrackID](msg.TrackSetting.TrackSids) {
			s.params.Participant.UpdateSubscribedTrackSettings(sid, msg.TrackSetting)
		}

	case *samvaad.SignalRequest_Leave:
		...
		case samvaad.DisconnectReason_CLIENT_INITIATED:
			reason = types.ParticipantCloseReasonClientRequestLeave
		case samvaad.DisconnectReason_USER_UNAVAILABLE:
			reason = types.ParticipantCloseReasonUserUnavailable
		case samvaad.DisconnectReason_USER_REJECTED:
			reason = types.ParticipantCloseReasonUserRejected
		case samvaad.DisconnectReason_AGENT_ERROR:
	...
	case *samvaad.SignalRequest_SubscriptionPermission:
	...
	case *samvaad.SignalRequest_SyncState:
	...
	case *samvaad.SignalRequest_Simulate:
	...
	case *samvaad.SignalRequest_UpdateMetadata:
		s.params.Participant.UpdateMetadata(msg.UpdateMetadata, false)

	case *samvaad.SignalRequest_UpdateAudioTrack:
	...
	case *samvaad.SignalRequest_UpdateVideoTrack:
