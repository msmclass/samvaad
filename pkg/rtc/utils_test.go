...
func TestPackStreamId(t *testing.T) {
	...
	require.Equal(t, samvaad.ParticipantID("PA_123abc"), pID)
	require.Equal(t, samvaad.TrackID("uuid-id"), trackID)

	require.Equal(t, packed, PackStreamID(pID, trackID))
}

func TestPackDataTrackLabel(t *testing.T) {
	pID := samvaad.ParticipantID("PA_123abc")
	trackID := samvaad.TrackID("TR_b3da25")
	label := "trackLabel"
...
func TestChunkProtoBatch(t *testing.T) {
	...
	var updates []*samvaad.ParticipantInfo
	for range 32 {
		updates = append(updates, &samvaad.ParticipantInfo{
