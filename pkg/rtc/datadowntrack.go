...
func (d *DataDownTrack) ID() samvaad.TrackID {
...
func (d *DataDownTrack) SubscriberID() samvaad.ParticipantID {
	// add `createdAt` to ensure repeated subscriptions from same subscriber to same publisher does not collide
	return samvaad.ParticipantID(fmt.Sprintf("%s:%d", d.params.SubscriberID, d.createdAt))
}
...
func (d *DataDownTrack) UpdateSubscriptionOptions(subscriptionOptions *samvaad.DataTrackSubscriptionOptions) {
	// DT-TODO
}
