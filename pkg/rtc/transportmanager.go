...
import (
	...
	"github.com/msmclass/samvaad/pkg/sfu/datachannel"
...
type TransportManager struct {
	...
	pendingDataChannelsPublisher []*samvaad.DataChannelInfo
	...
	onICEConfigChanged func(iceConfig *samvaad.ICEConfig)
...
func NewTransportManager(params TransportManagerParams) (*TransportManager, error) {
	...
	lgr := LoggerWithPCTarget(params.Logger, samvaad.SignalTarget_PUBLISHER)
	...
	var publisherSubscribeCodecs []*samvaad.Codec
	...
	if !t.params.UseOneShotSignallingMode && !t.params.UseSinglePeerConnection {
		lgr := LoggerWithPCTarget(params.Logger, samvaad.SignalTarget_SUBSCRIBER)
...
func (t *TransportManager) SendDataMessage(kind samvaad.DataPacket_Kind, data []byte) error {
...
func (t *TransportManager) SendDataMessageUnlabeled(data []byte, useRaw bool, sender samvaad.ParticipantIdentity) error {
	// downstream data is sent via primary peer connection
...
func (t *TransportManager) createDataChannelsForSubscriber(pendingDataChannels []*samvaad.DataChannelInfo) error {
	var (
...
func (t *TransportManager) AddICECandidate(candidate webrtc.ICECandidateInit, target samvaad.SignalTarget) {
	switch target {
	case samvaad.SignalTarget_PUBLISHER:
		if t.publisher != nil {
	...
	case samvaad.SignalTarget_SUBSCRIBER:
		if t.subscriber != nil {
...
func (t *TransportManager) HandleClientReconnect(reason samvaad.ReconnectReason) {
	var (
		isShort              bool
	...
	switch reason {
	case samvaad.ReconnectReason_RR_PUBLISHER_FAILED:
		if t.publisher != nil {
			resetShortConnection = true
			isShort, duration = t.publisher.IsShortConnection(time.Now())
		}

	case samvaad.ReconnectReason_RR_SUBSCRIBER_FAILED:
		if t.subscriber != nil {
			resetShortConnection = true
...
func (t *TransportManager) ICERestart(iceConfig *samvaad.ICEConfig) error {
...
func (t *TransportManager) OnICEConfigChanged(f func(iceConfig *samvaad.ICEConfig)) {
	t.lock.Lock()
	t.onICEConfigChanged = f
	t.lock.Unlock()
}

func (t *TransportManager) SetICEConfig(iceConfig *samvaad.ICEConfig) {
	if iceConfig != nil {
		t.configureICE(iceConfig, true)
	}
}

func (t *TransportManager) GetICEConfig() *samvaad.ICEConfig {
	t.lock.RLock()
...
func (t *TransportManager) configureICE(iceConfig *samvaad.ICEConfig, reset bool) {
	t.lock.Lock()
	...
	if iceConfig.PreferenceSubscriber != samvaad.ICECandidateType_ICT_NONE {
		t.mediaLossProxy.OnMediaLossUpdate(nil)
	}

	if t.publisher != nil {
		t.publisher.SetPreferTCP(iceConfig.PreferencePublisher == samvaad.ICECandidateType_ICT_TCP)
	}
	if t.subscriber != nil {
		t.subscriber.SetPreferTCP(iceConfig.PreferenceSubscriber == samvaad.ICECandidateType_ICT_TCP)
	}
...
func (t *TransportManager) handleConnectionFailed(isShortLived bool) {
	...
	getNext := func(ic *samvaad.ICEConfig) samvaad.ICECandidateType {
		...
		return samvaad.ICECandidateType_ICT_NONE
	}

	var preferNext samvaad.ICECandidateType
	if isShortLived {
	...
	t.configureICE(&samvaad.ICEConfig{
...
func (t *TransportManager) SetMigrateInfo(
	...
	t.lock.Lock()
	t.pendingDataChannelsPublisher = make([]*samvaad.DataChannelInfo, 0, len(dataChannels))
	pendingDataChannelsSubscriber := make([]*samvaad.DataChannelInfo, 0, len(dataChannels))
	for _, dci := range dataChannels {
		if dci.Target == samvaad.SignalTarget_SUBSCRIBER {
