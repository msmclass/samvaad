...
type Room struct {
	...
	protoRoom  *samvaad.Room
	internal   *samvaad.RoomInternal
	protoProxy *utils.ProtoProxy[*samvaad.Room]
	...
	participants              map[samvaad.ParticipantIdentity]types.LocalParticipant
	participantOpts           map[samvaad.ParticipantIdentity]*ParticipantOptions
	participantRequestSources map[samvaad.ParticipantIdentity]routing.MessageSource
	hasPublished              map[samvaad.ParticipantIdentity]bool
	agentParticpants          map[samvaad.ParticipantIdentity]*agentJob
	bufferFactory             *buffer.FactoryOfBufferFactory

	// batch update participant info for non-publishers
	batchedUpdates   map[samvaad.ParticipantIdentity]*ParticipantUpdate
	...
	disconnectSignalOnResumeParticipants           map[samvaad.ParticipantIdentity]time.Time
	disconnectSignalOnResumeNoMessagesParticipants map[samvaad.ParticipantIdentity]*disconnectSignalOnResumeNoMessages
...
func newAgentDispatch(ad *samvaad.AgentDispatch) *agentDispatch {
...
func newAgentJob(j *samvaad.Job) *agentJob {
	return &agentJob{
		Job:  j,
...
func NewRoom(
	...
	r := &Room{
		...
		participants:                         make(map[samvaad.ParticipantIdentity]types.LocalParticipant),
		participantOpts:                      make(map[samvaad.ParticipantIdentity]*ParticipantOptions),
		participantRequestSources:            make(map[samvaad.ParticipantIdentity]routing.MessageSource),
		hasPublished:                         make(map[samvaad.ParticipantIdentity]bool),
		agentParticpants:                     make(map[samvaad.ParticipantIdentity]*agentJob),
		...
		batchedUpdates:                       make(map[samvaad.ParticipantIdentity]*ParticipantUpdate),
		closed:                               make(chan struct{}),
		trailer:                              []byte(utils.RandomSecret()),
		disconnectSignalOnResumeParticipants: make(map[samvaad.ParticipantIdentity]time.Time),
		disconnectSignalOnResumeNoMessagesParticipants: make(map[samvaad.ParticipantIdentity]*disconnectSignalOnResumeNoMessages),
...
func (r *Room) ToProto() *samvaad.Room {
	return r.protoProxy.Get()
}

func (r *Room) Name() samvaad.RoomName {
	return samvaad.RoomName(r.protoRoom.Name)
}

func (r *Room) ID() samvaad.RoomID {
	return samvaad.RoomID(r.protoRoom.Sid)
}
...
func (r *Room) GetParticipant(identity samvaad.ParticipantIdentity) types.LocalParticipant {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) GetParticipantByID(participantID samvaad.ParticipantID) types.LocalParticipant {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) GetActiveSpeakers() []*samvaad.SpeakerInfo {
	participants := r.GetParticipants()
	speakers := make([]*samvaad.SpeakerInfo, 0, len(participants))
	for _, p := range participants {
		level, active := p.GetAudioLevel()
		if !active {
			continue
		}
		speakers = append(speakers, &samvaad.SpeakerInfo{
			Sid:    string(p.ID()),
			Level:  float32(level),
			Active: active,
		})
	...
	slices.SortFunc(speakers, func(a, b *samvaad.SpeakerInfo) int {
...
func (r *Room) Internal() *samvaad.RoomInternal {
	return r.internal
}
...
func (r *Room) Join(
	...
	r.launchTargetAgents(slices.Collect(maps.Values(r.agentDispatches)), participant, samvaad.JobType_JT_PARTICIPANT)
...
func (r *Room) ReplaceParticipantRequestSource(identity samvaad.ParticipantIdentity, reqSource routing.MessageSource) {
	r.lock.Lock()
...
func (r *Room) GetParticipantRequestSource(identity samvaad.ParticipantIdentity) routing.MessageSource {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) ResumeParticipant(
	...
	if err := p.HandleReconnectAndSendResponse(reason, &samvaad.ReconnectResponse{
...
func (r *Room) HandleSyncState(participant types.LocalParticipant, state *samvaad.SyncState) error {
	if state != nil {
...
func (r *Room) onSyncState(participant types.LocalParticipant, state *samvaad.SyncState) error {
	...
	for _, pubTrack := range pubTracks {
		...
		for _, existingPubTrack := range existingPubTracks {
			if existingPubTrack.ID() == samvaad.TrackID(ti.Sid) {
				found = true
				break
			}
		}
		if !found {
			...
			found = participant.GetPendingTrack(samvaad.TrackID(ti.Sid)) != nil
		}
		if !found {
	...
	for _, pubDataTrack := range pubDataTracks {
		...
		for _, existingPubDataTrack := range existingPubDataTracks {
			if existingPubDataTrack.ID() == samvaad.TrackID(dti.Sid) {
				found = true
				break
			}
		}
	...
	for _, trackSid := range state.TrackSidsDisabled {
		participant.UpdateSubscribedTrackSettings(samvaad.TrackID(trackSid), &samvaad.UpdateTrackSettings{Disabled: true})
	}

	participant.HandleUpdateSubscriptions(
		samvaad.StringsAsIDs[samvaad.TrackID](state.Subscription.TrackSids),
...
func (r *Room) onUpdateSubscriptionPermission(participant types.LocalParticipant, subscriptionPermission *samvaad.SubscriptionPermission) error {
...
func (r *Room) ResolveMediaTrackForSubscriber(sub types.LocalParticipant, trackID samvaad.TrackID) types.MediaResolverResult {
...
func (r *Room) ResolveDataTrackForSubscriber(sub types.LocalParticipant, trackID samvaad.TrackID) types.DataResolverResult {
...
func (r *Room) SendDataPacket(dp *samvaad.DataPacket, kind samvaad.DataPacket_Kind) {
...
func (r *Room) GetAgentDispatches(dispatchID string) ([]*samvaad.AgentDispatch, error) {
	r.lock.RLock()
	defer r.lock.RUnlock()

	var ret []*samvaad.AgentDispatch
...
func (r *Room) AddAgentDispatch(dispatch *samvaad.AgentDispatch) (*samvaad.AgentDispatch, error) {
	...
	for _, p := range r.participants {
		if p.IsPublisher() {
			r.launchTargetAgents([]*agentDispatch{ad}, p, samvaad.JobType_JT_PUBLISHER)
		}
		r.launchTargetAgents([]*agentDispatch{ad}, p, samvaad.JobType_JT_PARTICIPANT)
	}
	r.lock.RUnlock()
...
func (r *Room) DeleteAgentDispatch(dispatchID string) (*samvaad.AgentDispatch, error) {
	r.lock.Lock()
	...
	go func() {
		...
		for _, j := range jobs {
			...
			if state.ParticipantIdentity != "" {
				r.lock.RLock()
				agentJob := r.agentParticpants[samvaad.ParticipantIdentity(state.ParticipantIdentity)]
				p := r.participants[samvaad.ParticipantIdentity(state.ParticipantIdentity)]
				r.lock.RUnlock()
...
func (r *Room) onSimulateScenario(participant types.LocalParticipant, simulateScenario *samvaad.SimulateScenario) error {
	switch scenario := simulateScenario.Scenario.(type) {
	case *samvaad.SimulateScenario_SpeakerUpdate:
		...
		go func() {
			...
			r.sendSpeakerChanges([]*samvaad.SpeakerInfo{{
	...
	case *samvaad.SimulateScenario_Migration:
	...
	case *samvaad.SimulateScenario_NodeFailure:
	...
	case *samvaad.SimulateScenario_ServerLeave:
	...
	case *samvaad.SimulateScenario_SwitchCandidateProtocol:
		r.logger.Infow("simulating switch candidate protocol", "participant", participant.Identity())
		participant.ICERestart(&samvaad.ICEConfig{
			PreferenceSubscriber: samvaad.ICECandidateType(scenario.SwitchCandidateProtocol),
			PreferencePublisher:  samvaad.ICECandidateType(scenario.SwitchCandidateProtocol),
		})
	case *samvaad.SimulateScenario_SubscriberBandwidth:
	...
	case *samvaad.SimulateScenario_DisconnectSignalOnResume:
		...
		r.simulationLock.Unlock()
	case *samvaad.SimulateScenario_DisconnectSignalOnResumeNoMessages:
...
func (r *Room) createJoinResponseLocked(
	participant types.LocalParticipant,
	iceServers []*samvaad.ICEServer,
) *samvaad.JoinResponse {
	iceConfig := participant.GetICEConfig()
	hasICEFallback := iceConfig.GetPreferencePublisher() != samvaad.ICECandidateType_ICT_NONE || iceConfig.GetPreferenceSubscriber() != samvaad.ICECandidateType_ICT_NONE
	return &samvaad.JoinResponse{
		Room:        r.ToProto(),
...
func (r *Room) onTrackPublished(participant types.Participant, track types.MediaTrack) {
	...
	for _, existingParticipant := range r.participants {
		...
		if existingParticipant.State() != samvaad.ParticipantInfo_ACTIVE {
	...
	if !hasPublished {
		r.lock.RLock()
		r.launchTargetAgents(slices.Collect(maps.Values(r.agentDispatches)), participant, samvaad.JobType_JT_PUBLISHER)
		r.lock.RUnlock()
	...
	if participant.Kind() != samvaad.ParticipantInfo_EGRESS && r.internal != nil && r.internal.TrackEgress != nil {
		go func() {
...
func (r *Room) onStateChange(p types.LocalParticipant) {
	...
	switch p.State() {
	case samvaad.ParticipantInfo_ACTIVE:
		...
		meta := &samvaad.AnalyticsClientMeta{
	...
	case samvaad.ParticipantInfo_DISCONNECTED:
		// remove participant from room
		go r.RemoveParticipant(p.Identity(), p.ID(), p.CloseReason())
	}
}

func (r *Room) onDataMessage(source types.LocalParticipant, kind samvaad.DataPacket_Kind, dp *samvaad.DataPacket) {
	if kind == samvaad.DataPacket_RELIABLE && source != nil && dp.GetSequence() > 0 {
		...
		r.dataMessageCache.Add(&types.DataMessageCache{
			...
			Seq:            dp.GetSequence(),
			Data:           data,
			DestIdentities: samvaad.StringsAsIDs[samvaad.ParticipantIdentity](dp.DestinationIdentities),
		}, len(data))
	}
...
func (r *Room) onMetrics(source types.Participant, dp *samvaad.DataPacket) {
	BroadcastMetricsForRoom(r, source, dp, r.logger)
}

func (r *Room) onSubscribeStatusChanged(participant types.LocalParticipant, publisherID samvaad.ParticipantID, subscribed bool) {
	if subscribed {
		pub := r.GetParticipantByID(publisherID)
		if pub != nil && pub.State() == samvaad.ParticipantInfo_ACTIVE {
			...
			if active {
				_ = participant.SendSpeakerUpdate([]*samvaad.SpeakerInfo{
					{
						Sid:    string(pub.ID()),
						Level:  float32(level),
						Active: active,
			...
			if cq := pub.GetConnectionQuality(); cq != nil {
				update := &samvaad.ConnectionQualityUpdate{}
...
func (r *Room) UpdateSubscriptions(
	...
	for _, pt := range participantTracks {
		for _, trackID := range samvaad.StringsAsIDs[samvaad.TrackID](pt.TrackSids) {
			if subscribe {
				participant.SubscribeToTrack(trackID, false)
			} else {
				participant.UnsubscribeFromTrack(trackID)
...
func (r *Room) onUpdateDataSubscriptions(participant types.LocalParticipant, req *samvaad.UpdateDataSubscription) {
	for _, update := range req.Updates {
		trackID := samvaad.TrackID(update.TrackSid)
		if update.Subscribe {
...
func (r *Room) broadcastParticipantState(p types.Participant, opts broadcastOptions) {
	...
	if !opts.skipSource {
		defer func() {
			...
			if lp, ok := p.(types.LocalParticipant); ok {
				err := lp.SendParticipantUpdate([]*samvaad.ParticipantInfo{pi})
				if err != nil {
	...
	updates := PushAndDequeueUpdates(
		...
		p.CloseReason(),
		opts.immediate,
		r.GetParticipant(samvaad.ParticipantIdentity(pi.Identity)),
		r.batchedUpdates,
	)
...
func (r *Room) sendSpeakerChanges(speakers []*samvaad.SpeakerInfo) {
	for _, p := range r.GetParticipants() {
...
func (r *Room) updateProto() *samvaad.Room {
	r.lock.RLock()
...
func (r *Room) changeUpdateWorker() {
	...
	for !r.IsClosed() {
		...
		case <-subTicker.C:
			...
			r.batchedUpdates = make(map[samvaad.ParticipantIdentity]*ParticipantUpdate)
			r.batchedUpdatesMu.Unlock()
...
func (r *Room) audioUpdateWorker() {
	lastActiveMap := make(map[samvaad.ParticipantID]*samvaad.SpeakerInfo)
	for {
		if r.IsClosed() {
			return
		}
		...
		changedSpeakers := make([]*samvaad.SpeakerInfo, 0, len(activeSpeakers))
		nextActiveMap := make(map[samvaad.ParticipantID]*samvaad.SpeakerInfo, len(activeSpeakers))
		for _, speaker := range activeSpeakers {
			prev := lastActiveMap[samvaad.ParticipantID(speaker.Sid)]
			if prev == nil || prev.Level != speaker.Level {
				changedSpeakers = append(changedSpeakers, speaker)
			}
			nextActiveMap[samvaad.ParticipantID(speaker.Sid)] = speaker
		}
...
func (r *Room) connectionQualityWorker() {
	...
	prevConnectionInfos := make(map[samvaad.ParticipantID]*samvaad.ConnectionQualityInfo)
	// send updates to only users that are subscribed to each other
	for !r.IsClosed() {
		<-ticker.C

		participants := r.GetParticipants()
		nowConnectionInfos := make(map[samvaad.ParticipantID]*samvaad.ConnectionQualityInfo, len(participants))
		...
		maybeAddToUpdate := func(pID samvaad.ParticipantID, update *samvaad.ConnectionQualityUpdate) {
		...
		for _, op := range participants {
			if !op.ProtocolVersion().SupportsConnectionQuality() || op.State() != samvaad.ParticipantInfo_ACTIVE {
				continue
			}
			update := &samvaad.ConnectionQualityUpdate{}
...
func (r *Room) launchTargetAgents(ads []*agentDispatch, p types.Participant, jobType samvaad.JobType) {
...
func (r *Room) handleNewJobs(ad *samvaad.AgentDispatch, inc *sutils.IncrementalDispatcher[*samvaad.Job]) {
	inc.ForEach(func(job *samvaad.Job) {
		...
		if job.State != nil && job.State.ParticipantIdentity != "" {
			r.agentParticpants[samvaad.ParticipantIdentity(job.State.ParticipantIdentity)] = newAgentJob(job)
		}
		r.lock.Unlock()
	})
}
...
func (r *Room) createAgentDispatch(dispatch *samvaad.AgentDispatch) (*agentDispatch, error) {
	dispatch.State = &samvaad.AgentDispatchState{
...
func (r *Room) createAgentDispatchFromRoomDispatch(rad *samvaad.RoomAgentDispatch) (*agentDispatch, error) {
	if err := protoagent.ValidateDeployment(rad.GetDeployment()); err != nil {
		return nil, err
	}
	return r.createAgentDispatch(&samvaad.AgentDispatch{
...
func (r *Room) createAgentDispatchesFromRoomAgent() {
	...
	if len(roomDisp) == 0 {
		...
		roomDisp = []*samvaad.RoomAgentDispatch{{}}
	}
...
func (r *Room) IsDataMessageUserPacketDuplicate(up *samvaad.UserPacket) bool {
	return r.userPacketDeduper.IsDuplicate(up)
}

func (r *Room) GetCachedReliableDataMessage(seqs map[samvaad.ParticipantID]uint32) []*types.DataMessageCache {
...
func (l *localParticipantListener) OnMetrics(p types.Participant, dp *samvaad.DataPacket) {
...
func (l *localParticipantListener) OnDataMessage(p types.LocalParticipant, kind samvaad.DataPacket_Kind, dp *samvaad.DataPacket) {
...
func (l *localParticipantListener) OnSubscribeStatusChanged(p types.LocalParticipant, publisherID samvaad.ParticipantID, subscribed bool) {
...
func (l *localParticipantListener) OnUpdateSubscriptionPermission(p types.LocalParticipant, subscriptionPermission *samvaad.SubscriptionPermission) error {
	return l.room.onUpdateSubscriptionPermission(p, subscriptionPermission)
}

func (l *localParticipantListener) OnUpdateDataSubscriptions(p types.LocalParticipant, req *samvaad.UpdateDataSubscription) {
	l.room.onUpdateDataSubscriptions(p, req)
}

func (l *localParticipantListener) OnSyncState(p types.LocalParticipant, state *samvaad.SyncState) error {
	return l.room.onSyncState(p, state)
}

func (l *localParticipantListener) OnSimulateScenario(p types.LocalParticipant, simulateScenario *samvaad.SimulateScenario) error {
...
func (l participantTelemetryListener) OnTrackPublishRequested(pID samvaad.ParticipantID, identity samvaad.ParticipantIdentity, ti *samvaad.TrackInfo) {
	l.room.telemetry.TrackPublishRequested(context.Background(), l.room.ID(), l.room.Name(), pID, identity, ti)
}

func (l participantTelemetryListener) OnTrackPublished(pID samvaad.ParticipantID, identity samvaad.ParticipantIdentity, ti *samvaad.TrackInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackPublished(context.Background(), l.room.ID(), l.room.Name(), pID, identity, ti, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackUnpublished(pID samvaad.ParticipantID, identity samvaad.ParticipantIdentity, ti *samvaad.TrackInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackUnpublished(context.Background(), l.room.ID(), l.room.Name(), pID, identity, ti, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackSubscribeRequested(pID samvaad.ParticipantID, ti *samvaad.TrackInfo) {
	l.room.telemetry.TrackSubscribeRequested(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackSubscribed(pID samvaad.ParticipantID, ti *samvaad.TrackInfo, publisherInfo *samvaad.ParticipantInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackSubscribed(context.Background(), l.room.ID(), l.room.Name(), pID, ti, publisherInfo, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackUnsubscribed(pID samvaad.ParticipantID, ti *samvaad.TrackInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackUnsubscribed(context.Background(), l.room.ID(), l.room.Name(), pID, ti, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackSubscribeFailed(pID samvaad.ParticipantID, trackID samvaad.TrackID, err error, isUserError bool) {
	l.room.telemetry.TrackSubscribeFailed(context.Background(), l.room.ID(), l.room.Name(), pID, trackID, err, isUserError)
}

func (l participantTelemetryListener) OnTrackSubscribeStreamStarted(pID samvaad.ParticipantID, ti *samvaad.TrackInfo) {
}

func (l participantTelemetryListener) OnTrackMuted(pID samvaad.ParticipantID, ti *samvaad.TrackInfo) {
	l.room.telemetry.TrackMuted(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackUnmuted(pID samvaad.ParticipantID, ti *samvaad.TrackInfo) {
	l.room.telemetry.TrackUnmuted(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackPublishedUpdate(pID samvaad.ParticipantID, ti *samvaad.TrackInfo) {
	l.room.telemetry.TrackPublishedUpdate(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackMaxSubscribedVideoQuality(pID samvaad.ParticipantID, ti *samvaad.TrackInfo, mime mime.MimeType, maxQuality samvaad.VideoQuality) {
	l.room.telemetry.TrackMaxSubscribedVideoQuality(context.Background(), l.room.ID(), l.room.Name(), pID, ti, mime, maxQuality)
}

func (l participantTelemetryListener) OnTrackPublishRTPStats(pID samvaad.ParticipantID, trackID samvaad.TrackID, mimeType mime.MimeType, layer int, stats *samvaad.RTPStats) {
	l.room.telemetry.TrackPublishRTPStats(context.Background(), l.room.ID(), l.room.Name(), pID, trackID, mimeType, layer, stats)
}

func (l participantTelemetryListener) OnTrackSubscribeRTPStats(pID samvaad.ParticipantID, trackID samvaad.TrackID, mimeType mime.MimeType, stats *samvaad.RTPStats) {
	l.room.telemetry.TrackSubscribeRTPStats(context.Background(), l.room.ID(), l.room.Name(), pID, trackID, mimeType, stats)
}

func (l participantTelemetryListener) OnTrackStats(key telemetry.StatsKey, stat *samvaad.AnalyticsStat) {
...
func BroadcastDataPacketForRoom(
	...
	utils.ParallelExec(destParticipants, dataForwardLoadBalanceThreshold, 1, func(op types.LocalParticipant) {
		op.SendDataMessage(kind, dpData, samvaad.ParticipantID(dp.GetParticipantSid()), dp.GetSequence())
	})
}
...
func BroadcastMetricsForRoom(r types.Room, source types.Participant, dp *samvaad.DataPacket, logger logger.Logger) {
...
func CompareParticipant(pi1 *samvaad.ParticipantInfo, pi2 *samvaad.ParticipantInfo) int {
...
type ParticipantUpdate struct {
	ParticipantInfo         *samvaad.ParticipantInfo
...
func PushAndDequeueUpdates(
	...
	cache map[samvaad.ParticipantIdentity]*ParticipantUpdate,
) []*ParticipantUpdate {
	...
	if existing != nil {
		if pi.Sid == existing.ParticipantInfo.Sid {
			...
			if CompareParticipant(existing.ParticipantInfo, pi) < 0 {
				...
				shouldSend = true
				existing.ParticipantInfo.State = samvaad.ParticipantInfo_DISCONNECTED
...
func SendParticipantUpdates(updates []*ParticipantUpdate, participants []types.LocalParticipant, batchTargetSize int) {
	...
	var filteredUpdates []*samvaad.ParticipantInfo
	...
	var fullUpdates []*samvaad.ParticipantInfo
	for _, update := range updates {
...
) []*samvaad.ParticipantInfo {
	var lpIdentity samvaad.ParticipantIdentity
	if lp != nil {
		lpIdentity = lp.Identity()
	}

	pInfos := make([]*samvaad.ParticipantInfo, 0, len(allParticipants))
