...
import (
	...
	"github.com/msmclass/samvaad/pkg/rtc/dynacast"
	...
	util "github.com/msmclass/samvaad/pkg/samvaad/transport"
)
...
type MediaTrack struct {
	...
	*MediaLossProxy
	...
	backupCodecPolicy             samvaad.BackupCodecPolicy
...
type MediaTrackParams struct {
	...
	ParticipantKind                  samvaad.ParticipantInfo_Kind
	ParticipantKindDetails           []samvaad.ParticipantInfo_KindDetail
	...
	OnTrackEverSubscribed            func(samvaad.TrackID)
...
func NewMediaTrack(params MediaTrackParams, ti *samvaad.TrackInfo) *MediaTrack {
	t := &MediaTrack{
		params:            params,
	...
	if t.params.ForceBackupCodecPolicySimulcast {
		t.backupCodecPolicy = samvaad.BackupCodecPolicy_SIMULCAST
	}

	if t.backupCodecPolicy != samvaad.BackupCodecPolicy_SIMULCAST && len(ti.Codecs) > 1 {
	...
	if ti.Type == samvaad.TrackType_AUDIO {
	...
	t.MediaTrackReceiver.OnSubscriberMaxQualityChange(
		func(subscriberID samvaad.ParticipantID, mimeType mime.MimeType, layer int32) {
			if t.dynacastManager != nil {
	...
	t.MediaTrackReceiver.OnSubscriberAudioCodecChange(
		func(subscriberID samvaad.ParticipantID, mimeType mime.MimeType, enabled bool) {
			if t.dynacastManager != nil {
...
func (t *MediaTrack) NotifySubscriberNodeMaxQuality(nodeID samvaad.NodeID, qualities []types.SubscribedCodecQuality) {
	if t.dynacastManager != nil {
		t.dynacastManager.NotifySubscriberNodeMaxQuality(nodeID, qualities)
	}
}

func (t *MediaTrack) NotifySubscriptionNode(nodeID samvaad.NodeID, codecs []*samvaad.SubscribedAudioCodec) {
	if t.dynacastManager != nil {
...
func (t *MediaTrack) ToProto() *samvaad.TrackInfo {
...
func (t *MediaTrack) AddReceiver(receiver *webrtc.RTPReceiver, track sfu.TrackRemote, mid string) (bool, bool) {
		...
		for _, pkt := range pkts {
			...
			case *rtcp.SenderReport:
				if pkt.SSRC == uint32(track.SSRC()) {
					buff.SetSenderReportData(&samvaad.RTCPSenderReportState{
	...
	if wr == nil {
		...
		newWR.OnStatsUpdate(func(_ *sfu.WebRTCReceiver, stat *samvaad.AnalyticsStat) {
	...
	buff.OnFinalRtpStats(func(stats *samvaad.RTPStats) {
...
func (t *MediaTrack) GetConnectionScoreAndQuality() (float32, samvaad.ConnectionQuality) {
	receiver := t.ActiveReceiver()
	if rtcReceiver, ok := receiver.(*sfu.WebRTCReceiver); ok {
		return rtcReceiver.GetConnectionScoreAndQuality()
	}

	return connectionquality.MaxMOS, samvaad.ConnectionQuality_EXCELLENT
}
...
func (t *MediaTrack) enableRegression() bool {
	return t.backupCodecPolicy == samvaad.BackupCodecPolicy_REGRESSION ||
		(t.backupCodecPolicy == samvaad.BackupCodecPolicy_PREFER_REGRESSION && t.params.ShouldRegressCodec())
}
...
func (t *MediaTrack) OnDynacastSubscribedAudioCodecChange(codecs []*samvaad.SubscribedAudioCodec) {
	t.lock.RLock()
