...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/auth/authfakes"
	...
	"github.com/msmclass/samvaad/pkg/sfu/audio"
...
var iceServersForRoom = []*samvaad.ICEServer{{Urls: []string{"stun:stun.l.google.com:19302"}}}
...
func TestRoomJoin(t *testing.T) {
	t.Run("joining returns existing participant data", func(t *testing.T) {
		...
		require.Equal(t, samvaad.RoomID(res.Room.Sid), rm.ID())
	...
	t.Run("subscribe to existing channels upon join", func(t *testing.T) {
		...
		p.StateReturns(samvaad.ParticipantInfo_ACTIVE)
		lpl.OnStateChange(p)
	...
	t.Run("participant state change is broadcasted to others", func(t *testing.T) {
		...
		disconnectedParticipant.StateReturns(samvaad.ParticipantInfo_DISCONNECTED)
...
func TestParticipantUpdate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			func(p types.LocalParticipant) {
				p.SetTrackMuted(&samvaad.MuteTrackRequest{Muted: true}, false)
			},
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			callCounts := make(map[samvaad.ParticipantID]int)
...
func TestPushAndDequeueUpdates(t *testing.T) {
	identity := "test_user"
	publisher1v1 := &samvaad.ParticipantInfo{
		Identity:    identity,
		Sid:         "1",
		IsPublisher: true,
		Version:     1,
	...
	publisher1v2 := &samvaad.ParticipantInfo{
		Identity:    identity,
		Sid:         "1",
		IsPublisher: true,
		Version:     2,
	...
	publisher2 := &samvaad.ParticipantInfo{
		Identity:    identity,
		Sid:         "2",
		IsPublisher: true,
		Version:     1,
	...
	subscriber1v1 := &samvaad.ParticipantInfo{
		Identity: identity,
		Sid:      "1",
		Version:  1,
		JoinedAt: 0,
	}
	subscriber1v2 := &samvaad.ParticipantInfo{
		Identity: identity,
		Sid:      "1",
		Version:  2,
		JoinedAt: 1,
	...
	requirePIEquals := func(t *testing.T, a, b *samvaad.ParticipantInfo) {
	...
	testCases := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, rm *Room, _ []*ParticipantUpdate) {
				queued := rm.batchedUpdates[samvaad.ParticipantIdentity(identity)]
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			rm := newRoomWithParticipants(t, testRoomOpts{num: 1})
			if tc.existing != nil {
				rm.batchedUpdates[samvaad.ParticipantIdentity(tc.existing.ParticipantInfo.Identity)] = tc.existing
			}
			...
			updates := PushAndDequeueUpdates(
				...
				tc.closeReason,
				tc.immediate,
				rm.GetParticipant(samvaad.ParticipantIdentity(tc.pi.Identity)),
				rm.batchedUpdates,
			)
...
func TestNewTrack(t *testing.T) {
	t.Run("new track should be added to ready participants", func(t *testing.T) {
		...
		p0.StateReturns(samvaad.ParticipantInfo_JOINED)
		p1 := participants[1].(*typesfakes.FakeLocalParticipant)
		p1.StateReturns(samvaad.ParticipantInfo_ACTIVE)

		pub := participants[2].(*typesfakes.FakeLocalParticipant)

		// pub adds track
		track := NewMockTrack(samvaad.TrackType_VIDEO, "webcam")
...
func TestActiveSpeakers(t *testing.T) {
	t.Parallel()
	getActiveSpeakerUpdates := func(p *typesfakes.FakeLocalParticipant) [][]*samvaad.SpeakerInfo {
		var updates [][]*samvaad.SpeakerInfo
...
func TestDataChannel(t *testing.T) {
	...
	setSource := func(mode int, dp *samvaad.DataPacket, p types.LocalParticipant) {
		switch mode {
		case curAPI:
	...
	setDest := func(mode int, dp *samvaad.DataPacket, p types.LocalParticipant) {
		switch mode {
		case curAPI:
...
func TestHiddenParticipants(t *testing.T) {
	...
	t.Run("hidden participant subscribes to tracks", func(t *testing.T) {
		...
		hidden.StateReturns(samvaad.ParticipantInfo_ACTIVE)
...
func newRoomWithParticipants(t *testing.T, opts testRoomOpts) *Room {
	...
	for i := 0; i < opts.num+opts.numHidden; i++ {
		identity := samvaad.ParticipantIdentity(fmt.Sprintf("p%d", i))
		...
		require.NoError(t, err)
		participant.StateReturns(samvaad.ParticipantInfo_ACTIVE)
