...
type MediaTrackSubscriptions struct {
	...
	subscribedTracks   map[samvaad.ParticipantID]types.SubscribedTrack

	onDownTrackCreated           func(downTrack *sfu.DownTrack)
	onSubscriberMaxQualityChange func(subscriberID samvaad.ParticipantID, mime mime.MimeType, layer int32)
	onSubscriberAudioCodecChange func(subscriberID samvaad.ParticipantID, mime mime.MimeType, enabled bool)
}
...
func NewMediaTrackSubscriptions(params MediaTrackSubscriptionsParams) *MediaTrackSubscriptions {
	return &MediaTrackSubscriptions{
		params:           params,
		subscribedTracks: make(map[samvaad.ParticipantID]types.SubscribedTrack),
	}
}
...
func (t *MediaTrackSubscriptions) OnSubscriberMaxQualityChange(f func(subscriberID samvaad.ParticipantID, mime mime.MimeType, layer int32)) {
	t.onSubscriberMaxQualityChange = f
}

func (t *MediaTrackSubscriptions) OnSubscriberAudioCodecChange(f func(subscriberID samvaad.ParticipantID, mime mime.MimeType, enabled bool)) {
...
func (t *MediaTrackSubscriptions) IsSubscriber(subID samvaad.ParticipantID) bool {
...
func (t *MediaTrackSubscriptions) AddSubscriber(sub types.LocalParticipant, wr *WrappedReceiver) (types.SubscribedTrack, error) {
	...
	subTrack, err := NewSubscribedTrack(SubscribedTrackParams{
		...
		OnDownTrackClosed: func(subscriberID samvaad.ParticipantID) {
			t.subscribedTracksMu.Lock()
	...
	if transceiver == nil {
		...
		addTrackParams := types.AddTrackParams{
			Stereo: slices.Contains(info.AudioFeatures, samvaad.AudioTrackFeature_TF_STEREO),
...
func (t *MediaTrackSubscriptions) RemoveSubscriber(subscriberID samvaad.ParticipantID, isExpectedToResume bool) error {
...
func (t *MediaTrackSubscriptions) GetAllSubscribers() []samvaad.ParticipantID {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()

	subs := make([]samvaad.ParticipantID, 0, len(t.subscribedTracks))
...
func (t *MediaTrackSubscriptions) GetAllSubscribersForMime(mime mime.MimeType) []samvaad.ParticipantID {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()
...
func (t *MediaTrackSubscriptions) getSubscribedTrack(subscriberID samvaad.ParticipantID) types.SubscribedTrack {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()
