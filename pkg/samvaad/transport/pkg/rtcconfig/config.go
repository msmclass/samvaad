...
func (conf *RTCConfig) Validate(development bool) error {
	...
	if conf.NodeIP.IsEmpty() || conf.UseExternalIP {
		ipAddr, err := conf.determineIP()
		if err != nil {
			logger.Warnw("could not determine node ip", err)
			return err
		}
		if net.ParseIP(ipAddr).To4() != nil {
			conf.NodeIP.V4 = ipAddr
		} else {
			conf.NodeIP.V6 = ipAddr
		}
...
func (n *NodeIP) UnmarshalString(str string) error {
	...
	parts := strings.Split(str, ",")
	if len(parts) > 2 {
		return fmt.Errorf("invalid node ip %s, should be <ipv4>,<ipv6>", str)
	}

	for _, part := range parts {
		ip := net.ParseIP(strings.Trim(part, " "))
		if ip == nil {
			return fmt.Errorf("invalid ip %s", part)
		}
		if ip.To4() != nil {
			if n.V4 != "" {
				return fmt.Errorf("multiple ipv4 addresses provided: %s and %s", n.V4, ip.String())
			}
			n.V4 = ip.String()
		} else {
			if n.V6 != "" {
				return fmt.Errorf("multiple ipv6 addresses provided: %s and %s", n.V6, ip.String())
			}
			n.V6 = ip.String()
		}
	}
