...
func Validate(info *samvaad.IngressInfo) error {
	if info == nil {
...
func ValidateForSerialization(info *samvaad.IngressInfo) error {
	if info == nil {
		return ErrInvalidIngress("missing IngressInfo")
	}

	if info.InputType != samvaad.IngressInput_RTMP_INPUT && info.InputType != samvaad.IngressInput_WHIP_INPUT && info.InputType != samvaad.IngressInput_URL_INPUT {
...
func ValidateBypassTranscoding(info *samvaad.IngressInfo) error {
	if !info.BypassTranscoding {
		return nil
	}

	if info.InputType != samvaad.IngressInput_WHIP_INPUT {
...
func ValidateEnableTranscoding(info *samvaad.IngressInfo) error {
...
func ValidateVideoOptionsConsistency(options *samvaad.IngressVideoOptions) error {
	if options == nil {
		return nil
	}
	...
	case samvaad.TrackSource_UNKNOWN,
		...
		samvaad.TrackSource_SCREEN_SHARE:
		// continue
	default:
	...
	case nil:
		// Default, continue
	case *samvaad.IngressVideoOptions_Preset:
		_, ok := samvaad.IngressVideoEncodingPreset_name[int32(o.Preset)]
		if !ok {
			return NewInvalidVideoParamsError("invalid preset")
		}

	case *samvaad.IngressVideoOptions_Options:
...
func ValidateVideoEncodingOptionsConsistency(options *samvaad.IngressVideoEncodingOptions) error {
	if options == nil {
	...
	case samvaad.VideoCodec_DEFAULT_VC,
		samvaad.VideoCodec_H264_BASELINE,
		samvaad.VideoCodec_VP8:
		// continue
	default:
		return NewInvalidVideoParamsError("video codec unsupported")
	}

	layersByQuality := make(map[samvaad.VideoQuality]*samvaad.VideoLayer)
...
func ValidateAudioOptionsConsistency(options *samvaad.IngressAudioOptions) error {
	if options == nil {
		return nil
	}
	...
	switch options.Source {
	case samvaad.TrackSource_UNKNOWN,
		samvaad.TrackSource_MICROPHONE,
		samvaad.TrackSource_SCREEN_SHARE_AUDIO:
		// continue
	default:
	...
	case nil:
		// Default, continue
	case *samvaad.IngressAudioOptions_Preset:
		_, ok := samvaad.IngressAudioEncodingPreset_name[int32(o.Preset)]
		if !ok {
			return NewInvalidAudioParamsError("invalid preset")
		}

	case *samvaad.IngressAudioOptions_Options:
...
func ValidateAudioEncodingOptionsConsistency(options *samvaad.IngressAudioEncodingOptions) error {
	if options == nil {
		return NewInvalidAudioParamsError("empty options")
	}

	switch options.AudioCodec {
	case samvaad.AudioCodec_DEFAULT_AC,
		samvaad.AudioCodec_OPUS:
		// continue
	default:
