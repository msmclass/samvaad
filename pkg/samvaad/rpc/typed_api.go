...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/tracer"
...
type TypedSignalClient = SignalClient[samvaad.NodeID]
type TypedSignalServer = SignalServer[samvaad.NodeID]

func NewTypedSignalClient(nodeID samvaad.NodeID, bus psrpc.MessageBus, opts ...psrpc.ClientOption) (TypedSignalClient, error) {
	return NewSignalClient[samvaad.NodeID](bus, psrpc.WithClientOptions(opts...), psrpc.WithClientID(string(nodeID)))
}

func NewTypedSignalServer(nodeID samvaad.NodeID, svc SignalServerImpl, bus psrpc.MessageBus, opts ...psrpc.ServerOption) (TypedSignalServer, error) {
	return NewSignalServer[samvaad.NodeID](svc, bus, psrpc.WithServerOptions(opts...), psrpc.WithServerID(string(nodeID)))
}

type TypedRoomManagerClient = RoomManagerClient[samvaad.NodeID]
type TypedRoomManagerServer = RoomManagerServer[samvaad.NodeID]

func NewTypedRoomManagerClient(bus psrpc.MessageBus, opts ...psrpc.ClientOption) (TypedRoomManagerClient, error) {
	return NewRoomManagerClient[samvaad.NodeID](bus, opts...)
}
...
func NewTypedRoomManagerServer(svc RoomManagerServerImpl, bus psrpc.MessageBus, opts ...psrpc.ServerOption) (TypedRoomManagerServer, error) {
	return NewRoomManagerServer[samvaad.NodeID](svc, bus, opts...)
}

type ParticipantTopic string
type RoomTopic string

func FormatParticipantTopic(roomName samvaad.RoomName, identity samvaad.ParticipantIdentity) ParticipantTopic {
	return ParticipantTopic(fmt.Sprintf("%s_%s", roomName, identity))
}

func FormatRoomTopic(roomName samvaad.RoomName) RoomTopic {
...
func (f topicFormatter) ParticipantTopic(ctx context.Context, roomName samvaad.RoomName, identity samvaad.ParticipantIdentity) ParticipantTopic {
	return FormatParticipantTopic(roomName, identity)
}

func (f topicFormatter) RoomTopic(ctx context.Context, roomName samvaad.RoomName) RoomTopic {
	return FormatRoomTopic(roomName)
}

type TopicFormatter interface {
	ParticipantTopic(ctx context.Context, roomName samvaad.RoomName, identity samvaad.ParticipantIdentity) ParticipantTopic
	RoomTopic(ctx context.Context, roomName samvaad.RoomName) RoomTopic
}
...
type KeepalivePubSub interface {
	KeepaliveClient[samvaad.NodeID]
	KeepaliveServer[samvaad.NodeID]
}
...
func NewKeepalivePubSub(params ClientParams) (KeepalivePubSub, error) {
	client, err := NewKeepaliveClient[samvaad.NodeID](params.Args())
	if err != nil {
		return nil, err
	}
	server := must.Get(NewKeepaliveServer[samvaad.NodeID](nil, params.Bus))
