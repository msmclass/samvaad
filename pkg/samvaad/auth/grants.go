...
type RoomConfiguration samvaad.RoomConfiguration
...
func (c *RoomConfiguration) Clone() *RoomConfiguration {
	...
	return (*RoomConfiguration)(utils.CloneProto((*samvaad.RoomConfiguration)(c)))
}

func (c *RoomConfiguration) MarshalJSON() ([]byte, error) {
	return protojson.Marshal((*samvaad.RoomConfiguration)(c))
}

func (c *RoomConfiguration) UnmarshalJSON(data []byte) error {
	return protojson.Unmarshal(data, (*samvaad.RoomConfiguration)(c))
}
...
func checkOutputForCredentials(output any) error {
	...
	case *samvaad.EncodedFileOutput_S3:
		if msg.S3.Secret != "" {
			return ErrSensitiveCredentials
		}
	case *samvaad.SegmentedFileOutput_S3:
		if msg.S3.Secret != "" {
			return ErrSensitiveCredentials
		}
	...
	case *samvaad.EncodedFileOutput_Gcp:
		if msg.Gcp.Credentials != "" {
			return ErrSensitiveCredentials
		}
	case *samvaad.SegmentedFileOutput_Gcp:
		if msg.Gcp.Credentials != "" {
			return ErrSensitiveCredentials
		}
	...
	case *samvaad.EncodedFileOutput_Azure:
		if msg.Azure.AccountKey != "" {
			return ErrSensitiveCredentials
		}
	case *samvaad.SegmentedFileOutput_Azure:
		if msg.Azure.AccountKey != "" {
			return ErrSensitiveCredentials
		}
	...
	case *samvaad.EncodedFileOutput_AliOSS:
		if msg.AliOSS.Secret != "" {
			return ErrSensitiveCredentials
		}
	case *samvaad.SegmentedFileOutput_AliOSS:
		if msg.AliOSS.Secret != "" {
			return ErrSensitiveCredentials
		}
	case *samvaad.AutoTrackEgress_AliOSS:
		if msg.AliOSS.Secret != "" {
			return ErrSensitiveCredentials
		}
	}
...
func (c *ClaimGrants) SetParticipantKind(kind samvaad.ParticipantInfo_Kind) {
	c.Kind = kindFromProto(kind)
}

func (c *ClaimGrants) GetParticipantKind() samvaad.ParticipantInfo_Kind {
	return kindToProto(c.Kind)
}

func (c *ClaimGrants) SetKindDetail(details ...samvaad.ParticipantInfo_KindDetail) {
	c.KindDetails = kindDetailsFromProto(details)
}

func (c *ClaimGrants) GetKindDetails() []samvaad.ParticipantInfo_KindDetail {
	return kindDetailsToProto(c.KindDetails)
}

func (c *ClaimGrants) GetRoomConfiguration() *samvaad.RoomConfiguration {
	if c.RoomConfig == nil {
		return nil
	}
	return (*samvaad.RoomConfiguration)(c.RoomConfig)
}
...
func (c *ClaimGrants) MarshalLogObject(e zapcore.ObjectEncoder) error {
	...
	e.AddObject("RoomConfig", logger.Proto((*samvaad.RoomConfiguration)(c.RoomConfig)))
...
func (v *VideoGrant) SetCanPublishSources(sources []samvaad.TrackSource) {
...
func (v *VideoGrant) GetCanPublishSource(source samvaad.TrackSource) bool {
	if !v.GetCanPublish() {
		return false
	}
...
func (v *VideoGrant) GetCanPublishSources() []samvaad.TrackSource {
	if len(v.CanPublishSources) == 0 {
		return nil
	}

	sources := make([]samvaad.TrackSource, 0, len(v.CanPublishSources))
	for _, s := range v.CanPublishSources {
...
func (v *VideoGrant) MatchesPermission(permission *samvaad.ParticipantPermission) bool {
	if permission == nil {
		return false
	}
...
func (v *VideoGrant) UpdateFromPermission(permission *samvaad.ParticipantPermission) {
	if permission == nil {
		return
	}
...
func (v *VideoGrant) ToPermission() *samvaad.ParticipantPermission {
	return &samvaad.ParticipantPermission{
...
func sourceToString(source samvaad.TrackSource) string {
	return strings.ToLower(source.String())
}

func sourceToProto(sourceStr string) samvaad.TrackSource {
	if val, ok := samvaad.TrackSource_value[strings.ToUpper(sourceStr)]; ok {
		return samvaad.TrackSource(val)
	}
	return samvaad.TrackSource_UNKNOWN
}

func kindFromProto(source samvaad.ParticipantInfo_Kind) string {
	return strings.ToLower(source.String())
}

func kindToProto(sourceStr string) samvaad.ParticipantInfo_Kind {
	if val, ok := samvaad.ParticipantInfo_Kind_value[strings.ToUpper(sourceStr)]; ok {
		return samvaad.ParticipantInfo_Kind(val)
	}
	return samvaad.ParticipantInfo_STANDARD
}

func kindDetailsFromProto(details []samvaad.ParticipantInfo_KindDetail) []string {
...
func kindDetailsToProto(details []string) []samvaad.ParticipantInfo_KindDetail {
	result := make([]samvaad.ParticipantInfo_KindDetail, 0, len(details))
	for _, d := range details {
		if val, ok := samvaad.ParticipantInfo_KindDetail_value[strings.ToUpper(d)]; ok {
			result = append(result, samvaad.ParticipantInfo_KindDetail(val))
		}
	}
	return result
}
