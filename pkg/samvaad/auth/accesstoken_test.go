...
func TestAccessToken(t *testing.T) {
	...
	t.Run("generates a decode-able key", func(t *testing.T) {
		...
		require.EqualValues(t, samvaad.ParticipantInfo_AGENT, decodedGrant.GetParticipantKind())
	...
	t.Run("missing kind should be interpreted as standard", func(t *testing.T) {
		...
		// default validity
		require.EqualValues(t, samvaad.ParticipantInfo_STANDARD, decodedGrant.GetParticipantKind())
	})
	...
	t.Run("room configuration serialization and deserialization", func(t *testing.T) {
		apiKey, secret := apiKeypair()
		roomConfig := &samvaad.RoomConfiguration{
		...
		roomDecoded := (*samvaad.RoomConfiguration)(decodedGrant.RoomConfig)
		...
		t.Run("room configuration blocks sensitive credentials by default", func(t *testing.T) {
			...
			roomConfig := &samvaad.RoomConfiguration{
				Egress: &samvaad.RoomEgress{
					Room: &samvaad.RoomCompositeEgressRequest{
						FileOutputs: []*samvaad.EncodedFileOutput{{
							Output: &samvaad.EncodedFileOutput_S3{S3: &samvaad.S3Upload{Secret: "super-secret"}},
						}},
					},
				},
			}
		...
		t.Run("room configuration allows sensitive credentials when enabled", func(t *testing.T) {
			...
			rc := (*samvaad.RoomConfiguration)(claims.RoomConfig)
			require.NotNil(t, rc)
			require.NotNil(t, rc.Egress)
			require.NotNil(t, rc.Egress.Room)
			require.NotEmpty(t, rc.Egress.Room.FileOutputs)
			s3Out, ok := rc.Egress.Room.FileOutputs[0].Output.(*samvaad.EncodedFileOutput_S3)
			require.True(t, ok)
