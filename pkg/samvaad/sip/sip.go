...
func DispatchRulePriority(info *samvaad.SIPDispatchRuleInfo) int32 {
	...
	default:
		return last
	case *samvaad.SIPDispatchRule_DispatchRuleDirect:
		if rule.DispatchRuleDirect.GetPin() != "" {
			priority = 0
		} else {
			priority = 100
		}
	case *samvaad.SIPDispatchRule_DispatchRuleIndividual:
		if rule.DispatchRuleIndividual.GetPin() != "" {
			priority = 1
		} else {
			priority = 101
		}
	case *samvaad.SIPDispatchRule_DispatchRuleCallee:
...
func hasHigherPriority(r1, r2 *samvaad.SIPDispatchRuleInfo) bool {
...
func SortDispatchRules(rules []*samvaad.SIPDispatchRuleInfo) {
...
func ValidateDispatchRules(rules []*samvaad.SIPDispatchRuleInfo, opts ...MatchDispatchRuleOpt) error {
...
func ValidateDispatchRulesIter(it iters.Iter[*samvaad.SIPDispatchRuleInfo], opts ...MatchDispatchRuleOpt) (best *samvaad.SIPDispatchRuleInfo, _ error) {
...
func NewDispatchRuleValidator(opts ...MatchDispatchRuleOpt) *DispatchRuleValidator {
	...
	return &DispatchRuleValidator{
		opt:       opt,
		byRuleKey: make(map[dispatchRuleKey]*samvaad.SIPDispatchRuleInfo),
	}
}
...
type DispatchRuleValidator struct {
	...
	byRuleKey map[dispatchRuleKey]*samvaad.SIPDispatchRuleInfo
}

func (v *DispatchRuleValidator) ValidateIter(it iters.Iter[*samvaad.SIPDispatchRuleInfo]) iters.Iter[*samvaad.SIPDispatchRuleInfo] {
	return &dispatchRuleValidatorIter{v: v, it: it}
}

func (v *DispatchRuleValidator) Validate(r *samvaad.SIPDispatchRuleInfo) error {
...
type dispatchRuleValidatorIter struct {
	...
	it iters.Iter[*samvaad.SIPDispatchRuleInfo]
}

func (v *dispatchRuleValidatorIter) Next() (*samvaad.SIPDispatchRuleInfo, error) {
	r, err := v.it.Next()
	if err != nil {
		return nil, err
	}
...
func SelectDispatchRule(rules []*samvaad.SIPDispatchRuleInfo, req *rpc.EvaluateSIPDispatchRulesRequest, opts ...MatchDispatchRuleOpt) (*samvaad.SIPDispatchRuleInfo, error) {
...
func GetPinAndRoom(info *samvaad.SIPDispatchRuleInfo) (room, pin string, err error) {
...
// This function delegates to samvaad.NormalizeNumber for the actual implementation.
func NormalizeNumber(num string) string {
	return samvaad.NormalizeNumber(num)
}

func validateTrunkInbound(byInbound map[string]*samvaad.SIPInboundTrunkInfo, t *samvaad.SIPInboundTrunkInfo, opt *matchTrunkOpts) error {
...
func ValidateTrunks(trunks []*samvaad.SIPInboundTrunkInfo, opts ...MatchTrunkOpt) error {
	return ValidateTrunksIter(iters.Slice(trunks), opts...)
}

// ValidateTrunksIter checks a set of trunks for conflicts.
func ValidateTrunksIter(it iters.Iter[*samvaad.SIPInboundTrunkInfo], opts ...MatchTrunkOpt) error {
	defer it.Close()
	...
	opt.defaults()
	byOutboundAndInbound := make(map[string]map[string]*samvaad.SIPInboundTrunkInfo)
	for {
		t, err := it.Next()
		if err == io.EOF {
			break
		...
		if len(t.Numbers) == 0 {
			...
			if byInbound == nil {
				byInbound = make(map[string]*samvaad.SIPInboundTrunkInfo)
...
type TrunkMatchResult struct {
	...
	Trunk *samvaad.SIPInboundTrunkInfo
...
func MatchTrunk(trunks []*samvaad.SIPInboundTrunkInfo, call *rpc.SIPCall, opts ...MatchTrunkOpt) (*samvaad.SIPInboundTrunkInfo, error) {
...
func MatchTrunkDetailed(it iters.Iter[*samvaad.SIPInboundTrunkInfo], call *rpc.SIPCall, opts ...MatchTrunkOpt) (*TrunkMatchResult, error) {
	defer it.Close()
	var opt matchTrunkOpts
	for _, fnc := range opts {
		fnc(&opt)
	...
	var (
		selectedTrunk    *samvaad.SIPInboundTrunkInfo
		defaultTrunk     *samvaad.SIPInboundTrunkInfo
		defaultTrunkPrev *samvaad.SIPInboundTrunkInfo
		sawAnyTrunk      bool
	)
...
func (opt *matchTrunkOpts) defaults() {
	if opt.Filtered == nil {
		opt.Filtered = func(_ *samvaad.SIPInboundTrunkInfo, _ TrunkFilteredReason) bool {
			return false
		}
	}
	if opt.Conflict == nil {
		opt.Conflict = func(_, _ *samvaad.SIPInboundTrunkInfo, _ TrunkConflictReason) {}
	}
	if opt.Replace == nil {
		opt.Replace = func(t *samvaad.SIPInboundTrunkInfo) *samvaad.SIPInboundTrunkInfo {
			return t
		}
	}
}
...
type TrunkFilteredFunc func(tr *samvaad.SIPInboundTrunkInfo, reason TrunkFilteredReason) bool
...
type TrunkConflictFunc func(t1, t2 *samvaad.SIPInboundTrunkInfo, reason TrunkConflictReason)
...
type TrunkReplaceFunc func(t *samvaad.SIPInboundTrunkInfo) *samvaad.SIPInboundTrunkInfo
...
// Returns nil if no rules matched or an error if there are conflicting definitions.
func MatchTrunkIter(it iters.Iter[*samvaad.SIPInboundTrunkInfo], call *rpc.SIPCall, opts ...MatchTrunkOpt) (*samvaad.SIPInboundTrunkInfo, error) {
...
//
// Deprecated: use MatchDispatchRuleIter
func MatchDispatchRule(trunk *samvaad.SIPInboundTrunkInfo, rules []*samvaad.SIPDispatchRuleInfo, req *rpc.EvaluateSIPDispatchRulesRequest, opts ...MatchDispatchRuleOpt) (*samvaad.SIPDispatchRuleInfo, error) {
...
func (opt *matchDispatchRuleOpts) defaults() {
	if opt.Conflict == nil {
		opt.Conflict = func(_, _ *samvaad.SIPDispatchRuleInfo, _ DispatchRuleConflictReason) {}
	}
	if opt.Replace == nil {
		opt.Replace = func(r *samvaad.SIPDispatchRuleInfo) *samvaad.SIPDispatchRuleInfo {
			return r
		}
	}
}
...
type DispatchRuleConflictFunc func(r1, r2 *samvaad.SIPDispatchRuleInfo, reason DispatchRuleConflictReason)
...
type DispatchRuleReplaceFunc func(r *samvaad.SIPDispatchRuleInfo) *samvaad.SIPDispatchRuleInfo
...
// Trunk parameter can be nil, in which case only wildcard dispatch rules will be effective (ones without Trunk IDs).
func MatchDispatchRuleIter(trunk *samvaad.SIPInboundTrunkInfo, rules iters.Iter[*samvaad.SIPDispatchRuleInfo], req *rpc.EvaluateSIPDispatchRulesRequest, opts ...MatchDispatchRuleOpt) (*samvaad.SIPDispatchRuleInfo, error) {
	...
	var (
		specificRule    *samvaad.SIPDispatchRuleInfo
		specificRuleCnt int
		defaultRule     *samvaad.SIPDispatchRuleInfo
		defaultRuleCnt  int
	)
	noPin := req.NoPin
	sentPin := req.GetPin()
...
func InboundTrunkAuthPrompt(trunk *samvaad.SIPInboundTrunkInfo) (*rpc.GetSIPTrunkAuthenticationResponse, error) {
	return &rpc.GetSIPTrunkAuthenticationResponse{
		...
		ProviderInfo: &samvaad.ProviderInfo{
			Id:   trunk.SipTrunkId,
			Name: trunk.Name,
			Type: samvaad.ProviderType_PROVIDER_TYPE_EXTERNAL,
		},
	}, nil
}

// EvaluateDispatchRule checks a selected Dispatch Rule against the provided request.
func EvaluateDispatchRule(projectID string, trunk *samvaad.SIPInboundTrunkInfo, rule *samvaad.SIPDispatchRuleInfo, req *rpc.EvaluateSIPDispatchRulesRequest) (*rpc.EvaluateSIPDispatchRulesResponse, error) {
	rule.Upgrade()
	call := req.SIPCall()
	sentPin := req.GetPin()

	trunkID := req.SipTrunkId
	enc := samvaad.SIPMediaEncryption_SIP_MEDIA_ENCRYPT_DISABLE
	if trunk != nil {
	...
	attrs[samvaad.AttrSIPCallID] = call.LkCallId
	...
	if rule.HidePhoneNumber {
		...
		attrs[samvaad.AttrSIPPhoneNumber] = call.From.User
		attrs[samvaad.AttrSIPHostName] = call.From.Host
		attrs[samvaad.AttrSIPTrunkNumber] = call.To.User
	}
	...
	attrs[samvaad.AttrSIPDispatchRuleID] = rule.GetSipDispatchRuleId()
	...
	if krispEnabled {
		resp.EnabledFeatures = append(resp.EnabledFeatures, samvaad.SIPFeature_KRISP_ENABLED)
	}
	return resp, nil
}
