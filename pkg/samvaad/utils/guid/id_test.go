...
func TestMarshalUnmarshal(t *testing.T) {
	id0 := samvaad.TrackID(New(TrackPrefix))
	b0 := Marshal(id0)
	id1 := Unmarshal[samvaad.TrackID](b0)
	b1 := Marshal(id1)
...
func TestMarshalAppend(t *testing.T) {
	id0 := samvaad.RoomID(New(RoomPrefix))
	id1 := samvaad.RoomID(New(RoomPrefix))
	var b []byte
	b = MarshalAppend(b, id0)
	b = MarshalAppend(b, id1)
	require.Equal(t, id0, Unmarshal[samvaad.RoomID](samvaad.GuidBlock(b[0:])))
	require.Equal(t, id1, Unmarshal[samvaad.RoomID](samvaad.GuidBlock(b[9:])))
}
