...
func guidPrefix[T samvaad.Guid]() string {
	var id T
	switch any(id).(type) {
	case samvaad.TrackID:
		return TrackPrefix
	case samvaad.ParticipantID:
...
func Marshal[T samvaad.Guid](id T) samvaad.GuidBlock {
	return samvaad.GuidBlock(MarshalAppend(nil, id))
}

func MarshalAppend[T samvaad.Guid](b []byte, id T) []byte {
	off := len(b)
...
func Unmarshal[T samvaad.Guid](b samvaad.GuidBlock) T {
	prefix := guidPrefix[T]()
