...
func TestWebHook(t *testing.T) {
	...
	t.Run("test event payload", func(t *testing.T) {
		...
		wg.Add(1)
		expectedUrl = "/wh"
		require.NoError(t, notifier.QueueNotify(context.Background(), event, WithExtraWebhooks([]*samvaad.WebhookConfig{&samvaad.WebhookConfig{Url: "http://localhost:8765/wh"}})))
		wg.Wait()
...
func TestURLNotifierDropped(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		_ = urlNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventRoomStarted})
		_ = urlNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventParticipantJoined})
		_ = urlNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventRoomFinished})
	}
...
func TestURLNotifierLifecycle(t *testing.T) {
	...
	t.Run("times out after accepting connection", func(t *testing.T) {
		...
		err := urlNotifier.send(&samvaad.WebhookEvent{Event: EventRoomStarted}, &urlNotifier.params)
		require.Error(t, err)
	})
...
func TestResourceURLNotifierDropped(t *testing.T) {
	...
	t.Run("depth drop", func(t *testing.T) {
		...
		// send multiple notifications
		for i := 0; i < 10; i++ {
			err := resourceURLNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventRoomStarted})
			if err == errQueueFull {
				totalDropped.Inc()
			}
			err = resourceURLNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventParticipantJoined})
			if err == errQueueFull {
				totalDropped.Inc()
			}
			err = resourceURLNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventRoomFinished})
			if err == errQueueFull {
				totalDropped.Inc()
			}
		}
	...
	t.Run("age drop", func(t *testing.T) {
		...
		// send multiple notifications
		for i := 0; i < 10; i++ {
			_ = resourceURLNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventRoomStarted})
			time.Sleep(time.Millisecond)
			_ = resourceURLNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventParticipantJoined})
			time.Sleep(time.Millisecond)
			_ = resourceURLNotifier.QueueNotify(context.Background(), &samvaad.WebhookEvent{Event: EventRoomFinished})
			time.Sleep(time.Millisecond)
		}
	...
	t.Run("resource queue timeout", func(t *testing.T) {
		...
		for i := 0; i < 3; i++ {
			...
			require.NotNil(t, rqi.resourceQueue)
			require.NotSame(t, rqi.resourceQueue, rq)
			rq = rqi.resourceQueue //nolint:staticcheck // used in next loop iteration
			time.Sleep(10 * time.Millisecond)
		}
...
func TestResourceURLNotifierLifecycle(t *testing.T) {
	...
	t.Run("times out after accepting connection", func(t *testing.T) {
		...
		err := resourceURLNotifier.send(&samvaad.WebhookEvent{Event: EventRoomStarted}, &params)
		require.Error(t, err)
	})
