...
type NotifyParams struct {
	ExtraWebhooks []*samvaad.WebhookConfig
	Secret        string
}

type NotifyOption func(*NotifyParams)

func WithExtraWebhooks(wh []*samvaad.WebhookConfig) NotifyOption {
...
type QueuedNotifier interface {
	RegisterProcessedHook(f func(ctx context.Context, whi *samvaad.WebhookInfo))
	SetKeys(apiKey, apiSecret string)
	SetFilter(params FilterParams)
	QueueNotify(ctx context.Context, event *samvaad.WebhookEvent, opts ...NotifyOption) error
	Stop(force bool)
}
...
func (n *DefaultNotifier) QueueNotify(ctx context.Context, event *samvaad.WebhookEvent, opts ...NotifyOption) error {
	for _, u := range n.notifiers {
	...
	for _, wh := range p.ExtraWebhooks {
		lopts := []NotifyOption{
			WithExtraWebhooks([]*samvaad.WebhookConfig{wh}),
		}
...
func (n *DefaultNotifier) RegisterProcessedHook(hook func(ctx context.Context, whi *samvaad.WebhookInfo)) {
	for _, u := range n.notifiers {
...
func EventKey(event *samvaad.WebhookEvent) string {
...
func logFields(event *samvaad.WebhookEvent, url string) []interface{} {
