...
type DownTrackSpreader[T sender] struct {
	...
	downTracks       map[samvaad.ParticipantID]T
	downTracksShadow []T
}
...
func NewDownTrackSpreader[T sender](params DownTrackSpreaderParams) *DownTrackSpreader[T] {
	d := &DownTrackSpreader[T]{
		params:     params,
		downTracks: make(map[samvaad.ParticipantID]T),
	}
...
func (d *DownTrackSpreader[T]) ResetAndGetDownTracks() []T {
	...
	d.downTracks = make(map[samvaad.ParticipantID]T)
...
func (d *DownTrackSpreader[T]) Free(subscriberID samvaad.ParticipantID) {
	d.downTrackMu.Lock()
	defer d.downTrackMu.Unlock()
...
func (d *DownTrackSpreader[T]) HasDownTrack(subscriberID samvaad.ParticipantID) bool {
	d.downTrackMu.RLock()
	defer d.downTrackMu.RUnlock()
