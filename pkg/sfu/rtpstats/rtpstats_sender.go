...
func (r *RTPStatsSender) MaybeAdjustFirstPacketTime(publisherSRData *samvaad.RTCPSenderReportState, tsOffset uint64) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *RTPStatsSender) GetRtcpSenderReport(ssrc uint32, publisherSRData *samvaad.RTCPSenderReportState, tsOffset uint64, passThrough bool) *rtcp.SenderReport {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *RTPStatsSender) ToProto() *samvaad.RTPStats {
	r.lock.RLock()
	defer r.lock.RUnlock()
