...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/transport"
	...
	"github.com/msmclass/samvaad/pkg/sfu/codecmunger"
	dd "github.com/msmclass/samvaad/pkg/sfu/rtpextension/dependencydescriptor"
	"github.com/msmclass/samvaad/pkg/sfu/rtpstats"
	sfuutils "github.com/msmclass/samvaad/pkg/sfu/utils"
	"github.com/msmclass/samvaad/pkg/sfu/videolayerselector"
	"github.com/msmclass/samvaad/pkg/sfu/videolayerselector/temporallayerselector"
)
...
type refInfo struct {
	senderReport    *samvaad.RTCPSenderReportState
	tsOffset        uint64
...
type Forwarder struct {
	...
	refVideoLayerMode        samvaad.VideoLayer_Mode
	isDDAvailable            bool
...
func (f *Forwarder) DetermineCodec(codec webrtc.RTPCodecCapability, extensions []webrtc.RTPHeaderExtensionParameter, videoLayerMode samvaad.VideoLayer_Mode) {
	f.lock.Lock()
	defer f.lock.Unlock()

	if videoLayerMode == samvaad.VideoLayer_ONE_SPATIAL_LAYER_PER_STREAM_INCOMPLETE_RTCP_SR {
...
func (f *Forwarder) GetState() *samvaad.RTPForwarderState {
	f.lock.RLock()
	...
	state := &samvaad.RTPForwarderState{
		Started:                   f.started,
	...
	if vp8MungerState, ok := codecMungerState.(*samvaad.VP8MungerState); ok {
		state.CodecMunger = &samvaad.RTPForwarderState_Vp8Munger{
			Vp8Munger: vp8MungerState,
		}
	}

	state.SenderReportState = make([]*samvaad.RTCPSenderReportState, len(f.refInfos))
...
func (f *Forwarder) SeedState(state *samvaad.RTPForwarderState) {
...
func (f *Forwarder) getRefLayer() (int32, int32) {
	...
	if f.refVideoLayerMode == samvaad.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM {
...
func (f *Forwarder) SetRefSenderReport(layer int32, srData *samvaad.RTCPSenderReportState) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *Forwarder) GetSenderReportParams() (int32, bool, uint64, *samvaad.RTCPSenderReportState) {
	f.lock.RLock()
	defer f.lock.RUnlock()
	...
	return currentLayerSpatial, f.refVideoLayerMode == samvaad.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM, f.refInfos[refLayer].tsOffset, f.refInfos[refLayer].senderReport
}
...
func (f *Forwarder) Restart() {
	...
	f.refVideoLayerMode = samvaad.VideoLayer_MODE_UNUSED
}
...
func (f *Forwarder) getRefLayerRTPTimestamp(ts uint32, refLayer, targetLayer int32) (uint32, error) {
	...
	if refLayer == targetLayer || f.refVideoLayerMode == samvaad.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM {
		return ts, nil
	}
