...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/transport/pkg/bucket"
	...
	"github.com/msmclass/samvaad/pkg/sfu/streamtracker"
...
type TrackReceiver interface {
	...
	Mime() mime.MimeType
	VideoLayerMode() samvaad.VideoLayer_Mode
	...
	DeleteDownTrack(participantID samvaad.ParticipantID)
...
type ReceiverBaseParams struct {
	...
	IsSelfClosing                bool
	OnNewBufferNeeded            func(int32, *samvaad.TrackInfo) (buffer.BufferProvider, error)
	OnClosed                     func()
}
...
func NewReceiverBase(params ReceiverBaseParams, trackInfo *samvaad.TrackInfo, codecState ReceiverCodecState) *ReceiverBase {
	r := &ReceiverBase{
		params:         params,
...
func (r *ReceiverBase) TrackInfo() *samvaad.TrackInfo {
	r.bufferMu.RLock()
...
func (r *ReceiverBase) UpdateTrackInfo(ti *samvaad.TrackInfo) {
	r.bufferMu.Lock()
...
func (r *ReceiverBase) TrackID() samvaad.TrackID {
...
func (r *ReceiverBase) VideoLayerMode() samvaad.VideoLayer_Mode {
...
func (r *ReceiverBase) DeleteDownTrack(subscriberID samvaad.ParticipantID) {
...
func (r *ReceiverBase) getBufferLocked(layer int32) (buffer.BufferProvider, int32) {
	...
	if r.videoLayerMode == samvaad.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM {
		layer = 0
	}
...
func (r *ReceiverBase) GetTrackStats() *samvaad.RTPStats {
	r.bufferMu.RLock()
	defer r.bufferMu.RUnlock()

	allStats := make([]*samvaad.RTPStats, 0, len(r.buffers))
	for _, buff := range r.buffers {
		if buff == nil {
			continue
		}
	...
	for r.forwardersGeneration.Load() == forwarderGeneration {
		...
		if r.Kind() == webrtc.RTPCodecTypeVideo {
			if spatialTrackers[spatialLayer] == nil {
				...
				if spatialTrackers[spatialLayer] == nil {
					if r.videoLayerMode == samvaad.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM && extPkt.DependencyDescriptor != nil {
