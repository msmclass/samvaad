...
var (
	qualityTransitionScore = map[samvaad.ConnectionQuality]float64{
		samvaad.ConnectionQuality_GOOD: 80,
		samvaad.ConnectionQuality_POOR: 40,
		samvaad.ConnectionQuality_LOST: 20,
	}
)
...
func (q *qualityScorer) updateMuteAtLocked(isMuted bool, at time.Time) {
	if isMuted {
		...
		if q.score != qualityTransitionScore[samvaad.ConnectionQuality_LOST] {
			q.score = cMaxScore
		}
	} else {
		q.unmutedAt = at
...
func (q *qualityScorer) updateAtLocked(stat *windowStat, at time.Time) {
	...
	if stat.packets+stat.packetsPadding == 0 {
		if !stat.lastRTCPAt.IsZero() && at.Sub(stat.lastRTCPAt) > stat.duration {
			reason = "rtcp"
			score = qualityTransitionScore[samvaad.ConnectionQuality_LOST]
		} else {
			reason = "dry"
			score = qualityTransitionScore[samvaad.ConnectionQuality_POOR]
		}
	} else {
...
func (q *qualityScorer) GetScoreAndQuality() (float32, samvaad.ConnectionQuality) {
	q.lock.RLock()
...
func (q *qualityScorer) GetMOSAndQuality() (float32, samvaad.ConnectionQuality) {
	q.lock.RLock()
	defer q.lock.RUnlock()
...
func scoreToConnectionQuality(score float64) samvaad.ConnectionQuality {
	// R-factor -> samvaad.ConnectionQuality scale mapping roughly based on
	// https://www.itu.int/ITU-T/2005-2008/com12/emodelv1/tut.htm
	//
	// As there are only three levels in samvaad.ConnectionQuality scale,
	...
	if score > qualityTransitionScore[samvaad.ConnectionQuality_GOOD] {
		return samvaad.ConnectionQuality_EXCELLENT
	}

	if score > qualityTransitionScore[samvaad.ConnectionQuality_POOR] {
		return samvaad.ConnectionQuality_GOOD
	}

	if score > qualityTransitionScore[samvaad.ConnectionQuality_LOST] {
		return samvaad.ConnectionQuality_POOR
	}

	return samvaad.ConnectionQuality_LOST
}
