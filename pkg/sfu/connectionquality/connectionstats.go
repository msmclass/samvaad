...
type ConnectionStats struct {
	...
	onStatsUpdate func(cs *ConnectionStats, stat *samvaad.AnalyticsStat)
...
func (cs *ConnectionStats) OnStatsUpdate(fn func(cs *ConnectionStats, stat *samvaad.AnalyticsStat)) {
	cs.onStatsUpdate = fn
}
...
func (cs *ConnectionStats) GetScoreAndQuality() (float32, samvaad.ConnectionQuality) {
...
func (cs *ConnectionStats) getStat() {
	...
	if cs.onStatsUpdate != nil && len(streams) != 0 {
		analyticsStreams := make([]*samvaad.AnalyticsStream, 0, len(streams))
		...
		if len(analyticsStreams) != 0 {
			cs.onStatsUpdate(cs, &samvaad.AnalyticsStat{
				Score:   score,
...
func toAnalyticsStream(
	...
	return &samvaad.AnalyticsStream{
...
func toAnalyticsVideoLayer(layer int32, layerStats *rtpstats.RTPDeltaInfo) *samvaad.AnalyticsVideoLayer {
	if layerStats == nil {
		return nil
	}

	avl := &samvaad.AnalyticsVideoLayer{
		Layer:   layer,
