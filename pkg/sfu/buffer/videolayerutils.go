...
func LayerPresenceFromTrackInfo(mimeType mime.MimeType, trackInfo *samvaad.TrackInfo) *[samvaad.VideoQuality_HIGH + 1]bool {
	if trackInfo == nil {
		return nil
	}
	...
	var layerPresence [samvaad.VideoQuality_HIGH + 1]bool
	for _, layer := range layers {
		// WARNING: comparing protobuf enum
		if layer.Quality <= samvaad.VideoQuality_HIGH {
...
func RidToSpatialLayer(mimeType mime.MimeType, rid string, trackInfo *samvaad.TrackInfo, ridSpace VideoLayersRid) int32 {
	...
	case ridSpace[0]:
		switch {
		case lp[samvaad.VideoQuality_LOW] && lp[samvaad.VideoQuality_MEDIUM] && lp[samvaad.VideoQuality_HIGH]:
			fallthrough
		case lp[samvaad.VideoQuality_LOW] && lp[samvaad.VideoQuality_MEDIUM]:
			fallthrough
		case lp[samvaad.VideoQuality_LOW] && lp[samvaad.VideoQuality_HIGH]:
			fallthrough
		case lp[samvaad.VideoQuality_MEDIUM] && lp[samvaad.VideoQuality_HIGH]:
			return 0
...
func SpatialLayerToRid(mimeType mime.MimeType, layer int32, trackInfo *samvaad.TrackInfo, ridSpace VideoLayersRid) string {
	lp := LayerPresenceFromTrackInfo(mimeType, trackInfo)
	if lp == nil {
		switch layer {
		case 0:
...
func VideoQualityToRid(mimeType mime.MimeType, quality samvaad.VideoQuality, trackInfo *samvaad.TrackInfo, ridSpace VideoLayersRid) string {
	return SpatialLayerToRid(mimeType, VideoQualityToSpatialLayer(mimeType, quality, trackInfo), trackInfo, ridSpace)
}

func SpatialLayerToVideoQuality(mimeType mime.MimeType, layer int32, trackInfo *samvaad.TrackInfo) samvaad.VideoQuality {
	lp := LayerPresenceFromTrackInfo(mimeType, trackInfo)
	if lp == nil {
		switch layer {
		case 0:
			return samvaad.VideoQuality_LOW
		case 1:
			return samvaad.VideoQuality_MEDIUM
		case 2:
			return samvaad.VideoQuality_HIGH
		default:
			return samvaad.VideoQuality_OFF
		}
	}
	...
	case 0:
		...
		case lp[samvaad.VideoQuality_LOW] && lp[samvaad.VideoQuality_HIGH]:
			fallthrough
		case lp[samvaad.VideoQuality_LOW]:
			return samvaad.VideoQuality_LOW

		case lp[samvaad.VideoQuality_MEDIUM] && lp[samvaad.VideoQuality_HIGH]:
			fallthrough
		case lp[samvaad.VideoQuality_MEDIUM]:
			return samvaad.VideoQuality_MEDIUM
...
func VideoQualityToSpatialLayer(mimeType mime.MimeType, quality samvaad.VideoQuality, trackInfo *samvaad.TrackInfo) int32 {
	lp := LayerPresenceFromTrackInfo(mimeType, trackInfo)
	if lp == nil {
		switch quality {
		case samvaad.VideoQuality_LOW:
			return 0
		case samvaad.VideoQuality_MEDIUM:
			return 1
		case samvaad.VideoQuality_HIGH:
			return 2
		default:
...
func GetVideoLayerModeForMimeType(mimeType mime.MimeType, ti *samvaad.TrackInfo) samvaad.VideoLayer_Mode {
	if ti != nil {
	...
	return samvaad.VideoLayer_MODE_UNUSED
}

func GetVideoLayersForMimeType(mimeType mime.MimeType, ti *samvaad.TrackInfo) []*samvaad.VideoLayer {
	var layers []*samvaad.VideoLayer
	if ti != nil {
		for _, codec := range ti.Codecs {
			if mime.NormalizeMimeType(codec.MimeType) == mimeType {
				layers = codec.Layers
...
func GetSpatialLayerForRid(mimeType mime.MimeType, rid string, ti *samvaad.TrackInfo) int32 {
	if ti == nil {
		return InvalidLayerSpatial
	}
...
func GetSpatialLayerForVideoQuality(mimeType mime.MimeType, quality samvaad.VideoQuality, ti *samvaad.TrackInfo) int32 {
	if ti == nil || quality == samvaad.VideoQuality_OFF {
		return InvalidLayerSpatial
	}
...
func GetVideoQualityForSpatialLayer(mimeType mime.MimeType, spatialLayer int32, ti *samvaad.TrackInfo) samvaad.VideoQuality {
