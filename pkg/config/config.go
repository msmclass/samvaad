...
import (
	...
	redisSamvaad "github.com/msmclass/samvaad/pkg/samvaad/redis"
	...
	"github.com/msmclass/samvaad/pkg/sfu/bwe/remotebwe"
	"github.com/msmclass/samvaad/pkg/sfu/bwe/sendsidebwe"
...
type Config struct {
	...
	Redis          redisSamvaad.RedisConfig `yaml:"redis,omitempty"`
	...
	// LowResourceMode enables aggressive GC tuning and reduced buffer sizes
	// suitable for constrained sovereign infrastructure (512 MB / 1 vCPU nodes).
	// Enable via: SAMVAAD_LOW_RESOURCE_MODE=true or low_resource_mode: true in config.
	LowResourceMode bool            `yaml:"low_resource_mode,omitempty"`
	LowResource     LowResourceConfig `yaml:"low_resource,omitempty"`
}
...
type RoomConfig struct {
	...
	RoomConfigurations           map[string]*samvaad.RoomConfiguration `yaml:"room_configurations,omitempty"`
}
...
// LowResourceConfig holds tuning knobs activated when LowResourceMode is true.
// These defaults are designed for a 512 MB RAM / 1 vCPU sovereign edge node.
type LowResourceConfig struct {
	// MemLimitMiB is the hard memory limit passed to runtime/debug.SetMemoryLimit.
	// Defaults to 400 MiB. Set to 0 to disable the limit.
	MemLimitMiB int `yaml:"mem_limit_mib,omitempty"`

	// GCPercent is the GC target percentage passed to runtime/debug.SetGCPercent.
	// Defaults to 20 (aggressive collection). Set to -1 to disable GC tuning.
	GCPercent int `yaml:"gc_percent,omitempty"`

	// PacketBufferSizeVideo overrides rtc.packet_buffer_size_video when in low
	// resource mode. Defaults to 100 packets (vs 500 in standard mode).
	PacketBufferSizeVideo int `yaml:"packet_buffer_size_video,omitempty"`

	// PacketBufferSizeAudio overrides rtc.packet_buffer_size_audio when in low
	// resource mode. Defaults to 50 packets (vs 200 in standard mode).
	PacketBufferSizeAudio int `yaml:"packet_buffer_size_audio,omitempty"`

	// DisablePrometheus halts background Prometheus aggregation in low-resource
	// mode to reclaim CPU cycles. Prometheus port is still opened but metric
	// collection goroutines are suspended.
	DisablePrometheus bool `yaml:"disable_prometheus,omitempty"`
}

// DefaultLowResourceConfig holds sensible defaults for constrained nodes.
var DefaultLowResourceConfig = LowResourceConfig{
	MemLimitMiB:           400,
	GCPercent:             20,
	PacketBufferSizeVideo: 100,
	PacketBufferSizeAudio: 50,
...
var DefaultConfig = Config{
	...
	Redis: redisSamvaad.RedisConfig{},
	Room: RoomConfig{
		AutoCreate: true,
	...
	LowResource:      DefaultLowResourceConfig,
}
...
func GenerateCLIFlags(existingFlags []cli.Flag, hidden bool) ([]cli.Flag, error) {
	...
	for name, value := range (defaultConfig).ToCLIFlagNames(existingFlags) {
		...
		var flag cli.Flag
		envVar := fmt.Sprintf("SAMVAAD_%s", strings.ToUpper(strings.ReplaceAll(name, ".", "_")))
...
func SetLogger(l logger.Logger) {
	logger.SetLogger(l, "samvaad")
}

func InitLoggerFromConfig(config *LoggingConfig) {
	logger.InitFromConfig(&config.Config, "samvaad")
}
