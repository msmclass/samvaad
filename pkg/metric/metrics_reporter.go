...
type MetricsReporterConsumer interface {
	MetricsReporterBatchReady(mb *samvaad.MetricsBatch)
}
...
func (mr *MetricsReporter) Merge(other *samvaad.MetricsBatch) {
	if mr == nil {
		return
	}
...
func (mr *MetricsReporter) getMetricsBatchAndReset() *samvaad.MetricsBatch {
	mr.lock.Lock()
	mbb := mr.mbb
