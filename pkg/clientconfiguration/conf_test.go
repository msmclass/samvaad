...
func TestScriptMatchConfiguration(t *testing.T) {
	t.Run("no merge", func(t *testing.T) {
		confs := []ConfigurationItem{
			{
				...
				Configuration: &samvaad.ClientConfiguration{
					ResumeConnection: samvaad.ClientConfigSetting_ENABLED,
				},
			},
		}
		...
		conf := cm.GetConfiguration(&samvaad.ClientInfo{Protocol: 4})
		require.Nil(t, conf)

		conf = cm.GetConfiguration(&samvaad.ClientInfo{Protocol: 6, Browser: "firefox"})
		require.Nil(t, conf)

		conf = cm.GetConfiguration(&samvaad.ClientInfo{Protocol: 6, Browser: "chrome"})
		require.Equal(t, conf.ResumeConnection, samvaad.ClientConfigSetting_ENABLED)
	})
	...
	t.Run("merge", func(t *testing.T) {
		confs := []ConfigurationItem{
			...
			},
			{
				...
				Configuration: &samvaad.ClientConfiguration{
					Video: &samvaad.VideoConfiguration{
						HardwareEncoder: samvaad.ClientConfigSetting_DISABLED,
					},
				},
				Merge: true,
			},
		...
		conf = cm.GetConfiguration(&samvaad.ClientInfo{Protocol: 6, Browser: "chrome", Sdk: 3})
		require.Equal(t, conf.ResumeConnection, samvaad.ClientConfigSetting_ENABLED)
		require.Equal(t, conf.Video.HardwareEncoder, samvaad.ClientConfigSetting_DISABLED)
	})
}
