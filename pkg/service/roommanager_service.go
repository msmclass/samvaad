...
func (s whipService) Create(ctx context.Context, req *rpc.WHIPCreateRequest) (*rpc.WHIPCreateResponse, error) {
	...
	if err = s.RoomManager.StartSession(
		ctx,
		*pi,
		routing.NewNullMessageSource(samvaad.ConnectionID(req.StartSession.ConnectionId)), // no requestSource
		routing.NewNullMessageSink(samvaad.ConnectionID(req.StartSession.ConnectionId)),   // no responseSink
	...
	room := s.RoomManager.GetRoom(ctx, samvaad.RoomName(req.StartSession.RoomName))
	if room == nil {
	...
	if err := lp.HandleOffer(&samvaad.SessionDescription{
	...
	for publisherIdentity, trackList := range req.SubscribedParticipantTracks {
		for _, trackName := range trackList.TrackNames {
			eg.Go(func() error {
				for {
					if lp.IsTrackNameSubscribed(samvaad.ParticipantIdentity(publisherIdentity), trackName) {
						return nil
					}
...
func getMediaStateForParticipant(participant types.Participant) (*samvaad.InputVideoState, *samvaad.InputAudioState) {
	pParticipant := participant.ToProto()

	var video *samvaad.InputVideoState
	var audio *samvaad.InputAudioState
	...
	for _, v := range pParticipant.Tracks {
		...
		if v.Type != samvaad.TrackType_VIDEO {
			continue
		}

		video = &samvaad.InputVideoState{}
	...
	for _, a := range pParticipant.Tracks {
		...
		if a.Type != samvaad.TrackType_AUDIO {
			continue
		}

		audio = &samvaad.InputAudioState{}
...
func (r whipParticipantService) ICETrickle(ctx context.Context, req *rpc.WHIPParticipantICETrickleRequest) (*emptypb.Empty, error) {
	room := r.RoomManager.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	lp := room.GetParticipantByID(samvaad.ParticipantID(req.ParticipantId))
	if lp == nil {
		return nil, ErrParticipantNotFound
	}
