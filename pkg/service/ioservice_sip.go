...
func (s *IOInfoService) matchSIPTrunk(ctx context.Context, trunkID string, call *rpc.SIPCall) (*samvaad.SIPInboundTrunkInfo, error) {
	if s.ss == nil {
	...
	if trunkID != "" {
		...
		if tr, err := s.ss.LoadSIPInboundTrunk(ctx, trunkID); err == nil {
			tr, err = sip.MatchTrunkIter(iters.Slice([]*samvaad.SIPInboundTrunkInfo{tr}), call)
			if err == nil {
				return tr, nil
			}
		}
...
func (s *IOInfoService) SelectSIPInboundTrunk(ctx context.Context, called string) iters.Iter[*samvaad.SIPInboundTrunkInfo] {
	it := samvaad.ListPageIter(s.ss.ListSIPInboundTrunk, &samvaad.ListSIPInboundTrunkRequest{
...
func (s *IOInfoService) matchSIPDispatchRule(ctx context.Context, trunk *samvaad.SIPInboundTrunkInfo, req *rpc.EvaluateSIPDispatchRulesRequest) (*samvaad.SIPDispatchRuleInfo, error) {
	if s.ss == nil {
		return nil, ErrSIPNotConnected
	}
	var trunkID string
...
func (s *IOInfoService) SelectSIPDispatchRule(ctx context.Context, trunkID string) iters.Iter[*samvaad.SIPDispatchRuleInfo] {
	var trunkIDs []string
	if trunkID != "" {
		trunkIDs = []string{trunkID}
	}
	it := samvaad.ListPageIter(s.ss.ListSIPDispatchRule, &samvaad.ListSIPDispatchRuleRequest{
		TrunkIds: trunkIDs,
	})
	return iters.PagesAsIter(ctx, it)
}
