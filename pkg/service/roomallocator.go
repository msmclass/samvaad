...
func (r *StandardRoomAllocator) CreateRoom(ctx context.Context, req *samvaad.CreateRoomRequest, isExplicit bool) (*samvaad.Room, *samvaad.RoomInternal, bool, error) {
	token, err := r.roomStore.LockRoom(ctx, samvaad.RoomName(req.Name), 5*time.Second)
	if err != nil {
		return nil, nil, false, err
	}
	defer func() {
		_ = r.roomStore.UnlockRoom(ctx, samvaad.RoomName(req.Name), token)
	}()

	// find existing room and update it
	var created bool
	rm, internal, err := r.roomStore.LoadRoom(ctx, samvaad.RoomName(req.Name), true)
	...
	if req.MinPlayoutDelay > 0 || req.MaxPlayoutDelay > 0 {
		internal.PlayoutDelay = &samvaad.PlayoutDelay{
			Enabled: true,
...
func (r *StandardRoomAllocator) SelectRoomNode(ctx context.Context, roomName samvaad.RoomName, nodeID samvaad.NodeID) error {
...
func (r *StandardRoomAllocator) ValidateCreateRoom(ctx context.Context, roomName samvaad.RoomName) error {
...
func applyDefaultRoomConfig(room *samvaad.Room, internal *samvaad.RoomInternal, conf *config.RoomConfig) {
	...
	for _, codec := range conf.EnabledCodecs {
		room.EnabledCodecs = append(room.EnabledCodecs, &samvaad.Codec{
			Mime:     codec.Mime,
...
func (r *StandardRoomAllocator) applyNamedRoomConfiguration(req *samvaad.CreateRoomRequest) (*samvaad.CreateRoomRequest, error) {
	if req.RoomPreset == "" {
		return req, nil
	}
	...
	if clone.Agents == nil {
		clone.Agents = make([]*samvaad.RoomAgentDispatch, 0, len(conf.Agents))
