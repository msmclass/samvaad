...
func EnsureJoinPermission(ctx context.Context) (name samvaad.RoomName, err error) {
	...
	if claims.Video.RoomJoin {
		name = samvaad.RoomName(claims.Video.Room)
	} else {
		err = ErrPermissionDenied
	}
	return
...
func EnsureAdminPermission(ctx context.Context, room samvaad.RoomName) error {
	claims := GetGrants(ctx)
	if claims == nil || claims.Video == nil {
		return ErrPermissionDenied
	}

	if !claims.Video.RoomAdmin || room != samvaad.RoomName(claims.Video.Room) {
		return ErrPermissionDenied
	}
...
func EnsureDestRoomPermission(ctx context.Context, source samvaad.RoomName, destination samvaad.RoomName) error {
	claims := GetGrants(ctx)
	if claims == nil || claims.Video == nil {
		return ErrPermissionDenied
	}

	if !claims.Video.RoomAdmin || source != samvaad.RoomName(claims.Video.Room) || destination != samvaad.RoomName(claims.Video.DestinationRoom) {
		return ErrPermissionDenied
	}
