...
func HandshakeAgentWorker(c agent.SignalConn, serverInfo *samvaad.ServerInfo, registration agent.WorkerRegistration, l logger.Logger) (r agent.WorkerRegistration, ok bool) {
...
type AgentHandler struct {
	...
	jobToWorker map[samvaad.JobID]*agent.Worker
...
func NewAgentHandler(
	...
	return &AgentHandler{
		...
		jobToWorker:      make(map[samvaad.JobID]*agent.Worker),
...
func (h *AgentHandler) deregisterJob(jobID samvaad.JobID) {
...
func (h *AgentHandler) JobRequest(ctx context.Context, job *samvaad.Job) (*rpc.JobRequestResponse, error) {
	...
	for {
		...
		case samvaad.JobStatus_JS_RUNNING:
			...
			h.mu.Lock()
			h.jobToWorker[samvaad.JobID(job.Id)] = selected
			h.mu.Unlock()
...
func (h *AgentHandler) JobRequestAffinity(ctx context.Context, job *samvaad.Job) float32 {
	h.mu.Lock()
	defer h.mu.Unlock()
	...
	for _, w := range h.workers {
		...
		if w.Status() == samvaad.WorkerStatus_WS_AVAILABLE {
...
func (h *AgentHandler) JobTerminate(ctx context.Context, req *rpc.JobTerminateRequest) (*rpc.JobTerminateResponse, error) {
	h.mu.Lock()
	w := h.jobToWorker[samvaad.JobID(req.JobId)]
	h.mu.Unlock()
	...
	state, err := w.TerminateJob(samvaad.JobID(req.JobId), req.Reason)
	if err != nil {
		return nil, err
	}
...
func (h *AgentHandler) selectWorkerWeightedByLoad(key workerKey, ignore map[*agent.Worker]struct{}) (*agent.Worker, error) {
	...
	for _, w := range workers {
		if _, ok := ignore[w]; !ok && w.Status() == samvaad.WorkerStatus_WS_AVAILABLE {
...
func (w *agentHandlerWorker) HandleUpdateJob(update *samvaad.UpdateJobStatus) error {
	...
	if agent.JobStatusIsEnded(update.Status) {
		w.h.mu.Lock()
		w.h.deregisterJob(samvaad.JobID(update.JobId))
		w.h.mu.Unlock()
	}
	return nil
}
