...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/psrpc/pkg/metadata"
...
func (s *defaultSessionHandler) HandleSession(
	...
	rtcNode, err := s.router.GetNodeForRoom(ctx, samvaad.RoomName(pi.CreateRoom.Name))
	if err != nil {
		return err
	}

	if samvaad.NodeID(rtcNode.Id) != s.currentNode.NodeID() {
...
func (r *signalService) RelaySignal(stream psrpc.ServerStream[*rpc.RelaySignalResponse, *rpc.RelaySignalRequest]) (err error) {
	...
	sink := routing.NewSignalMessageSink(routing.SignalSinkParams[*rpc.RelaySignalResponse, *rpc.RelaySignalRequest]{
		...
		ConnectionID: samvaad.ConnectionID(ss.ConnectionId),
	})
	reqChan := routing.NewDefaultMessageChannel(samvaad.ConnectionID(ss.ConnectionId))
	...
	err = r.sessionHandler.HandleSession(ctx, *pi, samvaad.ConnectionID(ss.ConnectionId), reqChan, sink)
	if err != nil {
		sink.Close()
...
func (e signalResponseMessageWriter) Write(seq uint64, close bool, msgs []proto.Message) *rpc.RelaySignalResponse {
	r := &rpc.RelaySignalResponse{
		Seq:       seq,
		Responses: make([]*samvaad.SignalResponse, 0, len(msgs)),
		Close:     close,
	}
	for _, m := range msgs {
		r.Responses = append(r.Responses, m.(*samvaad.SignalResponse))
	}
	return r
}
