...
func TestRoomInternal(t *testing.T) {
	...
	internal := &samvaad.RoomInternal{
		TrackEgress: &samvaad.AutoTrackEgress{Filepath: "egress"},
	}

	require.NoError(t, rs.StoreRoom(ctx, room, internal))
	actualRoom, actualInternal, err := rs.LoadRoom(ctx, samvaad.RoomName(room.Name), true)
	...
	_, actualInternal, err = rs.LoadRoom(ctx, samvaad.RoomName(room.Name), true)
	require.NoError(t, err)
...
func TestParticipantPersistence(t *testing.T) {
	...
	roomName := samvaad.RoomName("room1")
	_ = rs.DeleteRoom(ctx, roomName)

	p := &samvaad.ParticipantInfo{
		Sid:      "PA_test",
		Identity: "test",
	...
	// result should match
	pGet, err := rs.LoadParticipant(ctx, roomName, samvaad.ParticipantIdentity(p.Identity))
	require.NoError(t, err)
	...
	require.NoError(t, rs.DeleteParticipant(ctx, roomName, samvaad.ParticipantIdentity(p.Identity)))
	...
	_, err = rs.LoadParticipant(ctx, roomName, samvaad.ParticipantIdentity(p.Identity))
...
func TestRoomLock(t *testing.T) {
	...
	roomName := samvaad.RoomName("myroom")
...
func TestEgressStore(t *testing.T) {
	...
	info := &samvaad.EgressInfo{
		...
		RoomName: roomName,
		Status:   samvaad.EgressStatus_EGRESS_STARTING,
	...
	// update
	info2.Status = samvaad.EgressStatus_EGRESS_COMPLETE
	...
	// list by room
	list, err = rs.ListEgress(ctx, samvaad.RoomName(roomName), false)
	require.NoError(t, err)
	require.Len(t, list, 1)

	// update
	info.Status = samvaad.EgressStatus_EGRESS_COMPLETE
...
func TestAgentStore(t *testing.T) {
	...
	ad := &samvaad.AgentDispatch{
		Id:        "dispatch_id",
		AgentName: "agent_name",
		Metadata:  "metadata",
		Room:      "room_name",
		State: &samvaad.AgentDispatchState{
			CreatedAt: 1,
			DeletedAt: 2,
			Jobs: []*samvaad.Job{
				&samvaad.Job{
					...
					Metadata:  "metadata",
					AgentName: "agent_name",
					State: &samvaad.JobState{
						Status:    samvaad.JobStatus_JS_RUNNING,
						StartedAt: 3,
						EndedAt:   4,
						Error:     "error",
					},
	...
	expected.State.Jobs[0].Participant = &samvaad.ParticipantInfo{
		Identity: "identity",
	}
	require.True(t, proto.Equal(expected, rd[0]))
...
func compareIngressInfo(t *testing.T, expected, v *samvaad.IngressInfo) {
