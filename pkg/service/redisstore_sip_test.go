...
func TestSIPStoreDispatch(t *testing.T) {
	...
	list, err := rs.ListSIPDispatchRule(ctx, &samvaad.ListSIPDispatchRuleRequest{})
	...
	rule := &samvaad.SIPDispatchRuleInfo{
		TrunkIds: []string{"trunk"},
		Rule: &samvaad.SIPDispatchRule{Rule: &samvaad.SIPDispatchRule_DispatchRuleDirect{
...
func TestSIPStoreTrunk(t *testing.T) {
	...
	oldList, err := rs.ListSIPTrunk(ctx, &samvaad.ListSIPTrunkRequest{})
	require.NoError(t, err)
	...
	inList, err := rs.ListSIPInboundTrunk(ctx, &samvaad.ListSIPInboundTrunkRequest{})
	require.NoError(t, err)
	...
	outList, err := rs.ListSIPOutboundTrunk(ctx, &samvaad.ListSIPOutboundTrunkRequest{})
	require.NoError(t, err)
	...
	// Creation without ID should fail.
	oldT := &samvaad.SIPTrunkInfo{
		Name: "Legacy",
	}
	err = rs.StoreSIPTrunk(ctx, oldT)
	require.Error(t, err)

	inT := &samvaad.SIPInboundTrunkInfo{
		Name: "Inbound",
	}
	err = rs.StoreSIPInboundTrunk(ctx, inT)
	require.Error(t, err)

	outT := &samvaad.SIPOutboundTrunkInfo{
		Name: "Outbound",
	}
	...
	listOld, err := rs.ListSIPTrunk(ctx, &samvaad.ListSIPTrunkRequest{})
	require.NoError(t, err)
	require.Len(t, listOld.Items, 3)
	slices.SortFunc(listOld.Items, func(a, b *samvaad.SIPTrunkInfo) int {
	...
	listIn, err := rs.ListSIPInboundTrunk(ctx, &samvaad.ListSIPInboundTrunkRequest{})
	require.NoError(t, err)
	require.Len(t, listIn.Items, 2)
	slices.SortFunc(listIn.Items, func(a, b *samvaad.SIPInboundTrunkInfo) int {
		return strings.Compare(a.Name, b.Name)
	})
	require.True(t, proto.Equal(inT, listIn.Items[0]))
	require.True(t, proto.Equal(oldT.AsInbound(), listIn.Items[1]))

	listOut, err := rs.ListSIPOutboundTrunk(ctx, &samvaad.ListSIPOutboundTrunkRequest{})
	require.NoError(t, err)
	require.Len(t, listOut.Items, 2)
	slices.SortFunc(listOut.Items, func(a, b *samvaad.SIPOutboundTrunkInfo) int {
		return strings.Compare(a.Name, b.Name)
	})
...
func TestSIPTrunkList(t *testing.T) {
	...
	testIter(t, func(ctx context.Context, id string) error {
		if strings.HasSuffix(id, "0") {
			return s.StoreSIPTrunk(ctx, &samvaad.SIPTrunkInfo{
				SipTrunkId:     id,
				OutboundNumber: id,
			})
		}
		return s.StoreSIPInboundTrunk(ctx, &samvaad.SIPInboundTrunkInfo{
			SipTrunkId: id,
			Numbers:    []string{id},
		})
	}, func(ctx context.Context, page *samvaad.Pagination, ids []string) iters.PageIter[*samvaad.SIPInboundTrunkInfo] {
		return samvaad.ListPageIter(s.ListSIPInboundTrunk, &samvaad.ListSIPInboundTrunkRequest{
...
func TestSIPRuleList(t *testing.T) {
	...
	testIter(t, func(ctx context.Context, id string) error {
		return s.StoreSIPDispatchRule(ctx, &samvaad.SIPDispatchRuleInfo{
			SipDispatchRuleId: id,
			TrunkIds:          []string{id},
		})
	}, func(ctx context.Context, page *samvaad.Pagination, ids []string) iters.PageIter[*samvaad.SIPDispatchRuleInfo] {
		return samvaad.ListPageIter(s.ListSIPDispatchRule, &samvaad.ListSIPDispatchRuleRequest{
...
func testIter[T listItem](
	...
	list func(ctx context.Context, page *samvaad.Pagination, ids []string) iters.PageIter[T],
) {
	ctx := context.Background()
	var all []string
	for i := range 250 {
	...
	it = list(ctx, &samvaad.Pagination{Limit: 10}, nil)
	got = allIDs(t, it)
	require.Equal(t, all, got)

	// List with pagination enabled, custom ID
	it = list(ctx, &samvaad.Pagination{Limit: 10, AfterId: all[55]}, nil)
	got = allIDs(t, it)
	require.Equal(t, all[56:], got)

	// List fixed IDs
	it = list(ctx, &samvaad.Pagination{Limit: 10, AfterId: all[5]}, []string{
		all[10],
		all[3],
		"invalid",
		all[8],
