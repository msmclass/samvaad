...
type WHIPService struct {
	...
	client            rpc.WHIPClient[samvaad.NodeID]
...
func NewWHIPService(
	...
	client, err := rpc.NewWHIPClient[samvaad.NodeID](clientParams.Args())
	if err != nil {
		return nil, err
	}
...
func (s *WHIPService) validateCreate(w http.ResponseWriter, r *http.Request) (*createRequest, int, error) {
	...
	clientInfoHeader := r.Header.Get("X-Samvaad-ClientInfo")
	if clientInfoHeader != "" {
	...
	fromIngress := r.Header.Get("X-Samvaad-Ingress")
	...
	pi := routing.ParticipantInit{
		Identity:      samvaad.ParticipantIdentity(claims.Identity),
		Name:          samvaad.ParticipantName(claims.Name),
		AutoSubscribe: true,
		Client:        ci,
		Grants:        claims,
...
func (s *WHIPService) handleCreate(w http.ResponseWriter, r *http.Request) {
	...
	connID := samvaad.ConnectionID(guid.New("CO_"))
	...
	res, err := s.client.Create(r.Context(), samvaad.NodeID(rtcNode.Id), &rpc.WHIPCreateRequest{
...
func (s *WHIPService) handleParticipantPatch(w http.ResponseWriter, r *http.Request) {
	...
	pID := samvaad.ParticipantID(r.PathValue("participant_id"))
	if pID == "" {
	...
	if ifMatch == "*" {
		s.iceRestart(w, r, roomName, samvaad.ParticipantIdentity(claims.Identity), pID, sdpFragment)
	} else {
		s.iceTrickle(w, r, roomName, samvaad.ParticipantIdentity(claims.Identity), pID, ifMatch, sdpFragment)
	}
}
...
func (s *WHIPService) handleParticipantDelete(w http.ResponseWriter, r *http.Request) {
	...
	_, err = s.participantClient.DeleteSession(
		r.Context(),
		s.topicFormatter.ParticipantTopic(r.Context(), roomName, samvaad.ParticipantIdentity(claims.Identity)),
