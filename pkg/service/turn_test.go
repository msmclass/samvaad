...
func mustAuthCreds(t *testing.T, h *TURNAuthHandler, pID samvaad.ParticipantID, ttlSeconds int) (username string, key []byte) {
	t.Helper()
	...
	return username, turn.GenerateAuthKey(username, SamvaadRealm, password)
}

func TestTURNAuthHandler_HandleAuth_ValidCredentials(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_valid")
...
func TestTURNAuthHandler_HandleAuth_ExpiredAllocateRejected(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_expired_alloc")
...
func TestTURNAuthHandler_HandleAuth_ExpiredNonAllocateAllowed(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_expired_refresh")
	...
	require.NoError(t, err)
	expectedKey := turn.GenerateAuthKey(username, SamvaadRealm, password)
...
func TestTURNAuthHandler_HandleAuth_TwoPartUsernameRejected(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_two_part")
...
func TestTURNAuthHandler_HandleAuth_ZeroExpiryRejected(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_zero_expiry")
...
func TestTURNAuthHandler_ParseUsername_TwoPartRejected(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_parse_two_part")
...
func TestTURNAuthHandler_ParseUsername_ZeroExpiryRejected(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_parse_zero_expiry")
...
func TestTURNAuthHandler_CreatePassword_ZeroExpiryRejected(t *testing.T) {
	h := newTestTurnAuthHandler()
	pID := samvaad.ParticipantID("PA_password_zero_expiry")
