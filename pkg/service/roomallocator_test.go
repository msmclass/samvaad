...
func TestCreateRoom(t *testing.T) {
	t.Run("ensure default room settings are applied", func(t *testing.T) {
		...
		room, _, _, err := ra.CreateRoom(context.Background(), &samvaad.CreateRoomRequest{Name: "myroom"}, true)
		require.NoError(t, err)
...
func SelectRoomNode(t *testing.T) {
	t.Run("reject new participants when track limit has been reached", func(t *testing.T) {
		...
		node, err := routing.NewLocalNode(conf)
		require.NoError(t, err)
		node.SetStats(&samvaad.NodeStats{
			NumTracksIn:  100,
			NumTracksOut: 100,
		})
...
func newTestRoomAllocator(t *testing.T, conf *config.Config, node *samvaad.Node) (service.RoomAllocator, *config.Config) {
