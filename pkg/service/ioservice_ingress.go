...
func (s *IOInfoService) CreateIngress(ctx context.Context, info *samvaad.IngressInfo) (*emptypb.Empty, error) {
	if s.is == nil {
...
func (s *IOInfoService) loadIngressFromInfoRequest(req *rpc.GetIngressInfoRequest) (info *samvaad.IngressInfo, err error) {
	if s.is == nil {
		return nil, ErrIngressNotConnected
	}
...
func (s *IOInfoService) UpdateIngressState(ctx context.Context, req *rpc.UpdateIngressStateRequest) (*emptypb.Empty, error) {
	...
	if info.State.Status != req.State.Status {
		...
		case samvaad.IngressState_ENDPOINT_ERROR,
			samvaad.IngressState_ENDPOINT_INACTIVE,
			samvaad.IngressState_ENDPOINT_COMPLETE:
		...
		case samvaad.IngressState_ENDPOINT_PUBLISHING:
