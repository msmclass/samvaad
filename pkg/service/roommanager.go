...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/transport/pkg/rtcconfig"
...
type RoomManager struct {
	...
	whipServer        rpc.WHIPServer[samvaad.NodeID]
	...
	rooms map[samvaad.RoomName]*rtc.Room
...
func NewLocalRoomManager(
	...
	r := &RoomManager{
		...
		rooms: make(map[samvaad.RoomName]*rtc.Room),

		iceConfigCache: sutils.NewIceConfigCache[iceConfigCacheKey](0),

		serverInfo: &samvaad.ServerInfo{
			Edition:       samvaad.ServerInfo_Standard,
	...
	r.whipServer, err = rpc.NewWHIPServer[samvaad.NodeID](whipService, bus, rpc.WithDefaultServerOptions(conf.PSRPC, logger.GetLogger()))
	if err != nil {
		return nil, err
	}
...
func (r *RoomManager) GetRoom(_ context.Context, roomName samvaad.RoomName) *rtc.Room {
	r.lock.RLock()
...
func (r *RoomManager) deleteRoom(ctx context.Context, roomName samvaad.RoomName) error {
...
func (r *RoomManager) CreateRoom(ctx context.Context, req *samvaad.CreateRoomRequest) (*samvaad.Room, error) {
...
func (r *RoomManager) StartSession(
		...
		if pi.Reconnect {
			if participant.IsClosed() {
				...
				if pv.SupportsRegionsInLeaveRequest() {
					leave = &samvaad.LeaveRequest{
						Reason: samvaad.DisconnectReason_STATE_MISMATCH,
						Action: samvaad.LeaveRequest_RECONNECT,
					}
				} else {
					leave = &samvaad.LeaveRequest{
						CanReconnect: true,
				...
				_ = responseSink.WriteMessage(&samvaad.SignalResponse{
			...
			if err = room.ResumeParticipant(
				...
				r.iceServersForParticipant(
					apiKey,
					participant,
					iceConfig.PreferenceSubscriber == samvaad.ICECandidateType_ICT_TLS,
				),
				pi.ReconnectReason,
			); err != nil {
	...
	if !slices.ContainsFunc(enabledCodecs, func(codec *samvaad.Codec) bool {
		return mime.IsMimeTypeStringRTX(codec.Mime)
	}) {
		enabledCodecs = append(enabledCodecs, &samvaad.Codec{Mime: mime.MimeTypeRTX.String()})
	}
	...
	iceServers := r.iceServersForParticipant(apiKey, participant, iceConfig.PreferenceSubscriber == samvaad.ICECandidateType_ICT_TLS)
	...
	persistRoomForParticipantCount := func(proto *samvaad.Room) {
	...
	clientMeta := &samvaad.AnalyticsClientMeta{Region: r.currentNode.Region(), Node: string(r.currentNode.NodeID())}
	...
	participant.OnICEConfigChanged(func(participant types.LocalParticipant, iceConfig *samvaad.ICEConfig) {
...
func (r *RoomManager) getOrCreateRoom(ctx context.Context, createRoom *samvaad.CreateRoomRequest) (*rtc.Room, error) {
	roomName := samvaad.RoomName(createRoom.Name)
...
func (r *RoomManager) roomAndParticipantForReq(ctx context.Context, req participantReq) (*rtc.Room, types.LocalParticipant, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.GetRoom()))
	if room == nil {
		return nil, nil, ErrRoomNotFound
	}

	participant := room.GetParticipant(samvaad.ParticipantIdentity(req.GetIdentity()))
	if participant == nil {
...
func (r *RoomManager) ListParticipants(ctx context.Context, req *samvaad.ListParticipantsRequest) (*samvaad.ListParticipantsResponse, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	participants := room.GetParticipants()
	items := make([]*samvaad.ParticipantInfo, 0, len(participants))
	for _, p := range participants {
		items = append(items, p.ToProto())
	}

	return &samvaad.ListParticipantsResponse{
		Participants: items,
	}, nil
}

func (r *RoomManager) GetParticipant(ctx context.Context, req *samvaad.RoomParticipantIdentity) (*samvaad.ParticipantInfo, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	participant := room.GetParticipant(samvaad.ParticipantIdentity(req.Identity))
	if participant == nil {
...
func (r *RoomManager) RemoveParticipant(ctx context.Context, req *samvaad.RoomParticipantIdentity) (*samvaad.RemoveParticipantResponse, error) {
	...
	room.RemoveParticipant(samvaad.ParticipantIdentity(req.Identity), "", types.ParticipantCloseReasonServiceRequestRemoveParticipant)
	return &samvaad.RemoveParticipantResponse{}, nil
}

func (r *RoomManager) MutePublishedTrack(ctx context.Context, req *samvaad.MuteRoomTrackRequest) (*samvaad.MuteRoomTrackResponse, error) {
	...
	track := participant.SetTrackMuted(&samvaad.MuteTrackRequest{
		Sid:   req.TrackSid,
		Muted: req.Muted,
	}, true)
	return &samvaad.MuteRoomTrackResponse{Track: track}, nil
}

func (r *RoomManager) UpdateParticipant(ctx context.Context, req *samvaad.UpdateParticipantRequest) (*samvaad.ParticipantInfo, error) {
	_, participant, err := r.roomAndParticipantForReq(ctx, req)
	if err != nil {
		return nil, err
	}

	if err = participant.UpdateMetadata(&samvaad.UpdateParticipantMetadata{
		Name:       req.Name,
...
func (r *RoomManager) ForwardParticipant(ctx context.Context, req *samvaad.ForwardParticipantRequest) (*samvaad.ForwardParticipantResponse, error) {
	return nil, errors.New("not implemented")
}

func (r *RoomManager) MoveParticipant(ctx context.Context, req *samvaad.MoveParticipantRequest) (*samvaad.MoveParticipantResponse, error) {
	return nil, errors.New("not implemented")
}

func (r *RoomManager) PerformRpc(ctx context.Context, req *samvaad.PerformRpcRequest) (*samvaad.PerformRpcResponse, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.GetRoom()))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	participant := room.GetParticipant(samvaad.ParticipantIdentity(req.GetDestinationIdentity()))
	if participant == nil {
		return nil, ErrParticipantNotFound
	}
	...
	case result := <-resultChan:
		return &samvaad.PerformRpcResponse{Payload: result}, nil
	case err := <-errorChan:
		return nil, err
	}
}

func (r *RoomManager) DeleteRoom(ctx context.Context, req *samvaad.DeleteRoomRequest) (*samvaad.DeleteRoomResponse, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		...
		err := r.roomStore.DeleteRoom(ctx, samvaad.RoomName(req.Room))
		if err != nil {
	...
	return &samvaad.DeleteRoomResponse{}, nil
}

func (r *RoomManager) UpdateSubscriptions(ctx context.Context, req *samvaad.UpdateSubscriptionsRequest) (*samvaad.UpdateSubscriptionsResponse, error) {
	room, participant, err := r.roomAndParticipantForReq(ctx, req)
	if err != nil {
		return nil, err
	}
	...
	room.UpdateSubscriptions(
		participant,
		samvaad.StringsAsIDs[samvaad.TrackID](req.TrackSids),
		req.ParticipantTracks,
		req.Subscribe,
	)
	return &samvaad.UpdateSubscriptionsResponse{}, nil
}

func (r *RoomManager) SendData(ctx context.Context, req *samvaad.SendDataRequest) (*samvaad.SendDataResponse, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
	...
	room.SendDataPacket(&samvaad.DataPacket{
		Kind:                  req.Kind,
		DestinationIdentities: req.DestinationIdentities,
		Value: &samvaad.DataPacket_User{
			User: &samvaad.UserPacket{
				Payload:               req.Data,
	...
	return &samvaad.SendDataResponse{}, nil
}

func (r *RoomManager) UpdateRoomMetadata(ctx context.Context, req *samvaad.UpdateRoomMetadataRequest) (*samvaad.Room, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
...
func (r *RoomManager) ListDispatch(ctx context.Context, req *samvaad.ListAgentDispatchRequest) (*samvaad.ListAgentDispatchResponse, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
	...
	ret := &samvaad.ListAgentDispatchResponse{
...
func (r *RoomManager) CreateDispatch(ctx context.Context, req *samvaad.AgentDispatch) (*samvaad.AgentDispatch, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
...
func (r *RoomManager) DeleteDispatch(ctx context.Context, req *samvaad.DeleteAgentDispatchRequest) (*samvaad.AgentDispatch, error) {
	room := r.GetRoom(ctx, samvaad.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
...
func (r *RoomManager) iceServersForParticipant(apiKey string, participant types.LocalParticipant, tlsOnly bool) []*samvaad.ICEServer {
	var iceServers []*samvaad.ICEServer
	...
	if r.config.TURN.Enabled {
		...
		if len(urls) > 0 {
			...
			if err != nil {
				...
				iceServers = append(iceServers, &samvaad.ICEServer{
					Urls:       urls,
					Username:   username,
					Credential: password,
				})
...
func (r *RoomManager) setIceConfig(roomName samvaad.RoomName, participant types.LocalParticipant) *samvaad.ICEConfig {
	iceConfig := r.getIceConfig(roomName, participant)
	participant.SetICEConfig(iceConfig)
	return iceConfig
}

func (r *RoomManager) getIceConfig(roomName samvaad.RoomName, participant types.LocalParticipant) *samvaad.ICEConfig {
...
func iceServerForStunServers(servers []string) *samvaad.ICEServer {
	iceServer := &samvaad.ICEServer{}
...
func (h *roomManagerParticipantHelper) GetParticipantInfo(pID samvaad.ParticipantID) *samvaad.ParticipantInfo {
...
func (h *roomManagerParticipantHelper) GetRegionSettings(ip string) *samvaad.RegionSettings {
	return nil
}

func (h *roomManagerParticipantHelper) GetSubscriberForwarderState(lp types.LocalParticipant) (map[samvaad.TrackID]*samvaad.RTPForwarderState, error) {
	return nil, nil
}

func (h *roomManagerParticipantHelper) ResolveMediaTrack(lp types.LocalParticipant, trackID samvaad.TrackID) types.MediaResolverResult {
	return h.room.ResolveMediaTrackForSubscriber(lp, trackID)
}

func (h *roomManagerParticipantHelper) ResolveDataTrack(lp types.LocalParticipant, trackID samvaad.TrackID) types.DataResolverResult {
...
func (h *roomManagerParticipantHelper) GetCachedReliableDataMessage(seqs map[samvaad.ParticipantID]uint32) []*types.DataMessageCache {
