...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/utils/xtwirp"
...
func NewSamvaadServer(conf *config.Config,
	...
	roomServer := samvaad.NewRoomServiceServer(roomService, serverOptions...)
	agentDispatchServer := samvaad.NewAgentDispatchServiceServer(agentDispatchService, serverOptions...)
	egressServer := samvaad.NewEgressServer(egressService, serverOptions...)
	ingressServer := samvaad.NewIngressServer(ingressService, serverOptions...)
	sipServer := samvaad.NewSIPServer(sipService, serverOptions...)
...
func (s *SamvaadServer) Node() *samvaad.Node {
	return s.currentNode.Clone()
}

func (s *SamvaadServer) HTTPPort() int {
	return int(s.config.Port)
}

func (s *SamvaadServer) IsRunning() bool {
	return s.running.Load()
}

func (s *SamvaadServer) Start() error {
	if s.running.Load() {
	...
	logger.Infow("starting Samvaad server", values...)
...
func (s *SamvaadServer) Stop(force bool) {
...
func (s *SamvaadServer) RoomManager() *RoomManager {
	return s.roomManager
}

func (s *SamvaadServer) debugGoroutines(w http.ResponseWriter, _ *http.Request) {
	_ = pprof.Lookup("goroutine").WriteTo(w, 2)
}

func (s *SamvaadServer) debugInfo(w http.ResponseWriter, _ *http.Request) {
...
func (s *SamvaadServer) backgroundWorker() {
