...
func (h *TURNAuthHandler) CreateUsername(apiKey string, pID samvaad.ParticipantID, ttlSeconds int) (string, int64) {
...
func (h *TURNAuthHandler) ParseUsername(username string) (string, samvaad.ParticipantID, int64, error) {
	...
	return parts[0], samvaad.ParticipantID(parts[1]), expiry, nil
}

func (h *TURNAuthHandler) CreatePassword(apiKey string, pID samvaad.ParticipantID, expiry int64) (string, error) {
...
func (h *TURNAuthHandler) computePassword(apiKey string, pID samvaad.ParticipantID, expiry int64) (string, error) {
...
func (h *TURNAuthHandler) HandleAuth(ra *turn.RequestAttributes) (userID string, key []byte, ok bool) {
	...
	password, err := h.computePassword(parts[0], samvaad.ParticipantID(parts[1]), expiry)
	if err != nil {
		logger.Warnw("could not create TURN password", err, "username", decoded)
		return "", nil, false
	}
	return parts[1], turn.GenerateAuthKey(username, SamvaadRealm, password), true
}
