...
func (s *RTCService) validateInternal(
	...
	strict bool,
) (samvaad.RoomName, routing.ParticipantInit, int, error) {
	...
	if wrappedJoinRequestBase64 == "" {
		...
		if wrappedProtoBytes, err := base64.URLEncoding.DecodeString(wrappedJoinRequestBase64); err != nil {
			...
			wrappedJoinRequest := &samvaad.WrappedJoinRequest{}
			...
			case samvaad.WrappedJoinRequest_NONE:
			...
			case samvaad.WrappedJoinRequest_GZIP:
	...
	pi := routing.ParticipantInit{
		Identity:                samvaad.ParticipantIdentity(res.grants.Identity),
		Name:                    samvaad.ParticipantName(res.grants.Name),
		Grants:                  res.grants,
		Region:                  res.region,
	...
	if wrappedJoinRequestBase64 == "" {
		...
		pi.ReconnectReason = samvaad.ReconnectReason(reconnectReason)

		if pi.Reconnect {
			pi.ID = samvaad.ParticipantID(r.FormValue("sid"))
		}
		...
		pi.ID = samvaad.ParticipantID(joinRequest.ParticipantSid)
	}
...
func (s *RTCService) serve(w http.ResponseWriter, r *http.Request, needsJoinRequest bool) {
	...
	var initialResponse *samvaad.SignalResponse
	...
	if !pi.Reconnect && initialResponse.GetJoin() != nil {
		joinRoomID := samvaad.RoomID(initialResponse.GetJoin().GetRoom().GetSid())
		if joinRoomID != "" {
			roomID = joinRoomID
		}

		pi.ID = samvaad.ParticipantID(initialResponse.GetJoin().GetParticipant().GetSid())
		pID = pi.ID
	...
	if pi.Reconnect && pi.ID != "" {
		signalStats.ResolveParticipant(&samvaad.ParticipantInfo{
			Sid:      string(pi.ID),
	...
	go func() {
		...
		for {
			...
			case msg := <-cr.ResponseSource.ReadChan():
				...
				res, ok := msg.(*samvaad.SignalResponse)
				if !ok {
					pLogger.Errorw(
				...
				case *samvaad.SignalResponse_RoomUpdate:
					updateRoomID := samvaad.RoomID(m.RoomUpdate.GetRoom().GetSid())
					if updateRoomID != "" {
						roomID = updateRoomID
						resolveLogger(false)
					}
				...
				case *samvaad.SignalResponse_RoomMoved:
					resetLogger()
					signalStats.Reset()

					roomName = samvaad.RoomName(m.RoomMoved.GetRoom().GetName())
					moveRoomID := samvaad.RoomID(m.RoomMoved.GetRoom().GetSid())
					if moveRoomID != "" {
						roomID = moveRoomID
					}
					participantIdentity = samvaad.ParticipantIdentity(m.RoomMoved.GetParticipant().GetIdentity())
					pID = samvaad.ParticipantID(m.RoomMoved.GetParticipant().GetSid())
					resolveLogger(false)
	...
	for {
		...
		case *samvaad.SignalRequest_Ping:
			count, perr := sigConn.WriteResponse(&samvaad.SignalResponse{
				Message: &samvaad.SignalResponse_Pong{
					//
		...
		case *samvaad.SignalRequest_PingReq:
			count, perr := sigConn.WriteResponse(&samvaad.SignalResponse{
				Message: &samvaad.SignalResponse_PongResp{
					PongResp: &samvaad.Pong{
		...
		switch m := req.Message.(type) {
		case *samvaad.SignalRequest_Offer:
			pLogger.Debugw("received offer", "offer", m)
		case *samvaad.SignalRequest_Answer:
...
) (connectionResult, *samvaad.SignalResponse, error) {
	var cr connectionResult
	var err error
...
func readInitialResponse(source routing.MessageSource, timeout time.Duration) (*samvaad.SignalResponse, error) {
