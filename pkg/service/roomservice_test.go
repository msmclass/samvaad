...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/rpc/rpcfakes"
...
func TestDeleteRoom(t *testing.T) {
	t.Run("missing permissions", func(t *testing.T) {
		...
		_, err := svc.DeleteRoom(ctx, &samvaad.DeleteRoomRequest{
			Room: "testroom",
		})
		require.Error(t, err)
	})
...
func TestMetaDataLimits(t *testing.T) {
	t.Run("metadata exceed limits", func(t *testing.T) {
		...
		ctx := service.WithGrants(context.Background(), grant, "")
		_, err := svc.UpdateParticipant(ctx, &samvaad.UpdateParticipantRequest{
			Room:     "testroom",
			Identity: "123",
			Metadata: "abcdefg",
		})
		...
		_, err = svc.UpdateRoomMetadata(ctx, &samvaad.UpdateRoomMetadataRequest{
			Room:     "testroom",
			Metadata: "abcdefg",
		})
		terr, ok = err.(twirp.Error)
