...
import (
	...
	serverutils "github.com/msmclass/samvaad/pkg/utils"
...
var jobTypeTopics = map[samvaad.JobType]string{
	samvaad.JobType_JT_ROOM:        RoomAgentTopic,
	samvaad.JobType_JT_PUBLISHER:   PublisherAgentTopic,
	samvaad.JobType_JT_PARTICIPANT: ParticipantAgentTopic,
}

type Client interface {
	// LaunchJob starts a room or participant job on an agent.
	// it will launch a job once for each worker in each namespace
	LaunchJob(ctx context.Context, desc *JobRequest) *serverutils.IncrementalDispatcher[*samvaad.Job]
	TerminateJob(ctx context.Context, jobID string, reason rpc.JobTerminateReason) (*samvaad.JobState, error)
	Stop() error
}
...
func (c *agentClient) LaunchJob(ctx context.Context, desc *JobRequest) *serverutils.IncrementalDispatcher[*samvaad.Job] {
	var wg sync.WaitGroup
	ret := serverutils.NewIncrementalDispatcher[*samvaad.Job]()
	defer func() {
...
func (c *agentClient) TerminateJob(ctx context.Context, jobID string, reason rpc.JobTerminateReason) (*samvaad.JobState, error) {
...
func (c *agentClient) getDispatcher(agName string, jobType samvaad.JobType) *serverutils.IncrementalDispatcher[string] {
	c.mu.Lock()
