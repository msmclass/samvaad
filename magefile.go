...
import (
	...
	"github.com/msmclass/samvaad/pkg/samvaad/mageutil"
	_ "github.com/msmclass/samvaad/pkg/samvaad/psrpc"
)

const (
	goChecksumFile = ".checksumgo"
	imageName      = "samvaad-server"
)
...
func Build() error {
	...
	if err := mageutil.RunDir(context.Background(), "cmd/server", "go build -o ../../bin/samvaad-server"); err != nil {
		return err
	}
...
func BuildLinux() error {
	...
	cmd := mageutil.CommandDir(context.Background(), "cmd/server", "go build -buildvcs=false -o ../../bin/samvaad-server-" + buildArch)
	cmd.Env = []string{
		"GOOS=linux",
		"GOARCH=" + buildArch,
		"HOME=" + os.Getenv("HOME"),
