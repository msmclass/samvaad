...
func setupServerWithWebhook() (server *service.SamvaadServer, testServer *webhookTestServer, finishFunc func(), err error) {
...
type webhookTestServer struct {
	server   *http.Server
	events   map[string]*samvaad.WebhookEvent
	lock     sync.Mutex
...
func newTestServer(addr string) *webhookTestServer {
	s := &webhookTestServer{
		events:   make(map[string]*samvaad.WebhookEvent),
...
func (s *webhookTestServer) GetEvent(name string) *samvaad.WebhookEvent {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *webhookTestServer) ClearEvents() {
	s.lock.Lock()
	s.events = make(map[string]*samvaad.WebhookEvent)
	s.lock.Unlock()
}
