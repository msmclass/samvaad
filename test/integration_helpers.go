...
var roomClient samvaad.RoomService
...
func setupSingleNodeTest(name string) (*service.SamvaadServer, func()) {
...
func setupMultiNodeTest(name string) (*service.SamvaadServer, *service.SamvaadServer, func()) {
	logger.Infow("----------------STARTING TEST----------------", "test", name)
...
func waitForServerToStart(s *service.SamvaadServer) {
	// wait till ready
...
func createSingleNodeServer(configUpdater func(*config.Config)) *service.SamvaadServer {
	var err error
	...
	currentNode.SetNodeID(samvaad.NodeID(guid.New(nodeID1)))
	...
	roomClient = samvaad.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d", defaultServerPort), &http.Client{})
	return s
}

func createMultiNodeServer(nodeID string, port uint32) *service.SamvaadServer {
	var err error
	conf, err := config.NewConfig("", true, nil, nil)
	if err != nil {
		panic(fmt.Sprintf("could not create config: %v", err))
	...
	currentNode.SetNodeID(samvaad.NodeID(nodeID))
	...
	roomClient = samvaad.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d", port), &http.Client{})
	return s
}
