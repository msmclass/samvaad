...
func TestAgents(t *testing.T) {
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			defer ac5.close()
			defer ac6.close()
			ac1.Run(samvaad.JobType_JT_ROOM, "default")
			ac2.Run(samvaad.JobType_JT_ROOM, "default")
			ac3.Run(samvaad.JobType_JT_PUBLISHER, "default")
			ac4.Run(samvaad.JobType_JT_PUBLISHER, "default")
			ac5.Run(samvaad.JobType_JT_PARTICIPANT, "default")
			ac6.Run(samvaad.JobType_JT_PARTICIPANT, "default")
...
func TestAgentNamespaces(t *testing.T) {
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			defer ac1.close()
			defer ac2.close()
			ac1.Run(samvaad.JobType_JT_ROOM, "namespace1")
			ac2.Run(samvaad.JobType_JT_ROOM, "namespace2")

			_, err = roomClient.CreateRoom(contextWithToken(createRoomToken()), &samvaad.CreateRoomRequest{
				Name: testRoom,
				Agents: []*samvaad.RoomAgentDispatch{
					{},
					{
						AgentName: "ag",
					},
...
func TestAgentMultiNode(t *testing.T) {
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			defer ac2.close()
			ac1.Run(samvaad.JobType_JT_ROOM, "default")
			ac2.Run(samvaad.JobType_JT_PUBLISHER, "default")
