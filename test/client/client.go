...
import (
	...
	"github.com/msmclass/samvaad/pkg/rtc/transport/transportfakes"
	"github.com/msmclass/samvaad/pkg/rtc/types"
	"github.com/msmclass/samvaad/pkg/sfu/buffer"
)

type SignalRequestHandler func(msg *samvaad.SignalRequest) error
type SignalRequestInterceptor func(msg *samvaad.SignalRequest, next SignalRequestHandler) error
type SignalResponseHandler func(msg *samvaad.SignalResponse) error
type SignalResponseInterceptor func(msg *samvaad.SignalResponse, next SignalResponseHandler) error
...
type RTCClient struct {
	...
	subscribedTracks   map[samvaad.ParticipantID][]*webrtc.TrackRemote
	localParticipant   *samvaad.ParticipantInfo
	remoteParticipants map[samvaad.ParticipantID]*samvaad.ParticipantInfo
	...
	pendingPublishedTracks map[string]*samvaad.TrackInfo
	...
	// map of samvaad.ParticipantID and last packet
	lastPackets   map[samvaad.ParticipantID]*rtp.Packet
	bytesReceived map[samvaad.ParticipantID]uint64

	subscriptionResponse atomic.Pointer[samvaad.SubscriptionResponse]

	nextDataTrackHandle        atomic.Uint32
	pendingPublishedDataTracks map[uint16]*samvaad.DataTrackInfo
	pendingDataTrackWriters    []TrackWriter
	subscribedDataTracks       map[samvaad.ParticipantID]map[uint16]*DataTrackRemote
}
...
func NewWebSocketConn(host, token string, opts *Options) (*websocket.Conn, error) {
	...
	if opts != nil && opts.UseJoinRequestQueryParam {
		...
		connectionSettings := &samvaad.ConnectionSettings{
			AutoSubscribe:          opts.AutoSubscribe,
			AutoSubscribeDataTrack: &opts.AutoSubscribeDataTrack,
		}

		joinRequest := &samvaad.JoinRequest{
		...
		if marshalled, err := proto.Marshal(joinRequest); err == nil {
			wrapped := &samvaad.WrappedJoinRequest{
		...
		if opts != nil {
			...
			if opts.ClientInfo != nil {
				...
				if opts.ClientInfo.Sdk != samvaad.ClientInfo_UNKNOWN {
...
func NewRTCClient(conn *websocket.Conn, useSinglePeerConnection bool, opts *Options) (*RTCClient, error) {
	c := &RTCClient{
		...
		pendingPublishedTracks:     make(map[string]*samvaad.TrackInfo),
		subscribedTracks:           make(map[samvaad.ParticipantID][]*webrtc.TrackRemote),
		remoteParticipants:         make(map[samvaad.ParticipantID]*samvaad.ParticipantInfo),
		me:                         &webrtc.MediaEngine{},
		lastPackets:                make(map[samvaad.ParticipantID]*rtp.Packet),
		bytesReceived:              make(map[samvaad.ParticipantID]uint64),
		pendingPublishedDataTracks: make(map[uint16]*samvaad.DataTrackInfo),
		subscribedDataTracks:       make(map[samvaad.ParticipantID]map[uint16]*DataTrackRemote),
		transportReady:             make(chan struct{}),
	}
	c.nextDataTrackHandle.Store(uint32(rand.IntN(8192)))
	c.ctx, c.cancel = context.WithCancel(context.Background())

	var codecs []*samvaad.Codec
	for _, codec := range []*samvaad.Codec{
		{
			Mime: "audio/opus",
		},
		{
...
func (c *RTCClient) createTransport(rtcconf webrtc.Configuration) error {
	...
	publisherHandler.OnICECandidateCalls(func(ic *webrtc.ICECandidate, t samvaad.SignalTarget) error {
		return c.SendIceCandidate(ic, samvaad.SignalTarget_PUBLISHER)
	})
	...
	if !c.useSinglePeerConnection {
		...
		subscriberHandler.OnICECandidateCalls(func(ic *webrtc.ICECandidate, t samvaad.SignalTarget) error {
			if ic == nil {
				return nil
			}
			return c.SendIceCandidate(ic, samvaad.SignalTarget_SUBSCRIBER)
		})
		...
		subscriberHandler.OnAnswerCalls(func(answer webrtc.SessionDescription, answerId uint32, _midToTrackID map[string]string) error {
			...
			return c.SendRequest(&samvaad.SignalRequest{
				Message: &samvaad.SignalRequest_Answer{
...
func (c *RTCClient) ID() samvaad.ParticipantID {
	return c.id
}
...
func (c *RTCClient) handleSignalResponse(res *samvaad.SignalResponse) error {
	switch msg := res.Message.(type) {
	case *samvaad.SignalResponse_Join:
		c.localParticipant = msg.Join.Participant
		c.id = samvaad.ParticipantID(msg.Join.Participant.Sid)
		c.lock.Lock()
		for _, p := range msg.Join.OtherParticipants {
			c.remoteParticipants[samvaad.ParticipantID(p.Sid)] = p
		}
		c.lock.Unlock()
	...
	case *samvaad.SignalResponse_Answer:
		logger.Infow(
	...
	case *samvaad.SignalResponse_Offer:
	...
	case *samvaad.SignalResponse_Trickle:
		candidateInit, err := signalling.FromProtoTrickle(msg.Trickle)
		if err != nil {
			return err
		}
		if msg.Trickle.Target == samvaad.SignalTarget_PUBLISHER {
			c.publisher.AddICECandidate(candidateInit)
		} else {
			c.subscriber.AddICECandidate(candidateInit)
		}

	case *samvaad.SignalResponse_Update:
		c.lock.Lock()
		for _, p := range msg.Update.Participants {
			if samvaad.ParticipantID(p.Sid) != c.id {
				if p.State != samvaad.ParticipantInfo_DISCONNECTED {
					c.remoteParticipants[samvaad.ParticipantID(p.Sid)] = p
				} else {
					delete(c.remoteParticipants, samvaad.ParticipantID(p.Sid))
				}
			}
		}
		c.lock.Unlock()

	case *samvaad.SignalResponse_TrackPublished:
		logger.Debugw(
			"track published",
			"participant", c.localParticipant.Identity,
	...
	case *samvaad.SignalResponse_RefreshToken:
		c.lock.Lock()
		c.refreshToken = msg.RefreshToken
		c.lock.Unlock()

	case *samvaad.SignalResponse_TrackUnpublished:
	...
	case *samvaad.SignalResponse_Pong:
		c.pongReceivedAt.Store(msg.Pong)

	case *samvaad.SignalResponse_SubscriptionResponse:
		c.subscriptionResponse.Store(msg.SubscriptionResponse)

	case *samvaad.SignalResponse_MediaSectionsRequirement:
		logger.Infow(
	...
	case *samvaad.SignalResponse_PublishDataTrackResponse:
		logger.Debugw(
	...
	case *samvaad.SignalResponse_DataTrackSubscriberHandles:
		logger.Infow(
		...
		for handle, publishedDataTrack := range msg.DataTrackSubscriberHandles.SubHandles {
			publisherID := samvaad.ParticipantID(publishedDataTrack.PublisherSid)
			...
			if tracks[uint16(handle)] == nil {
				tracks[uint16(handle)] = NewDataTrackRemote(
					samvaad.ParticipantIdentity(publishedDataTrack.PublisherIdentity),
					samvaad.ParticipantID(publishedDataTrack.PublisherSid),
					uint16(handle),
					samvaad.TrackID(publishedDataTrack.TrackSid),
...
func (c *RTCClient) ReadResponse() (*samvaad.SignalResponse, error) {
	for {
...
func (c *RTCClient) SubscribedTracks() map[samvaad.ParticipantID][]*webrtc.TrackRemote {
	// create a copy of this
	c.lock.Lock()
	defer c.lock.Unlock()
	tracks := make(map[samvaad.ParticipantID][]*webrtc.TrackRemote, len(c.subscribedTracks))
	maps.Copy(tracks, c.subscribedTracks)
	return tracks
}

func (c *RTCClient) SubscribedDataTracks() map[samvaad.ParticipantID]map[uint16]*DataTrackRemote {
	// create a copy of this
	c.lock.Lock()
	defer c.lock.Unlock()
	tracks := make(map[samvaad.ParticipantID]map[uint16]*DataTrackRemote, len(c.subscribedDataTracks))
...
func (c *RTCClient) RemoteParticipants() []*samvaad.ParticipantInfo {
	c.lock.Lock()
	defer c.lock.Unlock()
	return funk.Values(c.remoteParticipants).([]*samvaad.ParticipantInfo)
}

func (c *RTCClient) GetRemoteParticipant(sid samvaad.ParticipantID) *samvaad.ParticipantInfo {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *RTCClient) Stop() {
	...
	_ = c.SendRequest(&samvaad.SignalRequest{
		Message: &samvaad.SignalRequest_Leave{
			Leave: &samvaad.LeaveRequest{
				Reason: samvaad.DisconnectReason_CLIENT_INITIATED,
				Action: samvaad.LeaveRequest_DISCONNECT,
			},
		},
	})
...
func (c *RTCClient) GetSubscriptionResponseAndClear() *samvaad.SubscriptionResponse {
	return c.subscriptionResponse.Swap(nil)
}

func (c *RTCClient) SendPing() error {
	return c.SendRequest(&samvaad.SignalRequest{
		Message: &samvaad.SignalRequest_Ping{
			Ping: time.Now().UnixNano(),
		},
	})
}

func (c *RTCClient) SendRequest(msg *samvaad.SignalRequest) error {
...
func (c *RTCClient) SendIceCandidate(ic *webrtc.ICECandidate, target samvaad.SignalTarget) error {
	...
	return c.SendRequest(&samvaad.SignalRequest{
		Message: &samvaad.SignalRequest_Trickle{
...
func (c *RTCClient) SetAttributes(attrs map[string]string) error {
	return c.SendRequest(&samvaad.SignalRequest{
		Message: &samvaad.SignalRequest_UpdateMetadata{
			UpdateMetadata: &samvaad.UpdateParticipantMetadata{
				Attributes: attrs,
			},
		},
	})
...
func (c *RTCClient) SendAddTrack(cid string, mimeType string, name string, trackType samvaad.TrackType) error {
	return c.SendRequest(&samvaad.SignalRequest{
		Message: &samvaad.SignalRequest_AddTrack{
...
func (c *RTCClient) PublishData(data []byte, kind samvaad.DataPacket_Kind) error {
	...
	dpData, err := proto.Marshal(&samvaad.DataPacket{
		Value: &samvaad.DataPacket_User{
			User: &samvaad.UserPacket{Payload: data},
		},
	})
	if err != nil {
		return err
...
func (c *RTCClient) PublishDataTrack() (writer TrackWriter, err error) {
	...
	if err = c.SendRequest(&samvaad.SignalRequest{
		Message: &samvaad.SignalRequest_PublishDataTrackRequest{
			PublishDataTrackRequest: &samvaad.PublishDataTrackRequest{
...
func (c *RTCClient) handleDataMessage(kind samvaad.DataPacket_Kind, data []byte) {
	...
	dp.Kind = kind
	if val, ok := dp.Value.(*samvaad.DataPacket_User); ok {
...
func (c *RTCClient) handleMediaSectionsRequirement(mediaSectionsRequirement *samvaad.MediaSectionsRequirement) {
...
func (c *RTCClient) onOffer(offer webrtc.SessionDescription, offerId uint32, midToTrackID map[string]string) error {
	...
	return c.SendRequest(&samvaad.SignalRequest{
		Message: &samvaad.SignalRequest_Offer{
...
func (c *RTCClient) processRemoteTrack(track *webrtc.TrackRemote) {
	...
	if trackID == "" {
		trackID = samvaad.TrackID(track.ID())
	}
	c.lock.Lock()
