...
import (
	...
	testclient "github.com/msmclass/samvaad/test/client"
)
...
func TestSinglePublisher(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			audioTrack := funk.Find(remoteC1.Tracks, func(ti *samvaad.TrackInfo) bool {
...
func TestConnectionStats(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			receiverCaptures := make(map[samvaad.TrackID]*statCapture)
			downTrackCaptures := make(map[samvaad.ParticipantIdentity]map[samvaad.TrackID]*statCapture)
			for _, identity := range []samvaad.ParticipantIdentity{"c1", "c2"} {
				...
				for _, mt := range p.GetPublishedTracks() {
					...
					for _, r := range mt.Receivers() {
						...
						wr.OnStatsUpdate(func(_ *sfu.WebRTCReceiver, stat *samvaad.AnalyticsStat) {
							rc.lock.Lock()
							rc.stat = stat
							rc.lock.Unlock()
						})
				...
				dtCaps := make(map[samvaad.TrackID]*statCapture)
				...
				for _, st := range p.GetSubscribedTracks() {
					...
					dc := &statCapture{}
					dtCaps[st.ID()] = dc
					dt.OnStatsUpdate(func(_ *sfu.DownTrack, stat *samvaad.AnalyticsStat) {
						dc.lock.Lock()
						dc.stat = stat
						dc.lock.Unlock()
					})
			...
			validateAnalyticsStat := func(stat *samvaad.AnalyticsStat) string {
				if stat == nil {
					return "stat nil"
				}
...
func TestSingleNodeUpdateParticipant(t *testing.T) {
	...
	t.Run("update nonexistent participant", func(t *testing.T) {
		_, err := roomClient.UpdateParticipant(adminCtx, &samvaad.UpdateParticipantRequest{
			Room:     testRoom,
			Identity: "nonexistent",
			Permission: &samvaad.ParticipantPermission{
				CanPublish: true,
			},
		})
		require.Error(t, err)
...
func TestSingleNodeDoubleSlash(t *testing.T) {
	...
	roomClient = samvaad.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d/", s.HTTPPort()), &http.Client{})
	_, err := roomClient.ListRooms(contextWithToken(listRoomToken()), &samvaad.ListRoomsRequest{})
	require.NoError(t, err)
}
...
func TestAutoCreate(t *testing.T) {
	...
	t.Run("join with explicit createRoom", func(t *testing.T) {
		...
		// explicitly create
		_, err := roomClient.CreateRoom(contextWithToken(createRoomToken()), &samvaad.CreateRoomRequest{Name: testRoom})
		require.NoError(t, err)
...
func TestSingleNodeUpdateSubscriptionPermissions(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err = roomClient.UpdateParticipant(ctx, &samvaad.UpdateParticipantRequest{
				Room:     testRoom,
				Identity: "sub",
				Permission: &samvaad.ParticipantPermission{
					CanSubscribe: true,
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				require.Equal(t, samvaad.SubscriptionError_SE_CODEC_UNSUPPORTED, sr.Err)
				return true
		...
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			go func() {
				...
				for !stopWrite.Load() {
					...
					if err := pub.PublishData(buf, samvaad.DataPacket_RELIABLE); err != nil {
...
func TestTurnAuthFailure(t *testing.T) {
	...
	pID := samvaad.ParticipantID("PA_authfail")
