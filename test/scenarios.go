	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("scenarioDataPublish/testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			c2.OnDataReceived = func(data []byte, sid string) {
				if string(data) == payload && samvaad.ParticipantID(sid) == c1.ID() {
					received.Store(true)
				}
			}

			require.NoError(t, c1.PublishData([]byte(payload), samvaad.DataPacket_RELIABLE))
...
func scenarioJoinClosedRoom(t *testing.T) {
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err := roomClient.DeleteRoom(contextWithToken(createRoomToken()), &samvaad.DeleteRoomRequest{
				Room: testRoom,
			})
...
func closeNonRTCRoom(t *testing.T) {
	...
	_, err := roomClient.CreateRoom(createCtx, &samvaad.CreateRoomRequest{
		Name: testRoom,
	})
	require.NoError(t, err)

	_, err = roomClient.DeleteRoom(createCtx, &samvaad.DeleteRoomRequest{
		Room: testRoom,
	})
	require.NoError(t, err)
}
...
func roomServiceListRoom(t *testing.T) {
	...
	t.Run("list all rooms", func(t *testing.T) {
		res, err := roomClient.ListRooms(listCtx, &samvaad.ListRoomsRequest{})
		require.NoError(t, err)
