...
func TestMultiNodeUpdateRoomMetadata(t *testing.T) {
	...
	t.Run("when room is empty", func(t *testing.T) {
		...
		rm, err := roomClient.UpdateRoomMetadata(contextWithToken(adminRoomToken("emptyRoom")), &samvaad.UpdateRoomMetadataRequest{
			Room:     "emptyRoom",
...
func TestMultiNodeRemoveParticipant(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err := roomClient.RemoveParticipant(ctx, &samvaad.RoomParticipantIdentity{
				Room:     testRoom,
			...
			listRes, err := roomClient.ListParticipants(ctx, &samvaad.ListParticipantsRequest{
				Room: testRoom,
			})
			require.NoError(t, err)
...
func TestMultiNodeUpdateParticipantMetadata(t *testing.T) {
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			ctx := contextWithToken(adminRoomToken(testRoom))
			res, err := roomClient.UpdateParticipant(ctx, &samvaad.UpdateParticipantRequest{
				Room:     testRoom,
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			testutils.WithTimeout(t, func() string {
				res, err := roomClient.GetParticipant(ctx, &samvaad.RoomParticipantIdentity{
					Room:     testRoom,
					Identity: identity,
				})
				require.NoError(t, err)
			...
			res, err := roomClient.MutePublishedTrack(ctx, &samvaad.MuteRoomTrackRequest{
				Room:     testRoom,
				Identity: identity,
				TrackSid: trackIDs[0],
				Muted:    true,
