...
import (
	...
	"github.com/msmclass/samvaad/test/client"
)
...
func TestMultiNodeRouting(t *testing.T) {
	...
	_, err := roomClient.CreateRoom(contextWithToken(createRoomToken()), &samvaad.CreateRoomRequest{
		Name: testRoom,
	})
...
func TestMultiNodeUpdateAttributes(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err := roomClient.UpdateParticipant(contextWithToken(adminRoomToken(testRoom)), &samvaad.UpdateParticipantRequest{
				Room:     testRoom,
				Identity: "au1",
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			c2 := createRTCClient("c2", defaultServerPort, testRTCServicePath, &client.Options{
				SignalRequestInterceptor: func(msg *samvaad.SignalRequest, next client.SignalRequestHandler) error {
					switch msg.Message.(type) {
					case *samvaad.SignalRequest_Offer, *samvaad.SignalRequest_Answer, *samvaad.SignalRequest_Leave:
						return nil
					default:
						return next(msg)
					}
				},
				SignalResponseInterceptor: func(msg *samvaad.SignalResponse, next client.SignalResponseHandler) error {
					switch msg.Message.(type) {
					case *samvaad.SignalResponse_Offer, *samvaad.SignalResponse_Answer:
						return nil
					default:
						return next(msg)
					}
